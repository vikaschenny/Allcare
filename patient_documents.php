				...
				$( "#emailform" ).dialog({							      
				      ...
				      buttons: {
				        "Send Email": function() {
				          ...
				          if ( bValid ) {
				            $( "#emailresult" ).append( "send email.. "+ email.val() );
				            $.post("email_doc.php", { docid: docid, recipient: email.val(), pid: pid })						
							.done(function(data) {											
								alert(data);						  		
							});
				            $( this ).dialog( "close" );
...
if(isset($_REQUEST['id']) && ($_REQUEST['id'] != '')) {
	$sql = "SELECT pdata.id, pdata.fname, pdata.lname, pdata.email, pdata.providerID, docs.id as docid, docs.url, docs.mimetype, docs.status FROM patient_data as pdata INNER JOIN documents AS docs ON pdata.pid = docs.foreign_id INNER JOIN users ON
		...
		while ($docRec = sqlFetchArray($documentRes))
		{
			...
			echo '<div class="tableData">
					...
					<div id="" class="actions">
						<select class="docOps" name="action" rel="'.$iterator.'" pid="'.$_REQUEST['id'].'" docid="'.$docRec['docid'].'">
