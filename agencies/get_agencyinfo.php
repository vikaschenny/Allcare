...
//starting the PHP session (also regenerating the session id to avoid session fixation attacks)
    ...
    require_once($webserver_root.'/interface/globals.php');
    ...
    DEFINE("TBL_PAT_ACC_ON","tbl_allcare_agencyportal");
    DEFINE("COL_PID","uid");
    DEFINE("COL_POR_PWD","portal_password");
    ...
    $sql = "SELECT * FROM `tbl_addrbk_custom_attr_1to1` WHERE `addrbk_type_id` = ?";

    if ($userData = sqlQuery($sql, array($auth['uid']) )) { // if query gets executed
        ...
        if ($auth['uid'] != $userData['addrbk_type_id']) {
    ...
    if(str_replace(" ","+",$_REQUEST['param'])!='' ){
        $param  = GibberishAES::dec(str_replace(" ","+",$_REQUEST['param']), 'rotcoderaclla');
        header('Location: drive_view/view_file.php?file_id='.$param);
    }else{
