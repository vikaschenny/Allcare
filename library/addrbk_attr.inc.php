...
function getAddrbkAttr($id , $given="*") {
    $sql = "select $given from tbl_addrbk_custom_attr_1to1 where addrbk_type_id=?  limit 0,1";
    return sqlQuery($sql, array($id) );
}

function updateAddrbkAttr($id, $new, $create)
{
  /*******************************************************************
  ...
  *******************************************************************/
  ...
  if ($create) {
    $sql = "INSERT INTO tbl_addrbk_custom_attr_1to1 SET  addrbk_type_id='$id',created_date = NOW()";
   ...
   //echo $db_id = $new['id'];
    $rez = sqlQuery("SELECT * FROM tbl_addrbk_custom_attr_1to1 WHERE addrbk_type_id = '$id'");
    $sql = "UPDATE tbl_addrbk_custom_attr_1to1 SET updated_date = NOW()";
    foreach ($new as $key => $value) {
      $sql .= ", `$key` = " . pdValueOrNull($key, $value);
    }
    $sql .= " WHERE addrbk_type_id = '$id'";
    sqlStatement($sql);
  }
