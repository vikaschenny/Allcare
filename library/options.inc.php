...
function get_dos($pid) {
   
  return sqlStatement("SELECT fe.date,DATE_FORMAT(fe.date,'%m/%d/%Y') as fdate
                FROM forms f
...
// date-time
    ...
    echo '<link rel="stylesheet" type="text/css" href="/library/css/jquery.datetimepicker.custom.css"/ >
        <script type="text/javascript" src="/library/js/jquery-datetimepicker.js"></script>
       <script type="text/javascript" src="/library/js/jquery.datetimepicker.custom.js"></script>';
      
      echo "<input type='text' name='form_$field_id_esc' id='form_$field_id_esc'  value='$currescaped' placeholder='Click here to choose date and time'>";
      ?>
    <script type="text/javascript">
     jQuery.noConflict(); 
     jQuery('#form_<?php echo $field_id_esc; ?>' ).datetimepicker();
   </script>
    <?php
//    echo "<input type='text' size='10' name='form_$field_id_esc' id='form_$field_id_esc'" .
//      " value='$currescaped'" .
...
//      "<img src='$rootdir/pic/show_calendar.gif' align='absbottom' width='24' height='22'" .
//      " id='img_$field_id_esc' border='0' alt='[?]' style='cursor:pointer'" .
//      " title='" . htmlspecialchars( xl('Click here to choose a date and time'), ENT_QUOTES) . "' />";
//    $date_init .= " Calendar.setup({inputField:'form_$field_id', ifFormat:'%Y-%m-%d %h-%i-%s', button:'img_$field_id'});\n";
  }
  ...
  //dos
    ...
    echo "<select name='form_$field_id_esc' id='form_$field_id_esc' title='$description'>";
    echo "<option value='0'></option>";
    $pres = get_dos($_SESSION['pid']); 
    while ($prow = sqlFetchArray($pres)) {
      $key = $prow['date'];
      $optionValue = htmlspecialchars( $key, ENT_QUOTES);
      $optionLabel = htmlspecialchars($prow['fdate'], ENT_NOQUOTES);
      echo "<option value='$optionValue'";
      if ($currvalue == $key) echo " selected";
      echo ">$optionLabel</option>";
    }
  ...
  // insurance company
    ...
    dropdown_insurancecompany($selected = $currvalue, $name = "form_$field_id_esc", $allow_unspecified = true, $allow_allinsurancecompanies = false);
  }
  //users list
    ...
    $ures = sqlStatement("SELECT id, fname, lname, specialty FROM users " .
      ...
      "ORDER BY lname, fname");
    $tmp = explode('|', $currvalue);
    $nam='form_'.$field_id_esc.'[]';
    echo "<select name='$nam' id='form_$field_id_esc' multiple title='$description'>";
    echo "<option value=''>" . htmlspecialchars(xl($empty_title), ENT_NOQUOTES) . "</option>";
    while ($urow = sqlFetchArray($ures)) {
      $uname = htmlspecialchars( $urow['fname'] . ' ' . $urow['lname'], ENT_NOQUOTES);
      $optionId = htmlspecialchars( $urow['id'], ENT_QUOTES);
      echo "<option value='$optionId'"; foreach($tmp as $val1) {
    ...
    $ures = sqlStatement("SELECT id, name FROM insurance_companies " .
      "ORDER BY name");
    $tmp = explode('|', $currvalue);
    $nam='form_'.$field_id_esc.'[]';
    echo "<select name='$nam' id='form_$field_id_esc' multiple title='$description'>";
    echo "<option value=''>" . htmlspecialchars(xl($empty_title), ENT_NOQUOTES) . "</option>";
    while ($urow = sqlFetchArray($ures)) {
      $uname = htmlspecialchars(  $urow['name'], ENT_NOQUOTES);
      $optionId = htmlspecialchars( $urow['id'], ENT_QUOTES);
      echo "<option value='$optionId'"; foreach($tmp as $val1) {
      if ($urow['id'] == $val1) echo " selected"; }
      echo ">$uname</option>";
...
// date time
  ...
  //facilities drop-down list
    ...
    *****************************************************************/
    ...
    $tmp = '';
    if ($currvalue) {
      $urow = sqlQuery("SELECT fname, lname, username FROM users " .
        "WHERE id = ?", array($currvalue) );
      $tmp = ucwords($urow['fname'] . " " . $urow['lname']);
      if (empty($tmp)) $tmp = "($currvalue)";
    }
...
function generate_display_field($frow, $currvalue) {
  ...
  /*else if ($data_type == 14) {
    ...
    if (strpos($frow['edit_options'], 'L') !== FALSE)
      ...
      $tmp = "( username = '' OR authorized = 1 )";
    $ures = sqlStatement("SELECT id, fname, lname, organization, username FROM users " .
      "WHERE id=? AND active = 1 AND ( info IS NULL OR info NOT LIKE '%Inactive%' ) " .
      "AND $tmp " .
      "ORDER BY organization, lname, fname",array($currvalue));
    while ($urow = sqlFetchArray($ures)) {
      $uname = $urow['organization'];
      if (empty($uname) || substr($uname, 0, 1) == '(') {
        $uname = $urow['lname'];
  ...
  // dos
    ...
    while ($prow = sqlFetchArray($pres)) {
      $key = $prow['date'];
      if ($currvalue == $key) {
        $s .= htmlspecialchars($prow['fdate'] ,ENT_NOQUOTES);
      }
    }
  }    
 // insurance companies
else if ($data_type == 37) {
    $urow = sqlQuery("SELECT id, name FROM insurance_companies ".
      "WHERE id = ?", array($currvalue) );
    $s = htmlspecialchars($urow['name'],ENT_NOQUOTES);
  }
  //Users list
    ...
    $s = htmlspecialchars(ucwords($urow['fname'] . " " . $urow['lname']),ENT_NOQUOTES);
        ...
        $tmp_val = explode('|', $currvalue);
        foreach($tmp_val as $key => $val2){
            if ($val2) {
              $urow = sqlQuery("SELECT fname, lname, specialty FROM users " .
                "WHERE id = ?", array($val2) );
              $s .= htmlspecialchars(ucwords($urow['fname'] . " " . $urow['lname']),ENT_NOQUOTES). "<br />";
        ...
        foreach($tmp_val as $key => $val2){
            if ($val2) {
              $urow = sqlQuery("SELECT id, name FROM insurance_companies " .
                "WHERE id = ?", array($val2) );
              $s .= htmlspecialchars(ucwords( $urow['name']),ENT_NOQUOTES). "<br />";
...
// Generate JavaScript Group validation logic for the required fields.
//
function generate_group_validation($form_id, $groupname) {
  $fres = sqlStatement("SELECT * FROM layout_options " .
    "WHERE form_id = ? AND uor > 0 AND field_id != ''  AND group_name LIKE '%$groupname'" .
    "ORDER BY group_name, seq", array($form_id) );
...
function dropdown_insurancecompany($selected = '', $name = 'form_insurancecompany', $allow_unspecified = true, $allow_allinsurancecompanies = true) {
  $have_selected = false;
  ...
  if ($allow_allinsurancecompanies) {
    $option_value = '';
    $option_selected_attr = '';	
    if ($selected == '') {
      $option_selected_attr = ' selected="selected"';
    ...
    $option_content = htmlspecialchars('-- ' . xl('All Insurance Companies') . ' --', ENT_NOQUOTES);
    echo "    <option value=\"$option_value\" $option_selected_attr>$option_content</option>\n";
  } elseif ($allow_unspecified) {
  	$option_value = '0';
    $option_selected_attr = '';
  ...
  while ($frow = sqlFetchArray($fres)) {
    ...
    $option_selected_attr = '';
    if ($selected == $insurancecompany_id) {
      $option_selected_attr = ' selected="selected"';
      $have_selected = true;
    }
    $option_content = htmlspecialchars($frow['name'], ENT_NOQUOTES);
  ...
  if ($allow_unspecified && $allow_allinsurancecompanies) {
    $option_value = '0';
    $option_selected_attr = '';
    if ( $selected == '0' ) {
      $option_selected_attr = ' selected="selected"';
