...
// Patient screen Order : included common function file
...
function display_layout_rows($formtype, $result1, $result2='') {
  ...
  while ($frow = sqlFetchArray($fres)) {
    ...
    if ($formtype == 'DEM') {//echo "<script>alert('DEM=111');</script>";
    ...
    if (strcmp($this_group, $last_group) != 0) {//echo "<script>alert('DEM=222');</script>";
  ...
  $fres = sqlStatement("SELECT distinct group_name FROM layout_options " .
	...
	$first = false;
        ...
        <div class="tab ">
                ...
                <?php 
                     echo allcare1t01po($GLOBALS['pid']);                
                ?>
        ...
        <div class="tab ">
          ...
          <?php 
                    echo facilitylistview($GLOBALS['pid'],'addmode');                
          ?>
        </div>  
        ...
        <div class="tab ">
          ...
          <?php 
                  echo agencylistview($GLOBALS['pid'],'addmode');                
          ?>
        </div> 
        <?php
		  ...
		  $FieldSql =  "SELECT fg.Grouping_ID,fg.Table_ID,fg.POS_id, tb.Field_Name, tb.Table_Name
				...
				//$_SESSION['group_id_array'][$totalGroupCountForThisPatient]=array();
                       $_SESSION['group_id_array'][$totalGroupCountForThisPatient]=$FieldRes['Grouping_ID'];     
                                 ...
                                 $field1to1Sql ="select ".$FieldRes['Field_Name']." from tbl_allcare_patients1to1 where pos_id=".$existingPOSSRes['pos_id']." and pid=".$GLOBALS['pid']; 
 ...
 foreach ($recorsetIdarray as $key => $value) 
     {
      ...
      echo "<a id='new".$existingPOSSRes['pos_id'].$value."' href=''>Add new row</a>";
...
group by fg.Recordset_ID,fg.Field_ID order by tb.Field_Name";
	   ...
	   if(sqlNumRows($FieldSqlQry)>0)
	   {
		    while($FieldRes = sqlFetchArray($FieldSqlQry))     
		    {
		         ...
		         $field1to1Sql ="select ".$FieldRes['Field_Name']." from  tbl_allcare_patients1ton 
		                         where pos_id=".$existingPOSSRes['pos_id']." and pid=".$GLOBALS['pid']; 
		         $field1to1Qry = sqlStatement($field1to1Sql);
	...
	$RecordsetFieldNames = substr($RecordsetFieldNames,0,strlen($RecordsetFieldNames)-1);
	   ...
	   if(sqlNumRows($FieldSqlQry)>0)
	   {
		    ...
		    where pos_id=".$existingPOSSRes['pos_id']." and pid=".$GLOBALS['pid']." and Recordset_ID=".$value."";
...
echo "<table border=0  class='label'><tr><td width=25% align=left class='fontbold'>POS Type :</td>";
  echo "<td align='left'>";  
  echo "<select name='lstPOS' id='lstPOS' style='width:250px;' onchange='return showMapping(this.value);'>"
  . "<option value=0>- POS Type -</option>";
  ...
  $POS1tonSql = "SELECT pt.id,pt.title from tbl_pos_types pt 
                    ...
                    and pt.id not in(select pos_id from tbl_allcare_patients1to1 where pid=".$GLOBALS['pid'].")
  ...
  $POS1to1Sql = "SELECT pt.id,pt.title from tbl_pos_types pt 
                   ...
                   and pt.id not in(select pos_id from tbl_allcare_patients1ton where pid=".$GLOBALS['pid'].")
                    group by pt.title order by pt.title";
...
function showMapping(POSid)
{
    ...
    if(POSid!=0)
    {
        var AjxURL=document.URL.split('/patient_file')[0]+'/new/show_mapping.php?POSid='+POSid; 
        //alert(AjxURL);
...
<div id='divResponse1'></div>";
?>

</td></tr>
    </tbody></table>
                </div>
        <div class="tab " id="tab_facility">
            <div id='tooltipdiv'
...
></div>
            ...
            <div id='idFacilityAddButton'>
                  <a class="css_button" id='' href="javascript:void(0);" onclick="facilityformview('addview','','','','','','','')">
                    <span>Add</span>
                  </a> 
            </div>    
            <br><br>
             <?php echo facilitylistAddEditview($GLOBALS['pid'],'viewmode');   ?>
            <br>
            <center>
                
            <table cellpadding="10" cellspacing="0" width="60%" id="tblFacility" style="display:none;"><tbody>
            <?php
               $fres = sqlStatement("select id,name from facility order by name");
                if ($fres) {
                    echo "<tr class='text'><td width='40%'><span class='bold'>Facility </span></td><td  width='60%'>";
                    echo "<select name='selectFacility' id='selectFacility'>";
                    echo "<option value='0'>-Select</option>";
                  while ($frow = sqlFetchArray($fres))
                        {
                          
                          echo "<option value=".$frow['id'].">".$frow['name']."</option>";
                        }
                        
                   echo "</select>"; 
                   echo "</td></tr>";     
                   
                   echo "<tr class='text'><td><span class='bold'>Admit Date *</span></td>";
                   echo "<td> 
                       <input type='text' size='16' id='admitdate'   name='admitdate'  placeholder='yyyy-mm-dd' value='' required onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='yyyy-mm-dd'>
                          <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' id='img_admitdate' border='0' alt='[?]' style='cursor:pointer' title='Click here to choose a date'>
                       </td></tr>";
                   echo "<tr class='text'><td><span class='bold'>Discharge Date</span></td>";
                   echo "<td> 
                       <input type='text' size='16' id='dischargedate'  name='dischargedate' value='' onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='yyyy-mm-dd' placeholder='yyyy-mm-dd' onchange='facilitydatecomparison()'>
                          <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' id='img_dischargedate' border='0' alt='[?]' style='cursor:pointer' title='Click here to choose a date'>
                       </td></tr>";
                   
                   echo "<script>Calendar.setup({inputField:'admitdate', ifFormat:'%Y-%m-%d', button:'img_admitdate'});</script>";
                   echo "<script>Calendar.setup({inputField:'dischargedate', ifFormat:'%Y-%m-%d', button:'img_dischargedate'});</script>";
                   
                   echo "<tr class='text'><td><span class='bold'>Is Active</span></td>";
                   echo "<td><input type='checkbox' id='chkFacilityActive' name='chkFacilityActive' value='1'></td></tr>";
                   
                   echo "<tr class='text'><td valign='top'><span class='bold'>Notes</span></td>";
                   echo "<td><textarea  id='facilitynotes' name='facilitynotes' rows=3 cols=21></textarea></td></tr>";
                   
                   echo "<tr class='text'><td valign='top'><span class='bold'>Related Document Links</span></td>";
                   echo "<td><textarea  id='facilitydoclinks' name='facilitydoclinks' rows=3 cols=21></textarea><br><b>Note: Enter Multiple Document Links with Comma ',' Separation..<b></td></tr>";
                   
                }   
            ?>
                <input type='hidden' name='hideFacilityStatus' id='hideFacilityStatus' value="add">
                <input type='hidden' name='hideFacilityformid' id='hideFacilityformid' value="0">
                <td>
                ...
                <tr><td colspan="2">
                    ...
                    <a class="css_button" href="javascript:void(0);" onclick="facilityformview('listview','','','','','','','')">
						<span>Back</span>
			</a> 
                    </td></tr> 
            </table>
...
<div class="tab " id="tab_agency">
  <div id='tooltipdiv'
 style='position:absolute;width:400pt;border:1px solid black;padding:2px;background-color:#ffffaa;visibility:hidden;z-index:1000;font-size:9pt;'
></div>

            
            <div id='idagencyAddButton'>
                  <a class="css_button" id='' href="javascript:void(0);" onclick="agencyformview('addview','','','','','','','')">
                    <span>Add</span>
                  </a> 
            </div>    
            <br><br>
             <?php echo agencylistAddEditview($GLOBALS['pid'],'viewmode');   ?>
            <br>
            <center>
                
            <table cellpadding="10" cellspacing="0" width="80%" id="tblagency" style="display:none;"><tbody>
            <?php
               $fres = sqlStatement("SELECT id, list_options.title, users.organization
                                        FROM users
                                        JOIN list_options ON users.abook_type = list_options.option_id
                                        AND users.organization !=  ''
                                        AND users.abook_type !=  ''
                                        AND list_id =  'abook_type' ");
                if ($fres) {
                    echo "<tr class='text'><td width='40%'><span class='bold'>Agency Name</span></td><td  width='60%'>";
                    echo "<select name='selectagency' id='selectagency'>";
                    echo "<option value='0'>Select</option>";
                  while ($frow = sqlFetchArray($fres))
                        {
                          
                          echo "<option value=".$frow['id'].">".$frow['title'].'-'.$frow['organization']."</option>";
                        }
                   ...
                   echo "<tr class='text'><td><span class='bold'>Admit Date *</span></td>";
                   echo "<td> 
                        <input type='text' size='16' id='orgadmitdate'   name='orgadmitdate'  placeholder='yyyy-mm-dd' value='' required onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='yyyy-mm-dd' >
                        <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' id='img_admitdate2' border='0' alt='[?]' style='cursor:pointer' title='Click here to choose a date'>
                        </td></tr>";
                   echo "<tr class='text'><td><span class='bold'>Discharge Date</span></td>";
                   echo "<td> 
                        <input type='text' size='16' id='orgdischargedate'  name='orgdischargedate' value='' onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='yyyy-mm-dd' placeholder='yyyy-mm-dd' onchange='agencydatecomparison()'>
                          <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22' id='img_dischargedate2' border='0' alt='[?]' style='cursor:pointer' title='Click here to choose a date'>
                        </td></tr>";
                   
                   echo "<script>Calendar.setup({inputField:'orgadmitdate', ifFormat:'%Y-%m-%d', button:'img_admitdate2'});</script>";
                   echo "<script>Calendar.setup({inputField:'orgdischargedate', ifFormat:'%Y-%m-%d', button:'img_dischargedate2'});</script>";
                   
                   echo "<tr class='text'><td><span class='bold'>Is Active</span></td>";
                   echo "<td><input type='checkbox' id='chkagencyActive' name='chkagencyActive' value='1'></td></tr>";
                   
                   echo "<tr class='text'><td valign='top'><span class='bold'>Notes</span></td>";
                   echo "<td><textarea  id='agencynotes' name='agencynotes' rows=3 cols=21></textarea></td></tr>";
                   
                   echo "<tr class='text'><td valign='top'><span class='bold'>Related Document Links</span></td>";
                   echo "<td><textarea  id='agencydoclinks' name='agencydoclinks' rows=3 cols=21></textarea><br><b>Note: Enter Multiple Document Links with Comma ',' Separation..<b></td></tr>";
                  
                  }   
            ?>
                <input type='hidden' name='hideagencyStatus' id='hideagencyStatus' value="add">
                <input type='hidden' name='hideagencyformid' id='hideagencyformid' value="0">
                <td>
                ...
                <tr><td colspan="2">
                    ...
                    <a class="css_button" href="javascript:void(0);" onclick="agencyformview('listview','','','','','','','')">
						<span>Back</span>
			</a> 
                    </td></tr> 
            </table>
