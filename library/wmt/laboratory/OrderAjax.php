...
if ($type == 'details') {
	...
	$query = "SELECT procedure_code AS code, procedure_type, related_code AS components, specimen FROM procedure_type ";
	$query .= "WHERE activity = 1 AND procedure_code = ? ";
	$query .= "AND (procedure_type = 'ord' OR procedure_type = 'pro') ";
	$record = sqlQuery($query,array($code));

	$specimen = null;
	if ($record['specimen']) $specimen = $record['specimen'];
	...
	$aoe = array();
	$result = sqlStatement("SELECT question_code, question_text, maxsize, fldtype, options, tips, required FROM procedure_questions ".
		"WHERE procedure_code = ? AND lab_id = ? AND activity = 1 ORDER BY seq",
			array($code,$lab_id));
	
	while ($record = sqlFetchArray($result)) {
		$aoe[] = array('code'=>$record['question_code'],'question'=>$record['question_text'],'max'=>$record['maxsize'],'field'=>$record['fldtype'],'options'=>$record['options'],'prompt'=>$record['tips'],'required'=>$record['required']);
	}
	
	$data = array('profile'=>$profile,'aoe'=>$aoe,'type'=>$type,'specimen'=>$specimen);
	echo json_encode($data);
}
