    ...
    tpl: [
        '<div class="details">',
            '<tpl for=".">',
                    ...
                    '<div style="width:74px;height:74px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'icons/{thumb}\' id=\'{id}\')"></div>'),
                '<div class="details-info">',
                    '<b>Example Name:</b>',
                    '<span>{title}</span>',
                    '<b>Example URL:</b>',
                    '<span><a href="{alternateLink}" target="_blank">{title}.html</a></span>',
                    '<b>Type:</b>',
                    '<span>{mimeType}</span>',
                '</div>',
            '</tpl>',
    ...
    afterRender: function(){
        ...
        if (!Ext.isWebKit) {
            this.el.on('click', function(){
                Ext.Msg.alert('The Sencha Touch examples are intended to work on WebKit browsers. They may not display correctly in other browsers.');
            }, this, {delegate: 'a'});
        }    
    },
