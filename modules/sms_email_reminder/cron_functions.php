...
function cron_SendMail( $to, $subject, $vBody, $from )
{
    ...
    echo "<br>1=".$GLOBALS['smtp_host_name'];           
      echo "<br>2=".$GLOBALS['smtp_host_port'];
      echo "<br>3=".$GLOBALS['smtp_localhost'];
      echo "<br>4=".$GLOBALS['smtp_auth_user'];
      echo "<br>5=".$GLOBALS['smtp_auth_pass'];
      
    
  /* $GLOBALS['smtp_host_name']='smtpout.secureserver.net';
$GLOBALS['smtp_localhost']='localhost';
$GLOBALS['smtp_auth_user']='zcon';*/
	...
	{//echo "EELLSSEE";die;
		...
		//echo "<br>strFrom=".$strFrom;echo "<br>strTo=".$strTo;echo "<br>";
		if( $smtp->SendMessage(
			$strFrom,
			array( $strTo ),
			array(
...
function WriteLog( $data )
{
	...
	$log_folder_path = $GLOBALS['fileroot'].'/modules/sms_email_reminder';
...
// Purpose:	send sms
////////////////////////////////////////////////////////////////////
function cron_SendSMS( $to, $subject, $vBody, $from,$twilio_account_sid,
				$twilio_auth_token,$twilio_from_name )
{
	...
	// $mysms->token_pay("1234567890123456"); //spend voucher with SMS credits
        echo "<br>to==$to<br>";
            ...
            echo "<br>vBody==$vBody<br>";
            echo "<br>subject== ".$subject."<br>";                             
        $getSMSgateway = sqlStatement("SELECT sms_gateway_type FROM automatic_notification WHERE type='SMS'");
        $rowGetSMSgateway=sqlFetchArray($getSMSgateway);
        if($rowGetSMSgateway['sms_gateway_type']=='Twilio')
        {            
                require_once "/mnt/stor10-wc2-dfw1/551939/551948/allcare.dfwprimary.com/web/content/modules/sms_email_reminder/twilio/Services/Twilio.php";
		...
		$AccountSid = $twilio_account_sid;
                ...
                $twilio_from_name, // From this number   "YYY-YYY-YYYY"
                $to, // To this number     "XXX-XXX-XXXX"   
                $vBody
                );
...
function cron_getAlertpatientData( $type )
{
	...
	if($type=='SMS')
	{
		...
		$check_date = date("Y-m-d", mktime(date("h")+$SMS_NOTIFICATION_HOUR, 0, 0, date("m"), date("d"), date("Y")));
                $current_date = date("Y-m-d", mktime(date("h"), 0, 0, date("m"), date("d"), date("Y")));
	}else
	{
		// larry :: remove ope.pc_sendalertemail='No' - nothing like it in the calendar 
	...
	$ssql .= " and (ope.pc_eventDate>='$current_date' and ope.pc_eventDate<='$check_date')";
        
        $appStatus = " and ope.pc_apptstatus IN ('&#','#&','e*','e**','e***','s*','s**','s***')";
	...
	$query = "select $patient_field pd.pid,ope.pc_eid,ope.pc_pid,ope.pc_title,
		...
		where 
			ope.pc_pid=pd.pid $ssql $appStatus
		order by 
...
function cron_setmessage($prow,$db_email_msg)
{
	...
	//$replare_array = array($NAME,$PROVIDER,$DATE,$STARTTIME,$ENDTIME);
        $find_array = array("***NAME***","***DATE***","***STARTTIME***","***ENDTIME***");
	$replare_array = array($NAME,$DATE,$STARTTIME,$ENDTIME);
