    ...
    try
    {
        ...
        $get_layout2 = "SELECT group_name,list_id, title,field_id,form_id,
                ...
                END as field_type,                         CASE uor
                                ...
                                END as isRequired,description,max_length FROM layout_options WHERE form_id = 'DEM' AND uor <> 0 AND group_name LIKE '%$group_name' order by group_name, seq";
            $db->query( "SET NAMES utf8");
    ...
    try 
    {
        ...
        END as field_type,
                        CASE uor
                            WHEN 1 THEN 'Optional'
                            WHEN 2 THEN 'Required'
                            END as isRequired,description,max_length FROM layout_options WHERE form_id = 'AGENCY' and uor<> 0 order by group_name, seq";   
        $db->query( "SET NAMES utf8");
        $stmt = $db->prepare($sql) ;
        $stmt->execute();                       
