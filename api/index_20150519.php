...
function getPatientsByFacility($fid)
{
    ...
    if($fid == 0){
        $sql="select pd.pid, pd.title,pd.fname,pd.lname, DATE_FORMAT(pd.DOB,'%m-%d-%Y') AS DOB,
            ...
            pd.street,pd.city,pd.state,pd.country_code,pd.postal_code,pd.phone_cell from patient_data pd where pid not in (SELECT DISTINCT(patientid) FROM tbl_patientfacility)  AND pd.practice_status = 'YES' AND pd.deceased_stat != 'YES' order by pd.lname, pd.fname";  
        try 
        {
            $db = getConnection();
            $stmt = $db->prepare($sql) ;
...
function saveVitalsForm(){
    try 
    {
        ...
        GibberishAES::size(256);
        //$appres = GibberishAES::dec($request->getBody(), $key);
        $appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
...
function getapptstatus(){
    ...
    $old_key_size = GibberishAES::size();
    GibberishAES::size(256);
    $qry = "select option_id as id ,title from list_options where list_id='apptstat' ";
    try 
    {
        $db = getConnection();
        $stmt = $db->prepare($qry) ;
