...
$app->get('/getlayoutdemographics/:field/:pid', 'getLayoutDemographicsDynamic');
...
function getPatientsByday($loginUserId,$day)
{
    ...
    $sql = "SELECT ope.pc_eid as apptid,pd.pid,pd.title,pd.fname,pd.lname,if (pd.sex = 'Female' ,'F','M' ) as sex,
                ...
                WHERE ope.pc_aid=:loginUserId AND ope.pc_eventdate=:day and pd.practice_status = 'YES' AND pd.deceased_stat != 'YES' order by pd.lname, pd.fname";  
...
//                ";     
        ...
        try 
        {
            ...
            $patients = $stmt->fetchAll(PDO::FETCH_OBJ);     
            $sql2 = "SELECT li.title,li.option_id FROM layout_options lo INNER JOIN list_options li ON  li.list_id = lo.list_id WHERE lo.field_id ='practice_status'";
            $stmt2 = $db->prepare($sql2) ;
            $stmt2->execute();                       
            $practice = $stmt2->fetchAll(PDO::FETCH_OBJ); 
            
            $sql3 = "SELECT li.title,li.option_id FROM layout_options lo INNER JOIN  list_options li ON  li.list_id = lo.list_id WHERE lo.field_id ='deceased_stat'";
            $stmt3 = $db->prepare($sql3) ;
            $stmt3->execute();  
            $deceased = $stmt3->fetchAll(PDO::FETCH_OBJ); 
            
            $sql4 = "SELECT li.title,li.option_id FROM layout_options lo INNER JOIN  list_options li ON  li.list_id = lo.list_id WHERE lo.field_id ='cpo'";
            $stmt4 = $db->prepare($sql4) ;
            $stmt4->execute();  
            $cpo = $stmt4->fetchAll(PDO::FETCH_OBJ); 
            
            $sql5 = "SELECT li.title,li.option_id FROM layout_options lo INNER JOIN  list_options li ON  li.list_id = lo.list_id WHERE lo.field_id ='ccm'";
            $stmt5 = $db->prepare($sql5) ;
            $stmt5->execute();  
            $ccm = $stmt5->fetchAll(PDO::FETCH_OBJ); 
            
            $patientsdata['PatientData'] = $patients;
            $patientsdata['FilterData']['deceased_stat'] = $deceased;
...
function getAllPatients1($fromCount)
{
        $toCount=$fromCount+20;
	$key = 'rotcoderaclla';
        // The default key size is 256 bits.
...
//                ";     
        ...
        try 
        {
            ...
            $patientsdata['FilterData']['deceased_stat'] = $deceased;
            $patientsdata['FilterData']['practice_status'] = $practice;
            $patientsdata['FilterData']['cpo'] = $deceased;
            $patientsdata['FilterData']['cpm'] = $practice;
            //echo "<pre>"; print_r($patientsdata); echo "</pre>";
            if($patientsdata)
            {
                //returns patients appointment list
...
//                limit 20";     
        ...
        $sql = "SELECT pid,title,fname,lname, DATE_FORMAT(DOB,'%m-%d-%Y') AS DOB,DATE_FORMAT(FROM_DAYS(DATEDIFF(CURDATE(),DOB)), '%Y')+0 AS age,if (sex = 'Female' ,'F','M' ) as sex,street,city,state,country_code,postal_code,phone_home,phone_biz,phone_contact,phone_cell,contact_relationship as emergency_contact, phone_contact as emergency_phone, practice_status, deceased_stat, DATE_FORMAT( last_visit, '%Y-%m-%d' ) as last_visit,cpo,ccm
                from patient_data where  providerID=:loginProvderId order by lname, fname  limit  0, $toCount "; 
        try 
        {
            $db = getConnection();
            $stmt = $db->prepare($sql) ;
...
function createMessage()
{
    try
    {
        ...
        //$msgresult =  GibberishAES::dec('U2FsdGVkX1/+XAMDRmW/RAy14ML32t7/EUzMPDch4vj1xQN4h6yQ1gpabxIQYeA3IOtE3Chv9So6igcRickmbslkpifVxTC/VKlH4OKJfoWOKQUDYkNSneWu0sz6VCVT/Tbie65083Plsj2889SPMJTL5MGFjDUD7DM+Lo0gWXq248Mrs1CLbqTCtXE1dT5X', $key);
...
function getPatientsByFacility($fid)
{
    ...
    if($fid == 0){
        $sql="select pd.pid, pd.title,pd.fname,pd.lname, DATE_FORMAT(pd.DOB,'%m-%d-%Y') AS DOB,
            ...
            pd.street,pd.city,pd.state,pd.country_code,pd.postal_code,pd.phone_cell,pd.deceased_stat,pd.practice_status from patient_data pd where pid not in (SELECT DISTINCT(patientid) FROM tbl_patientfacility)  AND pd.practice_status = 'YES' AND pd.deceased_stat != 'YES' order by pd.lname, pd.fname";  
        try 
        {
            $db = getConnection();
            $stmt = $db->prepare($sql) ;
...
function getAppointmentStatuses()
{
    ...
    $qry = "select option_id as id,title from list_options where list_id='apptstat' and option_id IN('&#','@','#&','x','?')";
    $stmt = $db->prepare($qry) ;
    $stmt->execute();
...
function getAppointmentDates($loginProvderId)
{
	...
	{	
        ...
        $sql="select DISTINCT DATE_FORMAT(ope.pc_eventdate,'%m-%d-%Y') as datevalue from openemr_postcalendar_events ope where ope.pc_aid=$loginProvderId  ORDER BY ope.pc_eventdate ";
        $stmt = $db->prepare($sql) ; 
        $stmt->execute();
        $apptdates = $stmt->fetchAll(PDO::FETCH_OBJ);           
...
function getHistoryDetails3($patientId, $group_name2)
{
    try
    {	$newdemo = array();
        $dataarray = array();
...
//To get the history details
function getHistoryDetails2($patientId, $group_name2)
{
    try
    {	$newdemo = array();
        $dataarray = array();
    ...
    try 
        {
            ...
            $sql1 = "SELECT id,DATE_FORMAT( date, '%Y-%m-%d' ) as date,REPLACE(type,'_',' ') as type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'medical_problem' ORDER BY begdate";

            //To get Allergies
            $sql2 = "SELECT id,DATE_FORMAT( date, '%Y-%m-%d' ) as date,REPLACE(type,'_',' ') as type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'allergy' ORDER BY begdate";

            //To get Medications
            $sql3 = "SELECT id,DATE_FORMAT( date, '%Y-%m-%d' ) as date,REPLACE(type,'_',' ') as type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'medication' ORDER BY begdate";

            //To get Surgeries
            $sql4 = "SELECT id,DATE_FORMAT( date, '%Y-%m-%d' ) as date,REPLACE(type,'_',' ') as type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'surgery' ORDER BY begdate";

            //To get Dental Issues 
            $sql5 = "SELECT id,DATE_FORMAT( date, '%Y-%m-%d' ) as date,REPLACE(type,'_',' ') as type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'dental' ORDER BY begdate";
            ...
            //To get DME Issues 
            $sql8 = "SELECT id,date,type,title,begdate,enddate,returndate,occurrence,classification,referredby,extrainfo,diagnosis,activity,comments,pid,user,groupname,outcome,destination,reinjury_id,injury_part,injury_type,injury_grade,reaction,external_allergyid,erx_source,erx_uploaded,modifydate FROM lists WHERE pid = :patientId AND type = 'DME' ORDER BY begdate";
...
function getMessages($loginProvderId)
{
	try 
		...
		if($loginProvderId==0)
        {
			...
			where pn.deleted=0 order by pn.id desc";
			$db->query( "SET NAMES utf8"); 
			$stmt = $db->prepare($sql) ;
			$stmt->execute();
		...
		else
		{
			...
			if($rs)
			{
				...
				from pnotes pn inner join users us on pn.user=us.username
				inner join patient_data pd on pd.pid=pn.pid
				where pn.assigned_to='$username' and pn.deleted=0 order by pn.id desc";
                                $db->query( "SET NAMES utf8"); 
				$stmt = $db->prepare($sql) ;
				$stmt->execute();
				$messages = $stmt->fetchAll(PDO::FETCH_OBJ);           
...
//                  
        ...
        //json_encode($feres);
        $bilres = '[{"id":"1","Total_Charges":"'.$charges.'","balance_amount":"'.$balance.'"}]';    
...
function editPatientFacility()
{
	...
	//print_r($pfArray);
        ...
        if($id==0)
        {
                $insert_pf_Sql="INSERT INTO tbl_patientfacility(patientid,facilityid,facility_admitdate,facility_dischargedate,facility_isactive,facility_notes,facility_related_links,createdby,created_date)
...
//            AND prescriptions.active ='1'";
        $sql="CALL API_getPrescriptions_sql($patientId)";
        ...
        if($presclist)
        {
           echo $presres = json_encode($presclist); 
            //echo $presresult = GibberishAES::enc($presres, $key);
            
        }
        else
        {
           echo $presres = '[{"DataAvailable":"No"}]';
           // echo $presresult = GibberishAES::enc($presres, $key);
        }
    ...
    {
       echo $error = '{"error":{"text":'. $e->getMessage() .'}}'; 
         $presresult = GibberishAES::enc($error, $key);
    }    
}
//method to store mobile app settings
    ...
    try{
        ...
        if(!empty($patients2)){
            ...
            if(!empty($set_form_id_sql)){
                ...
                $form_ins = "INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),$copy_to_encounter,'Allcare Encounter Forms',$new_formid,$pid,'$user','default',1,0,'LBF2')";
        ...
        if(!empty($datacheck5)){
            ...
            if(!empty($copy_from_ros)){
                ...
                if(!empty($ros_copy_to)){
                    ...
                    $form_ins = "INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),$copy_to_encounter,'Allcare Review Of Systems',$copy_to_id,'".$datacheck5[0]->pid."','$user','default',1,0,'allcare_ros')";
                    $form_insstmt = $db->prepare($form_ins);
                    $form_insstmt->execute(); 
                }
        ...
        if(!empty($datacheck4)):
            ...
            if(!empty($copy_to_fid)):
                ...
                $form_ins = "INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),$copy_to_encounter,'Allcare Physical Exam',$new_id1,'".$datacheck4[0]->pid."','$user','default',1,0,'allcare_physical_exam')";
                $form_insstmt = $db->prepare($form_ins);
                $form_insstmt->execute();  
                $copy_to_id=$new_id1;
            endif;
            ...
            if(!empty($copy_to_details)):
               ...
               //this is for finalized ,pending  and log
                ...
                $array2[] = array( 'authuser' =>$user,'pending' => $pending,'finalized' => $finalized, 'date' => date("Y/m/d"), 'action'=>'Copied' ,'count'=> $count+1);
    ...
    try{
        ...
        if(!empty($agencydata)){
            ...
            foreach($agencydata as $agency_key){
                ...
                $get_user_abook = "SELECT (select title FROM list_options WHERE option_id=abook_type and list_id='abook_type') AS AddressBook, phonew1 AS workphone, phonecell, fax, email, street, city, state, zip FROM users WHERE id= ".$agencyid;
...
function getPatientInsuranceDataList($pid){
    ...
    $sql = " SELECT i.id, i.type as Insurance_Type,(SELECT CONCAT(title, fname,  ' ', lname ) AS name FROM users WHERE id= i.provider) as Provider,ic.name as Insurance_Company_Name,i.group_number as Group_Number,i.plan_name as Plan_Name,i.policy_number as Policy_Number,CONCAT(i.subscriber_lname,' ',i.subscriber_mname,' ', i.subscriber_fname) as Subscriber_Name,i.subscriber_relationship as Subscriber_Relationship,
            ...
            i.copay as Copay,i.date as Date, i.subscriber_sex as Subscriber_Sex,i.accept_assignment as Accept_Assignment,i.policy_type as Policy_Type
	...
	try 
        {
            ...
            $sql = "SELECT count(f.id) as count FROM `form_encounter` f INNER JOIN patient_data p ON p.pid = f.pid WHERE (`elec_signed_on` = '' AND `elec_signedby` = '')AND p.deceased_stat !=  'YES'
AND p.practice_status =  'YES' AND p.providerID=$providerid ";
...
//        $appres = GibberishAES::dec('', $apikey);
  ...
  try 
    {
        ...
        $old_key_size = GibberishAES::size();
        GibberishAES::size(256);
        $sql = "SELECT f.pc_catid, p.providerID as provider_id, f.facility_id, p.pid
                FROM  `form_encounter` f 
                INNER JOIN patient_data p ON p.pid = f.pid
                WHERE encounter =$eid";
        ...
        foreach ($namesresult as $value) {
            ...
            $sql2 ="SELECT DISTINCT  l.title AS screen_name,f.screen_group, l.description AS screen_link
                ...
                WHERE  `facilities` REGEXP ('$value->facility_id') AND  `users` REGEXP ('$value->provider_id') AND  `visit_categories` REGEXP ('$value->pc_catid') order by l.title ";      
                            ...
                            if($datacheck2[0]->field_id  == 'codes'){
                                ...
                                if(!empty($set_billing)){
                                    ...
                                    if($billing == 0){
                                     ...
                                     $field_value = 'Not Bbilled';
                                }
                            }
                            $formvalues[] = (object)array('form_type' => $datacheck2[0]->GroupName,  'form_id'=> $form_id, 'form_name' =>$datacheck2[0]->field_id, 'FormOrder' =>$datacheck2[0]->FormOrder, 'grouporder' => $datacheck2[0]->grouporder, 'GroupName' => $value3->screen_name, 'isRequired' => $datacheck2[0]->isRequired , 'description' => $datacheck2[0]->description, 'field_value' => $field_value);
                ...
                foreach ($formnames as $value4) {
                    ...
                    $datacheck = $stmt4->fetchAll(PDO::FETCH_OBJ);  
                        if(empty($datacheck) || ($value4->field_id == 'ros') || ($value4->field_id == 'physical_exam')){ 
                            ...
                            if($datacheck2[0]->field_id  == 'allergies' || $datacheck2[0]->field_id  == 'dental_problems' || $datacheck2[0]->field_id  == 'codes' || $datacheck2[0]->field_id  == 'immunization' || $datacheck2[0]->field_id  == 'medical_problem'|| $datacheck2[0]->field_id  == 'medication'):
                                $form_id = 0;
                                $field_value = '';
                            endif;
                            $formvalues[] = (object)array('form_type' => $datacheck2[0]->GroupName,  'form_id'=> $form_id, 'form_name' =>$datacheck2[0]->field_id, 'FormOrder' =>$datacheck2[0]->FormOrder, 'grouporder' => $datacheck2[0]->grouporder, 'GroupName' => $value3->screen_name, 'isRequired' => $datacheck2[0]->isRequired , 'description' => $datacheck2[0]->description, 'field_value' => $field_value);
                            ...
                            if($datacheck[0]->form_name  == 'allergies' || $datacheck[0]->form_name  == 'dental_problems' || $datacheck[0]->form_name  == 'codes' || $datacheck[0]->form_name  == 'immunization' || $datacheck[0]->form_name  == 'medical_problem'|| $datacheck[0]->form_name  == 'medication'):
                               $datacheck[0]->form_id = 0;
                            endif;
                           $formvalues[] = $datacheck[0];//$stmt4->fetchAll(PDO::FETCH_OBJ); 
                        }
...
function saveVitalsForm(){
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $key);
//        $appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
//        print_r($insertArray);
        ...
        $getformid = "SELECT form_id FROM forms WHERE formdir= 'LBF2' and deleted = 0 and encounter = $encounter and pid = $pid order by date asc";
        $stmt3 = $db->prepare($getformid) ;
        $stmt3->execute();
        $formidval = $stmt3->fetchAll(PDO::FETCH_OBJ);
        ...
        if(!empty($formidval)){
             $newformid =  $formidval[0]->form_id;
            if (count($db->query("SELECT * FROM lbf_data WHERE form_id = $newformid AND field_id = 'vitals_stat'")->fetchAll())==0) {
                $sql = "INSERT into lbf_data (form_id, field_id, field_value) VALUES ($newformid,'vitals_stat','$status')";
            } else {
               $sql = "UPDATE lbf_data SET field_value = '$status' WHERE field_id ='vitals_stat'  AND form_id = $newformid";
            }
            $stmt4 = $db->prepare($sql);
            $stmt4->execute();
        }    
...
function saveVitalsForm2(){
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $key);
//        $appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
        ...
        if($formid == 0){
            ...
            echo $sql = "INSERT INTO `form_vitals` ( `id`, `date`, `pid`, `user`,  `authorized`, `activity`, `bps`, `bpd`, `weight`, `height`, `temperature`, `temp_method`, `pulse`, `respiration`, `note`, `BMI`, `BMI_status`, `waist_circ`, `head_circ`,`oxygen_saturation`) 
                    VALUES ( $newformid, NOW(), $pid, $user,  $authorized, $activity, $bp_systolic, $bp_diastolic, $weight_lbs,  $height_inches, $temperature_fahrenheit,'". $temperature_location."', $pulse, $respiration,'". $note."', $BMI, '".$BMI_status ."',$waist_circumference_inches,$head_circumference_inches,  $oxygen_saturation)";
            $sqlstmt = $db->prepare($sql) ;
            $data =  $sqlstmt->execute();
...
function saveROSFormData(){
    try{
        ...
        if($formid == 0):
            ...
            $sql5 = "INSERT into `lbf_data`(`form_id`, `field_id`,`field_value`) VALUES(".$formid.",'ros_stat','".$formstatus."')";
...
function savePhysicalExamFormData(){
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $key);
        //$appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
        $nameslist = array('GENWELL', 'GENAWAKE','HEADNORM', 'HEADLESI', 'EYECP','EYECON','EYEPER','ENTTM','ENTNASAL','ENTORAL','ENTSEPT','NECKSUP','BACKCUR','CVRRR','CVNTOH','CVCP','CVNPE','CVNMU','CHNSD','RECTAB','REEFF','RELUN','GIOG','GIHERN','GISOFT','GIBOW','GUTEST','GUPROS','GUEG','GUAD','GULES','EXTREMIT','EXTREDEF','LYAD','MUSTR','MUROM','MUSTAB','MUINSP','NEUCN2','NEUREF','NEUSENS','NEULOCAL','NEUGROSS','PSYAFF','PSYJUD','PSYDEP','PSYSLE','PSYTHO','PSYAPP','PSYABL','SKRASH','SKCLEAN','OTHER');
        $formid                     = $insertArray['formid'];
        $encounter                  = $insertArray['encounter'];
        $pid                        = $insertArray['pid'];
        $user                       = $insertArray['user'];
...
//                $stmt->bindParam("comments", $comments); 
            ...
            $sql5 = "INSERT into `lbf_data`(`form_id`, `field_id`,`field_value`) VALUES(".$formid.",'physical_exam_stat','".$formstatus."')";
            $sqlstmt5 = $db->prepare($sql5) ;
            $data5 = $sqlstmt5->execute();
        }
...
function getLayoutDemographicsDynamic($field_id,$patientId){
    try
    {	
        $newdemo = array();
        $dataarray = array();
        ...
        //$old_key_size = GibberishAES::size();
       // GibberishAES::size(256);
        $getgroupnames = "CALL API_getLayoutDemographicsDynamic_getDemographicsData($field_id,$patientId)";
        $stmt = $db->prepare($getgroupnames) ;
        $stmt->execute();   
...
//        }
        //echo "<pre>"; print_r($data);echo "</pre>";
        $newdemo1 = encode_demo($setgroupnames2);  
...
function getFeesheetCodes4($encounterid)
{
    try{
        $db = getConnection();
        $apikey = 'rotcoderaclla';
        ...
        if(!empty($pid_res)){
            ...
            foreach($icd9codes as $element):
                 ...
                 $sql = "SELECT DISTINCT l.id, l.title AS Title, l.diagnosis AS Codes, i.long_desc AS Description
                        FROM lists AS l
                        LEFT JOIN issue_encounter AS ie ON ie.list_id = l.id
                        AND ie.encounter =$encounterid
                        INNER JOIN  `icd9_dx_code` i ON l.diagnosis = CONCAT(  'ICD9:', i.formatted_dx_code ) 
                        WHERE l.type =  'medical_problem'
                        AND l.pid =$pid1
                        AND (
                        (
            ...
            $selectquery = "SELECT b.id, b.code,b.billed, (select group_concat(justify) from billing WHERE encounter =   $encounterid and code_type='CPT4' and activity = 1) as justify, b.notecodes, b.code_text
                FROM billing b 
                INNER JOIN form_encounter f ON b.encounter = f.encounter  
                WHERE b.encounter =   $encounterid and code_type='CPT4' and b.activity = 1 order by b.date desc ";
            $selectstmt = $db->prepare($selectquery) ; 
            ...
            $billed = 0;
            if(!empty($selected_feesheet)){
                $icd_code = explode(":",str_replace(",","",$selected_feesheet[0]->justify));
                foreach($medical_problems as $element):
                    $billed = $selected_feesheet[0]->billed;
                    if($element->Codes == str_replace('|', ':', $icd_code[0])){
                        $element->primary = 1;
                    }else{
                        $element->primary = 0;
                ...
                INNER JOIN openemr_postcalendar_events p ON  f.pc_catid =p.pc_catid AND p.pc_eventDate = DATE_FORMAT( f.date,  '%Y-%m-%d' ) and p.pc_pid = f.pid 
                inner join users u on u.id = p.pc_aid 
                ...
                $set_providerName  = $selectsupervisor->fetchAll(PDO::FETCH_OBJ);
                if(!empty($set_providerName)){
                    $selected_feesheet = $set_providerName[0]->rendering_providerid;
                    $selected_feesheet = $set_providerName[0]->rendering_ProviderName;
                }
            }
            $provider = "SELECT id, CONCAT(fname,',',lname) as name FROM users WHERE " .
                    "( authorized = 1 OR info LIKE '%provider%' ) AND username != '' " .
...
function getFeesheetCodes3($encounterid)
{
    try{
        $db = getConnection();
        $apikey = 'rotcoderaclla';
...
function getFeesheetCodes2($encounterid)
{
    try{
        ...
        if(!empty($pid_res)){
            ...
            $selectquery = "SELECT b.id, b.code,billed, (select group_concat(justify) from billing WHERE encounter =   $encounterid and code_type='CPT4' and activity = 1) as justify, b.notecodes, b.code_text
                FROM billing b 
                INNER JOIN form_encounter f ON b.encounter = f.encounter  
                WHERE b.encounter =   $encounterid and code_type='CPT4' and b.activity = 1 order by b.date desc ";
            $selectstmt = $db->prepare($selectquery) ; 
    ...
    try{
        ...
        if($type=='allergy')
        {
            $query = "SELECT 
                    ...
                    THEN  'Active'
                    ELSE  'Inactive'
                    END AS isIssueActive, li.id,li.title AS Title, li.begdate AS Begdate, li.enddate AS Enddate, li.diagnosis AS Codes, 
        ...
        else
        {
            $query = "SELECT 
                    ...
                    ELSE  'Inactive'
                    END AS isIssueActive, li.id,li.title AS Title, li.begdate AS Begdate, li.enddate AS Enddate, li.diagnosis AS Codes,
                    li.occurrence AS Occurrence, li.referredby AS ReferredBy,ie.encounter, li.comments,
                    (SELECT COUNT( list_id ) 
                    FROM issue_encounter
                    WHERE list_id = li.id
                    ) AS enc,(select long_desc   FROM `icd9_dx_code` where CONCAT(  'ICD9:', formatted_dx_code ) = li.diagnosis and active = 1 ) AS Description
...
function saveFeeSheet5(){
    ...
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $apikey);
//        $appres = GibberishAES::dec('', $apikey);
        $insertArray = json_decode($appres,TRUE);
...
//        echo "<pre>"; print_r($insertArray); echo "</pre>"; 
        ...
        for($i=0; $i<count($codearray); $i++){
            ...
            //$justify=str_replace(",",":",str_replace(":","|",str_replace("|",",",$insertArray['justify']))).":";
            $justify=$insertArray['justify'].":";
            $codearray[$i]['id'] = 0;
            if($codearray[$i]['id']==0){
                if($codearray[$i]['code_type']=='CPT4' && $justify==''){     
                ...
                }else if($codearray[$i]['code_type']=='CPT4' && $justify!=''){
                    if($codearray[$i]['code']!=''){
                        $descr1="select code_text from codes where code='".$codearray[$i]['code']."' AND code_type=1";
                        $sqlstmte1 = $db->prepare($descr1) ;
                        $datas=  $sqlstmte1->execute();
                    ...
                    $sql2 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,modifier,units,fee,justify,target)"
                                . " VALUES ( NOW(),'".$codearray[$i]['code_type']."','".$codearray[$i]['code']."','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$code_text', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date','$process_date','$process_file','$modifier','".$codearray[$i]['units']."','".$codearray[$i]['fee']."','$justify','$target')
                                 "; 
                    $sqlstmt2 = $db->prepare($sql2);
                    $data =  $sqlstmt2->execute(); 

                    $sql3 = "UPDATE form_encounter SET provider_id ='$user'  WHERE pid ='$pid'  AND encounter ='$encounter'";
                    $sqlstmt3 = $db->prepare($sql3) ;
                    $data3 =  $sqlstmt3->execute();  
                }   
                
            if($codearray[$i]['code_type']=='ICD9'){
                if($codearray[$i]['code']!=''){
                    $descr3="select long_desc from icd9_dx_code where formatted_dx_code='".$codearray[$i]['code']."' and active= 1";
                    $sqlstmte3 = $db->prepare($descr3) ;
                    $datas3=  $sqlstmte3->execute();
                    $desval3 = $sqlstmte3->fetchAll(PDO::FETCH_OBJ);
                    if(!empty($desval3)){
                        $code_text =   $desval3[0]->long_desc;
                        $code_text1 = str_replace("'", "\\'", $code_text);
                    }else {
                        $code_text1='';
                    }
                }
                $sql1 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
                            . " VALUES ( 'NOW()','".$codearray[$i]['code_type']."','".$codearray[$i]['code']."','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$code_text1', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date', '$process_date','$process_file','$target')";
                $sqlstmt1 = $db->prepare($sql1) ;
                $data =  $sqlstmt1->execute();  

                $sql5 = "UPDATE form_encounter SET provider_id ='$user' WHERE pid ='$pid' AND encounter ='$encounter'";
                $sqlstmt5 = $db->prepare($sql5) ;
                $data =  $sqlstmt5->execute();        
            }
           ...
           if($codearray[$i]['code_type'] == 'CPT4' && $justify == '' ){
                ...
                $sql = "UPDATE billing SET  code='".$codearray[$i]['code']."', provider_id='$provider_id',  authorized='$authorized',billed='$billed', activity='$activity',
                   payer_id='$payer_id',bill_process='$bill_process',bill_date='$bill_date',process_date='$process_date' ,process_file='$process_file',modifier='$modifier',units='".$codearray[$i]['units']."',fee='".$codearray[$i]['fee']."',justify='$justify',target='$target' WHERE  id = $id";                   
                $sqlstmt = $db->prepare($sql) ;
                $data =  $sqlstmt->execute();
                ...
                $sql2 = "UPDATE billing SET code='".$codearray[$i]['code']."',provider_id='$provider_id', authorized='$authorized',billed='$billed', activity='$activity',
                   payer_id='$payer_id',bill_process='$bill_process',bill_date='$bill_date',process_date='$process_date' ,process_file='$process_file',modifier='$modifier',units='".$codearray[$i]['units']."',fee='".$codearray[$i]['fee']."',justify='$justify',target='$target' WHERE  id = $id";                   
                $sqlstmt2 = $db->prepare($sql2) ;
                $data =  $sqlstmt2->execute();
       ...
       for($i=0; $i<count($icds); $i++){
            ...
            $check_query = "SELECT * FROM billing WHERE code = '$title2'  AND activity = 1 and encounter = 4014 AND code_type = 'ICD9' AND pid = '$pid' AND code_text = '$title1'"; 
            $cquery = $db->prepare($check_query) ;
            $db->query( "SET NAMES utf8"); 
            $datas2=  $cquery->execute();
            $cqueryval = $cquery->fetchAll(PDO::FETCH_OBJ);
       ...
       for($i=0; $i<count($justify_array); $i++){
            ...
            $icd_code3 = str_replace("ICD9:", "",$icd_code2 );
            $check_query2 = "SELECT * FROM billing WHERE code = '$icd_code3' AND activity = 1 AND pid = '$pid' and encounter = 4014 AND code_text = '$title3'";
            $db->query( "SET NAMES utf8"); 
            $cquery2 = $db->prepare($check_query2) ;
            $datas2=  $cquery2->execute();
            $cqueryval2 = $cquery2->fetchAll(PDO::FETCH_OBJ);
...
function saveFeeSheet4(){
    ...
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $apikey);
//        $appres = GibberishAES::dec('', $apikey);
        $insertArray = json_decode($appres,TRUE);
...
//        echo "<pre>"; print_r($insertArray); echo "</pre>"; 
        ...
        for($i=0; $i<count($codearray); $i++){
            ...
            if($codearray[$i]['code_type']=='ICD9'){
                if($codearray[$i]['code']!=''){
                    $descr3="select long_desc from icd9_dx_code where formatted_dx_code='".$codearray[$i]['code']."'";
                    $sqlstmte3 = $db->prepare($descr3) ;
                    $datas3=  $sqlstmte3->execute();
                    $desval3 = $sqlstmte3->fetchAll(PDO::FETCH_OBJ);
                    if(!empty($desval3)){
       ...
       for($i=0; $i<count($icds); $i++){
            $icd_code = $icds[$i]['code'];
            $descr="select long_desc, formatted_dx_code from icd9_dx_code where dx_code='$icd_code'";
            $sqlstmte = $db->prepare($descr) ;
            $datas=  $sqlstmte->execute();
            $desval1 = $sqlstmte->fetchAll(PDO::FETCH_OBJ);
            if(!empty($desval1)){
            ...
            $check_query = "SELECT * FROM billing WHERE code = '$title2'  AND activity = 1 AND code_type = 'ICD9' AND pid = '$pid' AND code_text = '$title1'"; 
            $cquery = $db->prepare($check_query) ;
            $db->query( "SET NAMES utf8"); 
            $datas2=  $cquery->execute();
            $cqueryval = $cquery->fetchAll(PDO::FETCH_OBJ);
       ...
       for($i=0; $i<count($justify_array); $i++){
            ...
            $title1='';
            $exploded_code = str_replace("ICD9:", "", $icd_code2);
            $descr2="SELECT long_desc FROM icd9_dx_code WHERE formatted_dx_code ='$exploded_code'";
            $sqlstmte2 = $db->prepare($descr2) ;
            $datas2=  $sqlstmte2->execute();
            $desval112 = $sqlstmte2->fetchAll(PDO::FETCH_OBJ);
            if(!empty($desval112)){
            ...
            $icd_code3 = str_replace("ICD9:", "",$icd_code2 );
            $check_query2 = "SELECT * FROM billing WHERE code = '$icd_code3' AND activity = 1 AND pid = '$pid'  AND code_text = '$title3'";
            $db->query( "SET NAMES utf8"); 
            $cquery2 = $db->prepare($check_query2) ;
            $datas2=  $cquery2->execute();
            $cqueryval2 = $cquery2->fetchAll(PDO::FETCH_OBJ);
...
function saveFeeSheet3(){
    ...
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $apikey);
//        $appres = GibberishAES::dec('', $apikey);
        $insertArray = json_decode($appres,TRUE);
...
//        echo "<pre>"; print_r($insertArray); echo "</pre>"; 
       ...
       for($i=0; $i<count($icds); $i++){
            $icd_code = $icds[$i]['code'];
            $descr="select long_desc from icd9_dx_code where dx_code='$icd_code'";
            $sqlstmte = $db->prepare($descr) ;
            $datas=  $sqlstmte->execute();
            $desval1 = $sqlstmte->fetchAll(PDO::FETCH_OBJ);
            if(!empty($desval1)){
            ...
            $check_query = "SELECT * FROM billing WHERE code = '$icd_code' AND activity = 1 AND code_type = 'ICD9' AND pid = '$pid' AND code_text = '$title1'"; 
            $cquery = $db->prepare($check_query) ;
            $db->query( "SET NAMES utf8"); 
            $datas2=  $cquery->execute();
            $cqueryval = $cquery->fetchAll(PDO::FETCH_OBJ);
            if(empty($cqueryval)){
                ...
                $sql1 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
                                    . " VALUES ( NOW(),'ICD9','$icd_code','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$title1', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date', '$process_date','$process_file','$target')";
                $sqlstmt1 = $db->prepare($sql1);
                $data2 =  $sqlstmt1->execute();
           }
       }
       $justify_array = explode("|",$insertArray['justify']);
       
       for($i=0; $i<count($justify_array); $i++){
            $icd_code2 = $justify_array[$i];
            $title = '';
            $title1='';
            $descr2="select title from lists where diagnosis='$icd_code2'";
            $sqlstmte2 = $db->prepare($descr2) ;
            $datas2=  $sqlstmte2->execute();
            $desval112 = $sqlstmte2->fetchAll(PDO::FETCH_OBJ);
            if(!empty($desval112)){
                
                $title =   $desval112[0]->title;
                $title1 = str_replace("'", "\\'", $title);
            } else {
                $title1='';
            }
            $icd_code3 = str_replace("ICD9:", "",$icd_code2 );
            $check_query2 = "SELECT * FROM billing WHERE code = '$icd_code3' AND activity = 1 AND pid = '$pid'  AND code_text = '$title1'";
            $db->query( "SET NAMES utf8"); 
            $cquery2 = $db->prepare($check_query2) ;
            $datas2=  $cquery2->execute();
            $cqueryval2 = $cquery2->fetchAll(PDO::FETCH_OBJ);
            if(empty($cqueryval2)){
                $sql12 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
                                    . " VALUES ( NOW(),'ICD9','$icd_code3','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$title1', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date', '$process_date','$process_file','$target')";
                $sqlstmt12 = $db->prepare($sql12);
                $data3 =  $sqlstmt12->execute();
            }
       }
...
function saveFeeSheet2(){
    ...
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $key);
       // $appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
...
//        $code_type                  = $insertArray['CodeArray']['code_type'];
//        $code                       = $insertArray['CodeArray']['code'];
//        $provider_id                = $insertArray['provider_id'];
       
//        
        
//        $units                      = $insertArray['CodeArray']['units'];
//        $fee                        = $insertArray['CodeArray']['fee'];
//        $diags                      = $insertArray['CodeArray']['justify'];
//        $issues                     = $insertArray['CodeArray']['issues'];
...
//        $units                      = 1;
//        $fee                        = 0.00;
//        $diags                      = array('338.12','331.0');
//        //$diags                      = '';
//        $target                     = '';
//       // $issues=        array('E009.1','338.0')    ;   
        
//          $codearray= array('0' => array( 'id'=> '0','code_type' => 'ICD9','code' => 'E000.1','units'=>'1' ,'fee'=>'0.00','justify'=>array(),'issues'=>array()),
//                      '1' => array( 'id'=>'1286', 'code_type' => 'CPT4','code' => '99324','units'=>'1' ,'fee'=>'0.00','justify'=>array('338.4','331.0'),'issues'=>array('E007.1','E009.1')) );
  //echo "<pre>"; print_r($codearray); echo "</pre>";  //echo $codearray[0]         ;
//for($i=0; $i<count($codearray); $i++){
// $diags=$codearray[$i]['justify'][0];
// echo"<pre>";print_r($diags); echo "</pre>";
// echo $codearray[$i]['justify'][0];
//}
       ...
       for($i=0; $i<count($codearray); $i++){
           $id=$codearray[$i]['id'];     
           $diags=$codearray[$i]['justify'];
                $issues=$codearray[$i]['issues'];
                ...
                if(!empty($diags))   
                {    $justify='';
                    foreach($diags as $key12=>$value12)
                    {
                        foreach($value12 as $vals){ 
                            $justify.=$vals.":";
                        }
                    }
                } else 
       ...
       if($codearray[$i]['id']==0){
           if($codearray[$i]['code_type']=='CPT4' && $justify=='')
               {     
                ...
                $sql = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,modifier,units,fee,justify,target)"
                        . " VALUES ( NOW(),'".$codearray[$i]['code_type']."','".$codearray[$i]['code']."','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$string', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date','$process_date','$process_file','$modifier','".$codearray[$i]['units']."','".$codearray[$i]['fee']."','$justify','$target')"; 
    //             $sql = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
    //                    . " VALUES ( :date,:code_type,:code,:pid,:provider_id, :user,:groupname, :authorized,  :encounter, :code_text, :billed, :activity ,:payer_id,:bill_process,:bill_date, :process_date,:process_file,:target)";
    //                    
                       $sqlstmt = $db->prepare($sql) ;
                       $data =  $sqlstmt->execute();
    //                    $sqlstmt->bindParam("date",  NOW()); 
    //                    $sqlstmt->bindParam("code_type", $code_type);
    //                    $sqlstmt->bindParam("code", $code);
    //                    $sqlstmt->bindParam("pid",  $pid);
    //                    $sqlstmt->bindParam("provider_id",  $provider_id); 
    //                    $sqlstmt->bindParam("user", $user);
    //                    $sqlstmt->bindParam("groupname",  $groupname);
    //                    $sqlstmt->bindParam("authorized", $authorized); 
    //                    $sqlstmt->bindParam("encounter", $encounter);
    //                    $sqlstmt->bindParam("code_text", $code_text);
    //                    $sqlstmt->bindParam("billed",  $billed);
    //                    $sqlstmt->bindParam("activity",  $activity); 
    //                    $sqlstmt->bindParam("payer_id", $payer_id);
    //                    $sqlstmt->bindParam("bill_process", $bill_process);
    //                    $sqlstmt->bindParam("bill_date",  $bill_date);
    //                    $sqlstmt->bindParam("process_date", $process_date);
    //                    $sqlstmt->bindParam("process_file", $process_file);
    //                    $sqlstmt->bindParam("target",  $target);
    //                    $data =  $sqlstmt->execute();  

    //                   $sqlstmt1 = $db->prepare("INSERT INTO billing (date,code_type,code,pid,provider_id,user,groupname,authorized,encounter,code_text,billed,activity,payer_id,bill_process,bill_date,process_date,process_file,target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(date('yy-mm-dd'),$code_type,$code,$pid,$provider_id,$user,$groupname,$authorized,$encounter,$code_text,$billed,$activity,$payer_id,$bill_process,$bill_date,$process_date,$process_file,$target)) ;
    //                   $data =  $sqlstmt1->execute();
               ...
               else if($codearray[$i]['code_type']=='CPT4' && $justify!='')
               {
                   ...
                   $sql2 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,modifier,units,fee,justify,target)"
                    ...
                    $sqlstmt2 = $db->prepare($sql2);
                    $data =  $sqlstmt2->execute(); 
    //                $sqlstmt2 = $db->prepare("INSERT INTO billing (date,code_type,code,pid,provider_id,user,groupname,authorized,encounter,code_text,billed,activity,payer_id,bill_process,bill_date,process_date,process_file,target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(date('yy-mm-dd'),$code_type,$code,$pid,$provider_id,$user,$groupname,$authorized,$encounter,$code_text,$billed,$activity,$payer_id,$bill_process,$bill_date,$process_date,$process_file,$target)) ;
    //                $data =  $sqlstmt2->execute();
                    ...
                    foreach($diags as $diag1){
                       ...
                       $sql1 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
                                . " VALUES ( NOW(),'ICD9','$diag1','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$title1', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date', '$process_date','$process_file','$target')";
                        $sqlstmt1 = $db->prepare($sql1);
                        $data =  $sqlstmt1->execute();
                        }
    //                    $sqlstmt1 = $db->prepare("INSERT INTO billing (date,code_type,code,pid,provider_id,user,groupname,authorized,encounter,code_text,billed,activity,payer_id,bill_process,bill_date,process_date,process_file,target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(date('yy-mm-dd'),'ICD9',$diag1,$pid,$provider_id,$user,$groupname,$authorized,$encounter,$title,$billed,$activity,$payer_id,$bill_process,$bill_date,$process_date,$process_file,$target)) ;
    //                    $data =  $sqlstmt1->execute();
                        ...
                        if($codearray[$i]['code_type']=='CPT4' && $issue_list=='YES'){
                            foreach($issues as $issues1) { 
                                    foreach($issues1 as $issueval ){
                                $diagnosis="ICD9:".$issueval;
                                $descr="select long_desc from icd9_dx_code where formatted_dx_code='$issueval'";
                                $sqlstmte = $db->prepare($descr) ;
                                $datas=  $sqlstmte->execute();
                                $desval1 = $sqlstmte->fetchAll(PDO::FETCH_OBJ);
                                if(!empty($desval1)){
                               ...
                               $sql4 = "INSERT into lists(date,begdate,type,occurrence,classification,pid,diagnosis,title,modifydate) values('$enc_date','$enc_date','medical_problem',0,0,$pid,'$diagnosis','$title1',NOW())"; echo"<br>";
                                $sqlstmt4 = $db->prepare($sql4);
                                $data4 =  $sqlstmt4->execute();
                                ...
                                $sel1="select pid,list_id ,encounter from issue_encounter where encounter=$encounter";
                                $selstmt = $db->prepare($sel1) ;
                                $seldata =  $selstmt->execute();
                                $val = $selstmt->fetchAll(PDO::FETCH_OBJ);
                                ...
                                if(!empty($val)){
                                   foreach($val as $value){
                                       if($value->list_id!=$newid && $value->encounter!=$encounter){  
                                           $sql5="INSERT INTO issue_encounter ( pid, list_id, encounter) VALUES ( $pid, $newid, $encounter)";
                                           $sqlstmt5 = $db->prepare($sql5) ;
                                           $data5 =  $sqlstmt5->execute();
                                        }
                                    } 
    ...
    //                  echo  $sql1 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
    //                    . " VALUES ( :date,:code_type,:code,:pid,:provider_id, :user,:groupname, :authorized,  :encounter, :code_text, :billed, :activity ,:payer_id,:bill_process,:bill_date, :process_date,:process_file,:target)";
                      $sql1 = "INSERT INTO billing (date, code_type, code, pid, provider_id, user,  groupname, authorized, encounter,  code_text, billed, activity,payer_id,bill_process,bill_date,process_date,process_file,target)"
                                . " VALUES ( NOW(),'ICD9','$diag4','$pid','$provider_id', '$user','$groupname', ' $authorized',  ' $encounter', '$title1', '$billed', '$activity' ,'$payer_id','$bill_process','$bill_date', '$process_date','$process_file','$target')";

    //                   echo $sql1='"INSERT INTO billing (date,code_type,code,pid,provider_id,user,groupname,authorized,encounter,code_text,billed,activity,payer_id,bill_process,bill_date,process_date,process_file,target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(NOW(),"ICD9",$diag4,$pid,$provider_id,$user,$groupname,$authorized,$encounter,$title,$billed,$activity,$payer_id,$bill_process,$bill_date,$process_date,$process_file,$target)';
                        $sqlstmt1 = $db->prepare($sql1) ;
                       $data =  $sqlstmt1->execute();

    //                    $sqlstmt1 = $db->prepare("INSERT INTO billing (date,code_type,code,pid,provider_id,user,groupname,authorized,encounter,code_text,billed,activity,payer_id,bill_process,bill_date,process_date,process_file,target) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(date('yy-mm-dd'),'ICD9',$diag4,$pid,$provider_id,$user,$groupname,$authorized,$encounter,$title,$billed,$activity,$payer_id,$bill_process,$bill_date,$process_date,$process_file,$target)) ;
    //                    $data =  $sqlstmt1->execute();
                        }
                    ...
                    if($codearray[$i]['code_type']=='CPT4' && $issue_list == 'YES'){
                        foreach($issues as $issues4) {
                            foreach( $issues4 as $issueval){
                            $diagnosis="ICD9:".$issueval;
                        ...
                        $sql4 = "INSERT into lists(date,begdate,type,occurrence,classification,pid,diagnosis,title,modifydate) values('$enc_date','$enc_date','medical_problem',0,0,$pid,'$diagnosis','$title1',NOW())";
                        $sqlstmt4 = $db->prepare($sql4);
                        $data4 =  $sqlstmt4->execute();
                        ...
                        $sel1="select pid,list_id ,encounter from issue_encounter where encounter=$encounter AND pid='$pid'";
                        $selstmt = $db->prepare($sel1) ;
                        $seldata =  $selstmt->execute();
...
//                        }else{
           ...
           if($codearray[$i]['code_type']=='ICD9')
           {
               ...
               $sql1 = "UPDATE billing SET  code='".$codearray[$i]['code']."',provider_id='$provider_id', authorized='$authorized',billed='$billed', activity='$activity',
                   payer_id='$payer_id',bill_process='$bill_process',bill_date='$bill_date',process_date='$process_date' ,process_file='$process_file',target='$target' WHERE  id = $id";                   
                $sqlstmt1 = $db->prepare($sql1) ;
                $data =  $sqlstmt1->execute();
