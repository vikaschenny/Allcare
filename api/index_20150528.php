...
$app->get('/savevitalsform','saveVitalsForm');
...
function getDemographics2($patientId)
{   
    $key = 'rotcoderaclla';
    // The default key size is 256 bits.
    $old_key_size = GibberishAES::size();
...
*/
    ...
    $sql13 = "select pn.id,pn.date,pn.body,pn.pid,pn.user,pn.groupname,pn.activity,pn.authorized,pn.title,pn.assigned_to,pn.deleted,pn.message_status 
              from pnotes pn inner join patient_data pd on pd.pid = pn.pid where pid=$pid and deleted=0 ";

    $sql14 = "select el.id,el.date,el.event,el.user,el.recipient,el.description,el.patient_id 
              from extended_log el inner join patient_data pd on pd.pid = el.patient_id where patient_id=$pid ";

    $sql15 = "select f.id,f.date,f.pid,f.user,f.groupname,f.authorized,f.activity,f.bps,f.bpd,f.weight,f.height,f.temperature,f.temp_method,f.pulse,
                 f.respiration,f.note,f.BMI,f.BMI_status,f.waist_circ,f.head_circ,f.oxygen_saturation
          from form_vitals f inner join patient_data pd ON f.pid = pd.pid where pid=$pid ";
...
function getPatientsByFacility($fid)
{
	...
	where facility_isactive='YES' AND tpf.facilityid=$fid  AND pd.practice_status = 'YES' AND pd.deceased_stat != 'YES'";  
...
function getDosFormDataDetails_test2($eid){
    try 
    {
        $db = getConnection();
        $key = 'rotcoderaclla';
                    ...
                    $datacheck = $stmt4->fetchAll(PDO::FETCH_OBJ);  
                        if(empty($datacheck)){ 
                            $sql5 = "SELECT l.field_id, l.seq as FormOrder,SUBSTRING(l.group_name ,-length(l.group_name),1) as grouporder,SUBSTRING(l.group_name FROM 2) as GroupName,
                                ...
                                from layout_options l WHERE l.title LIKE '$value3->screen_name' and l.form_id='LBF1' AND l.uor=2";
                            $sqlcheck = "SELECT  * FROM layout_options l WHERE l.title LIKE '$value3->screen_name' AND l.uor=2 and l.form_id = 'LBF1' ";
                            $stmtcheck = $db->prepare($sqlcheck);
                            $stmtcheck->execute();  
                            $datacheckcheck = $stmtcheck->fetchAll(PDO::FETCH_OBJ);
                            if(!empty($datacheckcheck)){
                                $stmt5 = $db->prepare($sql5);
                                $stmt5->execute();  
                                ...
                                $fielddata = "SELECT field_value from lbf_data WHERE form_id = $form_id AND field_id  = '".$datacheck2[0]->field_id."_text'";
                                $fielddatasql = $db->prepare($fielddata);
                                    $fielddatasql->execute();  
                                    $fielddatasqlcheck = $fielddatasql->fetchAll(PDO::FETCH_OBJ);
                                    if(!empty($fielddatasqlcheck)):
                                        $dictationval = $fielddatasqlcheck[0]->field_value;
                                    else:
                                        $dictationval = '';
                                    endif;    
                                $formvalues[] = (object)array( 'form_id'=> $form_id, 'form_name' =>$datacheck2[0]->field_id, 'dictation_text' => $dictationval );
                            }
                        }
//                        else{ 
...
//                        }
            ...
            //echo "<pre>";print_r($formvalues); echo "</pre>"; 
             ...
             $newdemo['Encounter Text Data'] = check_data_available($new);
            if($newdemo) {
            ...
            if(!empty($patients2)){
                foreach ($patients2 as $value) { 
                    ...
                    foreach($patients3 as $val){
                        ...
                        elseif($val->field_id == 'family_exam_test' || $val->field_id == 'family_history'|| $val->field_id == 'family_med_con' || $val->field_id == 'history_past' || $val->field_id == 'history_social'):
                            $form_id = 0;;
                        elseif($val->field_id  == 'allergies' || $val->field_id  == 'dental_problems' || $val->field_id  == 'codes' || $val->field_id  == 'immunization' || $val->field_id  == 'medical_problem'|| $val->field_id  == 'medication'):
                            $form_id = 0;;
                        else:
                            $formid = $value->form_id ;
                        endif;
                        if(empty($patients4)):
                            $datatext[] = (object)array('form_name' => $val->field_id, 'form_id' => $formid, 'dictation_text' => '' );
                        else:
                           $datatext[] = (object)array('form_name' => $val->field_id, 'form_id' => $formid, 'dictation_text' => $patients4[0]->field_value );
                        endif;
                    }
                }
            }else{
...
function getDosFormDataDetails_test($eid){
...
function getPatientInsuranceDataList($pid){
    ...
    $old_key_size = GibberishAES::size();
    GibberishAES::size(256);
    $sql = " SELECT i.id, i.type,i.provider,ic.name as insurancecompany_name,i.group_number,i.plan_name,i.policy_number,i.subscriber_lname,i.subscriber_mname, i.subscriber_fname,i.subscriber_relationship,
	...
	try 
        {
            ...
            foreach($formlabels as $element): 
                ...
                $incompleteforms = '';
//                echo $requiredsql = "SELECT f.screen_group, l.title AS screen_name,f.screen_group, l.description AS screen_link,l.field_id, l.title
//                FROM  `tbl_allcare_facuservisit` f
//                INNER JOIN layout_options l ON l.group_name = f.screen_group
//                AND l.form_id = f.form_id
//                WHERE  `facilities` REGEXP ('$element->facility_id') AND  `users` REGEXP ('$element->provider_id') AND  `visit_categories` REGEXP ('$element->visitcategory_id') AND l.uor=2";
                if(!empty($datacheck)):
                    //$sql3 = "SELECT field_id, title FROM layout_options WHERE form_id = 'LBF1' and uor = 2";
                    $sql3 = "SELECT f.screen_group, l.title AS screen_name,f.screen_group, l.description AS screen_link,l.field_id, l.title
                            FROM  `tbl_allcare_facuservisit` f
                            INNER JOIN layout_options l ON l.group_name = f.screen_group
                            AND l.form_id = f.form_id
                            WHERE  `facilities` REGEXP ('$element->facility_id') AND  `users` REGEXP ('$element->provider_id') AND  `visit_categories` REGEXP ('$element->visitcategory_id') AND l.uor=2";
                    ...
                    AND (
                     `elec_signed_on` = '' AND `elec_signedby` = ''
                            ...
                            if($incompleteforms == '' && empty($datacheck)):
                                ...
                                $sql7 = "SELECT f.screen_group, l.title AS screen_name,f.screen_group, l.description AS screen_link,l.field_id, l.title
                                    FROM  `tbl_allcare_facuservisit` f
                                    INNER JOIN layout_options l ON l.group_name = f.screen_group
                                    AND l.form_id = f.form_id
                                    WHERE  `facilities` REGEXP ('$element->facility_id') AND  `users` REGEXP ('$element->provider_id') AND  `visit_categories` REGEXP ('$element->visitcategory_id') AND l.uor=2";
...
function getIncompleteEncounterList_test2($pid){
...
function getIncompleteEncounterList_test($pid){
...
function getDosFormData_test($eid){
  try 
    {
        $db = getConnection();
        $key = 'rotcoderaclla';
...
function getROSFormData2($formid){
 try 
        {
            $key1 = 'rotcoderaclla';
            // The default key size is 256 bits.
...
function savePhysicalExamFormData(){
    try 
    {
        ...
        GibberishAES::size(256);
        $appres = GibberishAES::dec($request->getBody(), $key);
        //$appres = GibberishAES::dec('', $key);
        $insertArray = json_decode($appres,TRUE);
        $nameslist = array('GENWELL', 'GENAWAKE','HEADNORM','HEADLESI', 'EYECP','EYECON','EYEPER','ENTTM','ENTNASAL','ENTORAL','ENTSEPT','NECKSUP','BACKCUR','CVRRR','CVNTOH','CVCP','CVNPE','CVNMU','CHNSD','RECTAB','REEFF','RELUN','GIOG','GIHERN','GISOFT','GIBOW','GUTEST','GUPROS','GUEG','GUAD','GULES','EXTREMIT','EXTREDEF','LYAD','MUSTR','MUROM','MUSTAB','MUINSP','NEUCN2','NEUREF','NEUSENS','NEULOCAL','PSYAFF','PSYJUD','PSYDEP','PSYSLE','PSYTHO','PSYAPP','PSYABL','SKRASH','SKCLEAN','OTHER');
        $formid                     = $insertArray['formid'];
        $encounter                  = $insertArray['encounter'];
...
function createNewEnounter2(){
    ...
    try{
        ...
        if($q->execute()){
            $sqlGetLastEncounter="SELECT MAX(encounter) as encounter FROM form_encounter WHERE pid=$pid AND provider_id=$user";
                            ...
                            foreach($sethistorydata[$i] as $key => $sethis){ 
                                ...
                                if($fieldnamesarray->list_id != ''){
                                    ...
                                    if($fieldnamesarray->data_type == 28){
                                        ...
                                        if(!empty($sethis)){
                                            $getfacilityname = "SELECT  name FROM facilities WHERE id = $sethis";
...
function getLayoutHistory2($patientId){
    try
    {	
        $newdemo = array();
        $dataarray = array();
                                ...
                                if($fieldnamesarray->list_id != ''){
                                    ...
                                    if($fieldnamesarray->data_type == 23  ){
                                        if(!empty($sethis)){
                                            ...
                                            $data[$setgroupnames->group_name]['N/A'] = rtrim($natype, ',');
                                            $data[$setgroupnames->group_name]['Normal'] = rtrim($ntype, ',');
                                            $data[$setgroupnames->group_name]['Abnormal'] = rtrim($atype, ',');
                                        }else{
                                            $data[$setgroupnames->group_name][$title] = $sethis;
                                        }    
                                    }elseif($fieldnamesarray->data_type == 32  ){
                                        if(!empty($sethis)){
                                            $explodeval = explode("|", $sethis);
                                            ...
                                            $smokingdata = $titlefield.$explodeval[0].str_repeat('&nbsp;', 5)."Status: ".$statusname. "  ".$explodeval[2];
                                            $data[$setgroupnames->group_name][$titlefield] = $smokingdata;
                                        }else{
                                            $data[$setgroupnames->group_name][$title] = $sethis;
                                        }    
                                    }else{
                                        if(!empty($sethis)){
                                            ...
                                            $data[$setgroupnames->group_name][$fieldnamesarray->title] = rtrim($arraydata,',');
                                        }else{
                                            $data[$setgroupnames->group_name][$title] = $sethis;
                                        }   
                                    }
                                    ...
                                    if($fieldnamesarray->data_type == 28){
                                        if(!empty($sethis)){
                                            ...
                                            $smokingdata = $explodeval[0].str_repeat('&nbsp;', 5)."Status: ".$statusname. "  ".$explodeval[2];
                                            $data[$setgroupnames->group_name][$fieldnamesarray->title] = $smokingdata;
                                        }else{
                                            $data[$setgroupnames->group_name][$title] = $sethis;
                                        }    
                                    }else{
...
function getIssuesByEncounter_old($encounter,$type)
{
