...
curl_setopt($curl, CURLOPT_USERAGENT, "valid user agent");
   ////execute the post

   $estimationID = curl_exec($curl);
...
//  Create a “representation” of the request by concatenating the following values into a single string (in this order, with no separators):
    $posted = "GEThttps://estimationapi.zirmed.com/1.0/document/".$estimationID.$formatDate."101247";
    $encoded_signature = base64_encode(hash_hmac('sha256',$posted, 'f6sdnmV1ItrAoOzlR1QZRSGSnGng5HV0KQZtSR4U',true));
   
   
   //echo "<pre>"; print_r(json_encode($arr)); echo "</pre>";
   
   $form_url = 'https://estimationapi.zirmed.com/1.0/document/'.$estimationID;
...
//   $request_headers[] = 'Authorization:'.$hmacSignature;
  ...
  // curl_setopt($curl,CURLOPT_POST, sizeof($data));
   ...
   curl_close($curl);
    ...
    $destination = dirname(__FILE__) . '/'.$estimationID.'.pdf';
...
<embed src="<?php echo 'http://'. $_SERVER['HTTP_HOST'].'/api/'.$estimationID.'.pdf'; ?>" width="800" height="600" alt="pdf">