...
//    print_r($request);
    $pid            = $insertArray['pid'];
    $provider       = $insertArray['provider'];
    $encoded_string = $insertArray['hiddendata'];
    ...
    try{
        ...
        $form_url = "http://qa2allcare.texashousecalls.com/api/DriveSync/uploadMobileHTML";
        
        // This is the data to POST to the form. The KEY of the array is the name of the field. The value is the value posted.
        $data_to_post = array();
        
        // to get email id 
        $get_email_query = 'select notes from list_options where list_id="AllcareDriveSync" and option_id = "mobile_email"';
...
//            $encounter_date = isset($set_patient_query[0]->date)? $set_encounter_date_query[0]->date : '';
        ...
        $file_id = str_replace("File ID: ", "",$result);
        //
...
//                    $setpagebr5= sqlFetchArray($getpagebr5);
        ...
        if(!empty($eidval[0]->elec_signedby)  && !empty($eidval[0]->elec_signed_on) && $eidval[0]->elec_signed_on != '0000-00-00 00:00:00'){
            $getSignImg="SELECT signature_image, CONCAT(fname,' ', lname) as fname,
                    (SELECT provider_credentials FROM tbl_patientuser where userid = '".$eidval[0]->elec_signedby."' ) as designation
                    FROM users
...
//        $encoded_string = base64_encode( htmlentities("<html><style>@page { size 8.5in 11in; margin: 1cm; }
//            * { font-family: Arial, Helvetica, sans-serif;}
//            div.page { page-break-before: always } 
//            ul{    list-style-type: none;    -webkit-padding-start: 0px !important;}li { padding-right:40px; }li:empty {   display: none; }</style><body class='changbg'>".$display_string2."<footer style='clear:both;' align='right'>
//              <p>Printed on ". $date->format('d-M-Y H:i:s a').".</p>
//            </footer></body></html>") );
        $encoded_string = base64_encode( "<html><style>@page { size 8.5in 11in; margin: 2cm; }
        ...
        echo "<!DOCTYPE html'><html><head>
            ...
            <script>
                    ...
                    function sendToDrive(element,event){
                    ...
                    function savetodrivefunction(pid,encounter,provider){
                        var hiddendata  = document.getElementById(hiddenpost);
                        var pid         = document.getElementById(pid);
                        var provider    = document.getElementById(provider);
                        var encounter   = document.getElementById(encounter);
                        
                        var fieldname = {hiddendata:hiddendata,pid:pid,provider:provider,encounter:encounter};
                        jQuery.ajax({
                            type: 'POST',

                            url: document.location.origin+'/api/medrapi.php/savetodrive',
                            data: fieldname,
                            dataType : 'jsonp',
...
//                                            while($setvalname=sqlFetchArray($getvalname)){
                                        ...
                                        if(!empty($explodeval[0]) || !empty($statusname) || !empty($explodeval[2])){
                                            $display_string .= $explodeval[0].str_repeat('&nbsp;', 5)."<b><u>Status</u>:</b> ".$statusname. "  ".$explodeval[2]."</li>";
                                            //$i++;$datacheck = '';
                                        }else{
                                            echo "</li>";
                                        }     
