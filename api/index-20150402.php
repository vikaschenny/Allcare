...
function getPatientsByday($loginUserId,$day)
{
    ...
    $sql = "SELECT ope.pc_eid as apptid,pd.pid,pd.title,pd.fname,pd.lname,if (pd.sex = 'Female' ,'F','M' ) as sex,
                ...
                WHERE ope.pc_aid=:loginUserId AND ope.pc_eventdate=:day";  
        ...
        if($resFormExists)
        {
			...
			$sqlGetDictationText="SELECT fd.pid,fd.date,fd.user,fd.dictation,f.form_name
				...
				fd.id=f.form_id
				  ...
				  AND f.deleted=0
				  AND f.form_name='Speech Dictation' order by f.date DESC LIMIT 1";
...
function createDictation()
{
	...
	//$dictationArray = json_decode($request->getBody(),TRUE);
		...
		*/
			...
			$qry = "SELECT pid FROM patient_data WHERE pid=$pid";
			$stmt = $db->prepare($qry) ;          
			$stmt->execute();
			$prs = $stmt->fetchAll();
...
function getPatientsByFacility($fid)
{
    ...
    if($fid == 0){
        $sql="select pd.pid, pd.title,pd.fname,pd.lname, DATE_FORMAT(pd.DOB,'%m-%d-%Y') AS DOB,
            ...
            pd.street,pd.city,pd.state,pd.country_code,pd.postal_code,pd.phone_cell from patient_data pd where pid not in (SELECT DISTINCT(patientid) FROM tbl_patientfacility)";  
        try 
        {
            $db = getConnection();
            $stmt = $db->prepare($sql) ;
	...
	where facility_isactive=1 AND tpf.facilityid=$fid";  
	...
	try 
		...
		else
		{
			...
			if($rs)
			{
				...
				from pnotes pn inner join users us on pn.user=us.username
				inner join patient_data pd on pd.pid=pn.pid
				where pn.assigned_to='$username' and pn.deleted=0";
...
function getActiveMessages($loginProvderId, $isActive)
{
	...
	if($rs)
	{
		...
		{	
			...
			from pnotes pn inner join users us on pn.user=us.username
			inner join patient_data pd on pd.pid=pn.pid
			where pn.assigned_to='$username' and pn.deleted=0 and pn.message_status!='Done'";
		}
...
function getTotalPatients()
{
	...
	$sql = "select pid,fname,lname from patient_data";      
        $count= "select count(*) from patient_data";
        $key = 'rotcoderaclla';
        // The default key size is 256 bits.
        $old_key_size = GibberishAES::size();
        ...
        try 
        {
            ...
            foreach ($dataresult[0] as $key=>$value) {
                if($key == 'constitutional' || $key == 'eyes' || $key == 'ent' || $key == 'breast' || $key == 'respiratory' || $key == 'cardiovascular' || $key == 'gastrointestinal' || $key == 'genitourinary' || $key == 'genitourinarymale' || $key == 'genitourinaryfemale' || $key == 'musculoskeletal' || $key == 'extremities' || $key == 'neurologic' || $key == 'skin' || $key == 'psychiatric' || $key == 'endocrine' || $key == 'hai' || $key == 'neck'){
                    ...
                    $data['data']['valuelist'] = array('Not Specified', 'Select Details', 'Normal');
                    $dataarray[] = $data; 
                }
            }
            foreach ($dataresult[0] as $key=>$value) {
...
function saveROSFormData(){
    try 
    {
        ...
        $constitutional             = $insertArray['constitutional']; 
        $eyes                       = $insertArray['eyes'];  
        $ent                        = $insertArray['ent'];  
        $breast                     = $insertArray['breast'];  
        $respiratory                = $insertArray['respiratory']; 
        $cardiovascular             = $insertArray['cardiovascular'];  
        $gastrointestinal           = $insertArray['gastrointestinal'];  
        $genitourinary              = $insertArray['genitourinary'];  
        $genitourinarymale          = $insertArray['genitourinarymale'];  
        $genitourinaryfemale        = $insertArray['genitourinaryfemale'];  
        $musculoskeletal            = $insertArray['musculoskeletal'];  
        $extremities                = $insertArray['extremities'];  
        $neurologic                 = $insertArray['neurologic']; 
        $skin                       = $insertArray['skin']; 
        $psychiatric                = $insertArray['psychiatric']; 
        $endocrine                  = $insertArray['endocrine']; 
        $hai                        = $insertArray['hai'];  
        $neck                       = $insertArray['neck'];  
