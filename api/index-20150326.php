...
// method to face to face configuration notes
...
// To retrieve Encountered Reason
$app->get('/encounteredReason/:pid', 'getencounteredReason');
// To retrieve date of service
$app->get('/dateofService/:pid/:encounter', 'getDateOfService');
/* ============ Hema methods calls end here ================== */
...
/*
$app->post('/speechDictation', function () use ($app) {
   
    $dictationJSONArray=$app->request()->post('jarrDictation');
    $dictationArray = json_decode($dictationJSONArray, TRUE);
   
   createDictation($dictationArray);
});
*/
...
//$app->get('/createfacetoface/:Id/:pid/:loginProvderId/:encounter/:radBound/:radCare/:radPhysician/:radVisit/:nextVisitDays/:txtMedical/:txtNursing/:txtPhysical/:txtOccupational/:txtSpeech/:txtTreatment/:txtFindings/:txtHomeBound/:txtNurse/:txtNursePractitionerSignDate/:txtPhysicianSignature/:txtPrintedName/:txtPrintedDate/:txtVisitDate','createFacetoFace');


/*$app->post('/createfacetoface', function () use ($app) {

//$app->response()->header("Content-Type", "application/json");

    $facetofaceJSONArray=$app->request()->post('jarrSaveFaceToFace_new');
    $facetofaceArray = json_decode($facetofaceJSONArray, TRUE);
    createFacetoFace($facetofaceArray);
});*/
$app->post('/createfacetoface', 'createFacetoFace');


                   //   76/1/51/~/specimen_week/N/Y/Y/~/~/CXR,UA/Y/Accept/physician_signature/2014-05-18
//$app->get('/labrequest/:Id/:pid/:loginProvderId/:encounter/:date_of_request/:specimen_week/:fasting/:frail_health/:is_home_bound/:is_preference_home_health/:diagnosis_codes/:tests/:is_colonoscopy_required/:patient_has/:physician_signature/:date_of_signature','createLabRequest');

/*
$app->post('/labrequest', function () use ($app) {
//echo "8989";
    $labrequestJSONArray=$app->request()->post('jarrSaveLabRequest_new');
    $labrequestArray = json_decode($labrequestJSONArray, TRUE);
    createLabRequest($labrequestArray);
});
*/
...
//$app->get('/vital/:Id/:pid/:loginProvderId/:encounter/:bps/:bpd/:weight/:weight_unit/:height/:height_unit/:temperature/:temperature_unit/:temp_method/:pulse/:respiration/:note/:BMI/:BMI_status/:waist_circ/:waist_circ_unit/:head_circ/:head_circ_unit/:oxygen_saturation','createVital');
/*
$app->post('/vital', function () use ($app) {

    $vitalsJSONArray=$app->request()->post('jarrSaveVitals_new');
    $vitalsArray = json_decode($vitalsJSONArray, TRUE);
    createVital($vitalsArray);
});*/
$app->post('/vital', 'createVital');


//$app->get('/soap/:Id/:pid/:loginProvderId/:encounter/:subjective/:objective/:assessment/:plan','createSoap');
/*
$app->post('/soap', function () use ($app) {
//echo "in soap";
    $soapJSONArray=$app->request()->post('jarrSaveSoap_new');
    $soapArray = json_decode($soapJSONArray, TRUE);
    createSoap($soapArray);
});*/
...
/*
$app->post('/createappointment', function () use ($app) {
    $appointmentDetailsJSONArray=$app->request()->post('txtAppointmentDetails');
    $appointmentDetailsArray = json_decode($appointmentDetailsJSONArray, TRUE);
    createAppointment($appointmentDetailsArray);
});*/
$app->post('/createappointment', 'createAppointment');

//$app->get('/checkTimeSlot','checkTimeSlot');

//$app->get('/availabletimeslots/:loginProvderId','getAvailableTimeSlots');
...
*/
	$sql = "select pd.pid,pd.title,pd.fname,pd.lname, DATE_FORMAT(pd.DOB,'%m-%d-%Y') AS DOB,DATE_FORMAT(FROM_DAYS(DATEDIFF(CURDATE(),pd.DOB)), '%Y')+0 AS age,if (pd.sex = 'Female' ,'F','M' ) as sex,pd.street,pd.city,pd.state,pd.country_code,pd.postal_code,pd.phone_home,pd.phone_biz,pd.phone_contact,pd.phone_cell
...
/*
	$sqlCheckDictationExists="SELECT f.form_id
		                      FROM forms f
		                      WHERE f.pid=$pid
		                      AND f.user='$username'
...
/*
	...
	$qryIsAr_Activity="SELECT pid, encounter, code_type, code, modifier, payer_type, post_time, post_user, session_id, pay_amount, account_code
        ...
        $stmtIsAr_Activity = $db->prepare($qryIsAr_Activity) ;		
        $stmtIsAr_Activity->execute();
        $resIsAr_Activity = $stmtIsAr_Activity->fetchAll(PDO::FETCH_OBJ);
	...
	$qryIsAr_Session="SELECT payer_id, user_id, closed, pay_total, created_time, global_amount, payment_type, description, adjustment_code, post_to_date, patient_id
                        FROM ar_session
                        WHERE encounter=$encounter AND patient_id=$pid 
                        AND user_id=$loginProvderId AND DATE(created_time)=DATE(now())";
        $stmtIsAr_Session = $db->prepare($qryIsAr_Session) ;		
        $stmtIsAr_Session->execute();
        $resIsAr_Session = $stmtIsAr_Session->fetchAll(PDO::FETCH_OBJ);
...
//function createLabRequest($labrequestArray)
{
  ...
  fasting,frail_health,is_home_bound,is_preference_home_health,
  diagnosis_codes,tests,is_colonoscopy_required,patient_has,
  physician_signature,date_of_signature,created_date)
  values (:pid,:encounter,:loginProvderId,:date_of_request,:specimen_week,:fasting,
:frail_health,:is_home_bound,:is_preference_home_health,:diagnosis_codes,:tests,:is_colonoscopy_required,:patient_has,:physician_signature,:date_of_signature,NOW())";
*/
...
{
/*                       
     $sql = "SELECT fv.id,fv.date,fv.pid,fv.user,fv.authorized,fv.activity,fv.bps,fv.bpd,fv.weight,fv.weight_unit,fv.height,fv.height_unit,fv.temperature,fv.temperature_unit,
...
{
	$db = getConnection();
    $qry = "select title from list_options where list_id='apptstat'";
    $stmt = $db->prepare($qry) ;
    $stmt->execute();
...
// To encounter reason for patient

function getencounteredReason($pid){

	$sql = "SELECT encounter,reason from form_encounter WHERE pid = :pid";
	...
	try 
        {
            ...
            {
                //echo 'No Such patient data available';
                echo 'No Encounters found for this patient';
            }
        } 
        catch(PDOException $e) 
        {
...
function getDateOfService($pid,$encounter){

	$sql = "SELECT DATE(date) as date from form_encounter WHERE pid = :pid AND encounter = :encounter";
	...
	try 
        {
            ...
            {
                //echo 'No Such patient data available';
                echo 'No Date of Service for this patient';
            }
        } 
        catch(PDOException $e) 
        {
