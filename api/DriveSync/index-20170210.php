...
//                            echo "select * from patient_data where patient_folder LIKE '%$folderId%'";
            ...
            $sql=$f3->set('result',$db->exec("select username from tbl_user_custom_attr_1to1 uc inner join users u u.id=uc.userid where drive_sync_folder LIKE '%$folderId%'"));
       ...
       foreach($files as $key => $val){
            ...
            $createdFile = $service->files->create($fileMetadata, array(
             ...
             $ins_log=$f3->set('result',$db->exec("insert into DriveSync_log (date,user,email,encounter,patient_id,google_folder,file_name ,file_id,status,watsID,category)values(now(),'".$user."','".$email."','','$obj_id','$individual_link','$originalFile','','files uploaded in $category','','$category')"));
        } 
...
//        $user_cus=$f3->set('result',$db->exec("select upload_settings from tbl_drivesync_authentication where email='$email'  order by id desc"));   
//        $ser1=unserialize($user_cus[0]['upload_settings']);                      
//        $ser=explode(",",unserialize($ser1['user']));
   
        
//        $myArray = array_filter( $ser );
//        foreach($myArray as $val){
//            $user_cus1=$f3->set('result',$db->exec("select email from tbl_user_custom_attr_1to1 where userid='".$val."'"));
//            $user_cus2=$f3->set('result',$db->exec("select username from users where id='".$val."' and username!=''"));
//            $user_nam=$user_cus2[0]['username'];
//            $email_id=$user_cus1[0]['email'];
//            /*********send Emr message notification******/
//            $body="(Uploaded files in $category)".$msglink;
...
////                            values (date_format(now(), '%Y-%m-%d %H:%i'),'".$body."', 0, '$user_nam', 'Default', 1, 1, 'Uploaded files in $category', '".$user_nam."', 'new')"));//
//            $data3=$f3->set('result',$db->exec("INSERT INTO tbl_allcare_custom_messages (date, body, obj_id, user, groupname, activity, authorized, title, assigned_to, message_status,object_type , priority)
//                            values (date_format(now(), '%Y-%m-%d %H:%i'),'".$body."', '$obj_id1', '$user_nam', 'Default', 1, 1, 'Uploaded files in $category', '".$user_nam."', 'new','$category','Very_High')"));
...
//            
//            
//           $smtp_details  =$f3->set('result',$db->exec("select gl_name, gl_value from globals where gl_name IN('SMTP_HOST','SMTP_PORT','SMTP_PASS','SMTP_USER') "));
//            
//            /*********send email notification******/
//            $mail = new PHPMailer;
//            $mail->isSMTP();
