...
//			 $redirect_uri ="http://qa2allcare.texashousecalls.com/interface/main/allcarereports/drivesync_config.php?status=1"; 
...
//                          $redirect_uri ="http://qa2allcare.texashousecalls.com/interface/main/allcarereports/drivesync_config.php?status=0"; 
//                         header('Location: ' . filter_var($redirect_uri, FILTER_SANITIZE_URL));
...
//            echo $_SESSION['access_token'];
            ...
            $redirect_uri = 'http://' . $_SERVER['HTTP_HOST'] . '/api/DriveSync/oauth2callback_web.php';
           // $redirect_uri ="http://qa2allcare.texashousecalls.com/interface/main/allcarereports/drivesync_config.php?from=1"; 
            header('Location: ' . filter_var($redirect_uri, FILTER_SANITIZE_URL));
        }
}		
);
...
$f3->route('GET /movefile_web/@useremail/@fileid/@tofolderid',
function($f3)
{
	movefile_web($f3->get('PARAMS.useremail'),$f3->get('PARAMS.fileid'),$f3->get('PARAMS.tofolderid'),$f3);
});

$f3->route('GET /renameFile_web/@fileId/@newTitle/@email',
function($f3)
{
    renameFileWeb($f3->get('PARAMS.fileId'),$f3->get('PARAMS.newTitle'),$f3->get('PARAMS.email'),$f3);			
...
function movefile_web($email,$fileId,$folderId,$framework)
{
	updateTokenWeb($email,$framework);
	$db=$framework->get('DB');
	$data=$framework->set('result',$db->exec("select accesstoken from drive_users where email='".$email."' order by id desc"));
	...
	$file = $driveService->files->update($fileId, $emptyFileMetadata, array(
                                                                              ...
                                                                              'fields' => 'id, parents'));
	$t=time();
        updateLogWeb('Moving file',$t,$email,'Success',get_client_ip(),'File moved to: '.  $filename.'--->'.$tofolder,$framework);
	echo 'sucess';	  
}


$f3->route('GET /deletefile_web/@useremail/@fileid',
function($f3)
{
	$email=$f3->get('PARAMS.useremail');
	$fileId=$f3->get('PARAMS.fileid');
...
function renameFileWeb( $fileId, $newTitle,$email,$framework) {
    $useremail=$email;
...
$f3->route('GET|POST|PUT /export_web/@email/@fileid', 
   function($f3){
        $email= $f3->get('PARAMS.email');
        $mediaid = $f3->get('PARAMS.fileid');
        updateTokenWeb($email,$f3);
...
//                                                        $replace[] = "src=\"http://qa2allcare.texashousecalls.com/api/DriveSync/email_attachments/$uniqueFilename\"";
...
//                                    }
                ...
                if($message!=''){
//                     
                  // printFileWeb($email,$message,$folder_name_format,$createdFolder->id,$framework);
                   ...
                   $user_cus=$framework->set('result',$db->exec("select email_parent_folder,email_to_users from tbl_drivesync_authentication where email='$email'  order by id desc"));
                   // $user_cus=$framework->set('result',$db->exec("select userid,emr_msg,drive_email,email from tbl_user_custom_attr_1to1 where emr_msg!='' and drive_email!=''"));
                   $ser=explode(",",unserialize($user_cus[0]['email_to_users']));
                    ...
                    foreach($ser as $val){
                      ...
                      $email_id=$user_cus[$i]['email'];
                            ...
                            $data3=$framework->set('result',$db->exec("INSERT INTO pnotes (date, body, pid, user, groupname, activity, authorized, title, assigned_to, message_status)
                            values (NOW(), CONCAT(date_format(now(), '%Y-%m-%d %H:%i'),'".' '."(folder location)".' '."https://drive.google.com/drive/folders/$createdFolder->id'), 0, '$user_nam', 'Default', 1, 1, 'Store Email Attachment', '".$user_nam."', 'new')"));
                        
                        
                            $to = $email_id;
                            $subject = "Email Attachment Folder Link";
                            $link='https://drive.google.com/drive/folders/'.$createdFolder->id;
                            $email_content=$message."<br>".$link;
...
//	$username = 'bhavyae@smartmbbs.com';
//	$password = 'echambadi@528';
	 $data2=$framework->set('result',$db->exec("select imap_pwd from tbl_drivesync_authentication where email='$email'  order by id desc"));
...
$f3->route('GET|POST|PUT /uploadfile_web/@email/@folderId/@category',
    function($f3){
        $f3->set('UPLOADS','uploads/'); // don't forget to set an Upload directory, and make it writable!
        $email=	$f3->get('PARAMS.email');	
        $folderId   = $f3->get('PARAMS.folderId');
       ...
       foreach($files as $key => $val){
            ...
            $createdFile = $service->files->create($fileMetadata, array(
             ...
             $link.='https://drive.google.com/drive/folders/'.$createdFile->id."||";
             $view_link.='https://drive.google.com/drive/folders/'.$createdFile->id."<br>";             
             
        } 
	$link1=trim($link,"||");	
      
        $user_cus=$f3->set('result',$db->exec("select userid,emr_msg,drive_email,email from tbl_user_custom_attr_1to1 where emr_msg!='' and drive_email!=''"));
                   
        for($i=0;$i<count($user_cus);$i++){
            $user_cus1=$f3->set('result',$db->exec("select username,email from users where id='".$user_cus[$i]['userid']."' and username!=''"));
            $user_nam=$user_cus1[0]['username'];
            $email_id=$user_cus[$i]['email'];
            if($user_cus[$i]['emr_msg']=='YES'){
                $data3=$f3->set('result',$db->exec("INSERT INTO pnotes (date, body, pid, user, groupname, activity, authorized, title, assigned_to, message_status)
                values (NOW(), CONCAT(date_format(now(), '%Y-%m-%d %H:%i'),'".' '."(uploaded files in $category)".' '." $view_link'), 0, '$user_nam', 'Default', 1, 1, 'Uploaded files in $category', '".$user_nam."', 'new')"));
                $lastid=$f3->set('result',$db->exec("select LAST_INSERT_ID() as lastid")); 
                 $data3=$f3->set('result',$db->exec("INSERT INTO tbl_pnotes_file_relation (date, mid, doc_links, type)
                values (NOW(),'".$lastid[0]['lastid']."','$link1','$category')"));
            }
           
            if($user_cus[$i]['drive_email']=='YES'){
                $to = $email_id;
                $subject = "Uploaded folder links";
               
                $email_content="Uploaded file links in $category <br>".$view_link;
