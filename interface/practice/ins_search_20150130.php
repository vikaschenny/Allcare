...
if($_POST['hiddensqlGroupRows']>0)
      {
          ...
          if($_POST['hiddenarrayGroup']!='')
          {
             ...
             $groupCheckQuery = sqlStatement("select id from tbl_allcare_insurance1to1 where insurance_company_id=".$ins_id);
             if(sqlNumRows($groupCheckQuery)>0)
             {
                 
               echo $groupupdateSql1to1 ="update tbl_allcare_insurance1to1 set $updateValues where insurance_company_id =".$groupCheckQuery['id'];   
       ...
       sqlStatement("delete from tbl_allcare_insurance1ton where insurance_company_id=".$ins_id);
                  ...
                  $insertline='';
                    for($cnt2=0;$cnt2<$rowsvalues;$cnt2++)
                    {
                      ...
                      $insertSql = "insert into tbl_allcare_insurance1ton(pid,insurance_company_id,Recordset_ID,".$_POST['hiddenrecid'][$key].") "
                            . "values (0,$ins_id,$key,$insertline)" ;
                       // echo "<br>".$insertSql;
...
// ******* Insert Insurance Attributes ENDS ***************************


	
// ******* Insert Pharmacy Attributes  **************************
...
if($_POST['hiddensqlGroupRows_pharmacy']>0)
      {
          ...
          if($_POST['hiddenarrayGroup_pharmacy']!='')
          {
             ...
             $groupCheckQuery = sqlStatement("select id from tbl_allcare_pharmacy1to1 where pharmacy_id=".$ins_id);
             if(sqlNumRows($groupCheckQuery)>0)
             {
                 
               echo $groupupdateSql1to1 ="update tbl_allcare_pharmacy1to1 set $updateValues where pharmacy_id =".$groupCheckQuery['id'];   
       ...
       sqlStatement("delete from tbl_allcare_pharmacy1ton where pharmacy_id=".$ins_id);
...
// ******* Insert Pharmacy Attributes ENDS ***************************
...
<ul class="tabNav">
<li class='current'>
<a onclick="javascript:jQuery('#div_patient_insurance_attributes').hide();
                       jQuery('#div_patient_insurance').show();
...
<li>
<a onclick="javascript: addedit_insurance_attributes();jQuery('#div_patient_insurance_attributes').show();
                       jQuery('#div_patient_insurance').hide();
                       jQuery('.tabNav > li').removeClass('current');
...
<div id="div_patient_insurance_attributes">
