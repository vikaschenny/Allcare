    ...
    if(isset($FieldName))   //if(isset($relatedTables))
    {
        foreach($FieldName as $key=>$value)   //foreach($relatedTables as $key=>$value)
        {
            if($value!='none')
            {
                $extension_name=$_POST['extension_name'];
                $meta_table_name='';
                if($extension_name=='patients')
                {
                    $meta_table_name='tbl_allcare_tablemeta';
                }
                else
                {
                    $meta_table_name='tbl_allcare_tablemeta_'.$extension_name; 
                }
                ...
                $lastRecordFieldId=  sqlStatement('SELECT field_ID FROM '.$meta_table_name.'
                                                   WHERE id IN (SELECT MAX(id) FROM '.$meta_table_name.')');
                
                //$lastInsertedFieldId=  sqlStatement('SELECT field_ID FROM tbl_allcare_tablemeta_$extension_name WHERE id='.mysql_insert_id().'');
                $lastFieldId=sqlFetchArray($lastRecordFieldId);
                
                $tableId=($tableName=='tbl_allcare_'.$extension_name.'1to1')?1:2;
                 
                $table_original=($relatedTables[$key]!='none') ? $relatedTables[$key] : '' ;

                sqlStatement("INSERT INTO $meta_table_name(table_ID,table_Name,field_ID,field_Name,is_Field_Editable,created_Date) 
...
{
    insert_in_tables('tbl_allcare_'.$extension_name.'1to1');
}

if(isset($_POST['chk1ton']) && $_POST['chk1ton']==1)
{
    insert_in_tables('tbl_allcare_'.$extension_name.'1ton');
}
