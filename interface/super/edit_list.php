...
if ($_POST['formaction']=='save' && $list_id) {
    ...
    }else if ($list_id == 'AllcareRolesMapping') {
      // special case for code types
      sqlStatement("DELETE FROM tbl_allcare_roles_mapping");
      for ($lino = 1; isset($opt["$lino"]['wp_role']); ++$lino) {
        $iter = $opt["$lino"];
        $role_active = empty($iter['active' ]) ? 0 : 1;
        $wp_role  = formTrim($iter['wp_role']);
        $emr_role  = formTrim($iter['emr_role']);
        $provider  = empty($iter['provider' ]) ? 0 : 1;
       
        if (strlen($wp_role) > 0 ) {
          sqlInsert("INSERT INTO tbl_allcare_roles_mapping ( " .
            "active, wp_role, emr_role, provider " .
            ") VALUES ( "   .
            "'$role_active' , " .
            "'$wp_role'  , " .
            "'$emr_role' , " .
...
//  echo "</td>\n";
  echo "  <td align='center' class='optcell'>";
  echo "<input type='text' name='opt[$opt_line_no][id]' value='" .
       htmlspecialchars($option_id, ENT_QUOTES) . "' size='12' class='optin' />";
  echo "</td>\n";

  echo "  <td align='center' class='optcell'>";
  echo "<input type='text' name='opt[$opt_line_no][title]' value='" .
       htmlspecialchars($title, ENT_QUOTES) . "' size='20' class='optin' />";
  echo "</td>\n";
  ...
  echo "<input type='text' name='opt[$opt_line_no][codes]' title='" .
      ...
      htmlspecialchars($codes, ENT_QUOTES) . "' onclick='select_clin_term_code(this)' size='25' class='optin' />";
  echo "</td>\n";
...
//  echo "</td>\n";
  echo "  <td align='center' class='optcell'>";
  echo "<input type='text' name='opt[$opt_line_no][category]' value='" .
       htmlspecialchars($category, ENT_QUOTES) . "' size='20' class='optin' />";
  echo "</td>\n";

  echo "  <td align='center' class='optcell'>";
  echo "<input type='text' name='opt[$opt_line_no][option]' value='" .
       htmlspecialchars($option, ENT_QUOTES) . "' size='20' class='optin' />";
  echo "</td>\n";
 ...
 * and  custom function: writeRoleLine().
 */
...
//custom
function writeRoleLine($rl_array) {
  global $opt_line_no;
  ...
  echo ctGenCBox($opt_line_no, $rl_array, 'active',
    xl('Is this  role active?'));
  echo ctGenCell($opt_line_no, $rl_array, 'wp_role' , 6, 15,
    xl('WP Role'));
  echo ctGenCell($opt_line_no, $rl_array, 'emr_role'  ,10, 200,
    xl('Emr role'));
  echo ctGenCell($opt_line_no, $rl_array, 'provider' , 6, 30,
    xl('Provider'));
...
<?php }else if ($list_id == 'AllcareRolesMapping') { ?>
  <td><b><?php xl('Active'      ,'e'); ?></b></td>
  <td><b><?php xl('WP Roles'        ,'e'); ?></b></td>
  <td><b><?php xl('Allcare Roles'          ,'e'); ?></b></td>
  <td><b><?php xl('Provider'       ,'e'); ?></b></td>
...
if ($list_id) {
  if ($list_id == 'feesheet') {
    ...
    $res1 = sqlStatement("SELECT * FROM tbl_allcare_roles_mapping "  );
