...
$getquery = sqlStatement("SELECT fo.fs_option, vc.code_options,fo.fs_codes FROM fee_sheet_options fo INNER JOIN tbl_allcare_vistcat_codegrp vc ON vc.code_groups = fo.fs_category  WHERE `facility` = $facility_id AND `visit_category` = $pc_catid AND vc.code_options REGEXP (fo.fs_option)");
$array = array();
