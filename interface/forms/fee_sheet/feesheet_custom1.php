        ...
        function checkRequiredPrimary(){
            ...
            jQuery("#cpticd_table tr").each(function(){
                if(jQuery(this).find("td:first").text() != ''){
                    var splitString = jQuery(this).find("td:first").text().split('-');
            ...
            jQuery('#cpt_Primary option').each(function() {
                ...
                if(isthere != -1){
                    jQuery("#cpt_Primary").prop('required',false);alert('1');
                    jQuery("label[for='cpt_Primary']").text(jQuery("label[for='cpt_Primary']").text());alert('2');
                    jQuery("label[for='cpt_Primary']").css("color", "black");alert('3');
                    break;
                }else{
                    jQuery("#cpt_Primary").prop('required',true);alert("5");
                    jQuery("label[for='cpt_Primary']").text(jQuery("label[for='cpt_Primary']").text());alert("6");
                    jQuery("label[for='cpt_Primary']").css("color", "red");alert("7");
                }
            });
            return;
        }
    ...
    <div style="border: 1px solid black;overflow: none" id='icddiv'>
        ...
        <table id="justify" border="0" > <?php 
                ...
                while($setsql = sqlFetchArray($sql)){
                    ...
                    if(substr( str_replace(",","",$justify), 0, strlen($setsql['Codes']) )  === str_replace(':',"|",$setsql['Codes'])) 
                            echo " checked  ";
                    echo ">Primary</input>";
...
//    }
    ...
    <button id="save_btndown" class="btn btn-default fa fa-floppy-o" onClick="submitFunction()"> Save</button>
