...
//            }
        ...
        echo "<div id='cptdivcheck' name='cptdivcheck' role='form' class='form-horizontal'>";
            foreach($cptlist as $cptkey => $cptvalue){
                ...
                echo '<div class="col-sm-10 input-group btn-group">';
...
//                echo "</div>";
                echo "<span id= 'cpt".$cptkey1."span' style='display:none;' class='spanfrom-control'>No related Codes Mapped</span>";
                        ?>
                <button type="button" class="btn btn-primary" style='display:none' id="cpt<?php echo $cptkey1; ?>btnclear" onclick="cleardropdown('cpt_<?php echo $cptkey1; ?>')">Clear Dropdown</button></div></div>
                <script>
...
//            foreach($getcpts as $cpt_key => $cpt_value)
        ...
        $selectquery2 = sqlStatement("SELECT (select group_concat(justify) from billing WHERE encounter =   '$encounterid' and code_type='CPT4' and activity = 1) as justify, b.notecodes, b.code_text,f.rendering_provider as rendering_providerid, (SELECT  CONCAT( fname,  ' ', lname ) FROM users where id = f.provider_id)  AS rendering_ProviderName
                        FROM billing b 
                        INNER JOIN form_encounter f ON b.encounter = f.encounter  
                        WHERE b.encounter =   '$encounterid' and code_type='CPT4' and b.activity = 1 order by b.date asc ");
    ...
    <div class="row">
        <div class="col-md-12 form-horizontal">
            ...
            <div class="form-group">
                <div class="col-xs-6 col-sm-9 col-sm-push-1 input-group-sm">
                  <select id='icd_dropdown' name='icd_dropdown' class="form-control" aria-describedby="fromadion1">
                <option>       </option>
                  </select>
                </div>
                <div class="col-xs-6 col-sm-2 col-sm-push-1">
                    <button class="btn btn-default btn-sm" type="button" id="icdbtnclear" onclick="cleardropdown('icd')">Clear Dropdown</button>
                </div>
            </div>
        </div>
    </div>
    <div style="border: 1px solid black;overflow: none" id='icddiv'>
        ...
        <table id="justify" name="justify" border="0" > <?php 
                while($setsql = sqlFetchArray($sql)){
                    ...
                    echo ">Justify</input>";
                    echo "</td><td style='text-align:center;'>";
                    echo "<input type='checkbox' name='icd_mproblem[]' id='icd_checkboxes' value ='".$setsql['Codes']."'>Active</input>" ;
                    echo "</td><td><span>|";
...
//    }
    ...
    <button id="save_btndown" class="btn btn-default fa fa-floppy-o" onclick="my_form.submit()"> Save</button>
