...
<body class="body_top">
    ...
    echo "<form method='post' name='my_form' id='my_form' action='$rootdir/forms/auditform/save_custom.php?id=" . attr($formid) ."&isSingleView=$isSingleView&isFromCharts=$isFromCharts'>\n";
...
<!--                    <script>
    ...
    <?php
    echo "<br>";
//    $note=$_POST['form_audit_note'].".";
//
//    if($note!=''){
//        ECHO "UPDATE `form_encounter` SET `audit_note`='$note' WHERE encounter='$encounter' AND pid='$pid'";
...
//    }
    ...
    <div class="tabContainer">							
        <?php
        $getlbfformid = sqlStatement("SELECT form_id FROM forms WHERE pid = $pid AND encounter = $encounter and formdir='newpatient' and deleted = 0");
        $lbfformid = 0;
        while($setlbfformid = sqlFetchArray($getlbfformid)){
            $lbfformid = $setlbfformid['form_id'];
        }
        ?>
        ...
        $auditQ = sqlStatement("SELECT audit_data FROM tbl_form_audit fa, forms f 
                                WHERE fa.id = f.form_id AND f.encounter = ".$encounter." AND f.formdir = 'auditform' AND f.pid= $pid");
        ...
        $providerselect = '';
        $isprovider = sqlStatement("SELECT id FROM `users`  WHERE active=1 AND authorized=1 AND id = ".$_SESSION['authUserID']);
        ...
        $processaudit = sqlStatement("SELECT processaudit FROM tbl_user_custom_attr_1to1 WHERE userid= ".$_SESSION['authUserID']);
