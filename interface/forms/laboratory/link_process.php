...
$form_title = 'Processing Result Link';
$form_name = 'laboratory_link';
$form_id = $_REQUEST['id'];
...
function labPnote($pid, $newtext, $assigned_to = '', $datetime = '') {
	...
	$body = date('Y-m-d H:i') . ' (Processor to '. $assigned_to;
	$body .= ') ' . $newtext;
...
$result_data = new wmtOrder('laboratory',$form_id);
...
if ($process) { // doing the work
	...
	$query .= "WHERE o.list_id = 'Procedure_Site_Identifiers' AND o.title = '$result_data->facility_id' ";
	...
	addForm($encounter, "GENERATED RESULT ENCOUNTER",
		sqlInsert("INSERT INTO form_encounter SET " .
			...
			"reason = 'GENERATED ENCOUNTER FOR RESULTS', " .
	...
	$result_form = "Linked Results - ".$result_data->procedure_order_id;
	if ($result_data->specimen_num) $result_form .= " (".$result_data->specimen_num.")";
	addForm($encounter, $result_form, $result_data->id, "laboratory", $form_pid, 0, 'NOW()', $provider_username);

	// send them a message
	if ($provider_username) {
		$link_ref = "$rootdir/forms/laboratory/update.php?id=$result_data->id&pid=".$form_pid."&enc=".$encounter;
		$note = "\n\nLab results for order number '".$result_data->procedure_order_id."' have been manually linked to your patient '".$pat_data->fname." ".$pat_data->lname."' (pid: ".$form_pid."). ";
