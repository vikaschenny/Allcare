...
require_once("{$GLOBALS['srcdir']}/wmt/laboratory/ResultClient.php");
require_once("{$GLOBALS['srcdir']}/wmt/laboratory/ResultReport.php");
...
// GET LAB DATA
if ($LAB) {
	$lab_data = sqlQuery("SELECT * FROM procedure_providers WHERE (type = 'laboratory' OR type = 'radiology') AND ppid = ?",array($LAB));
...
$query .= "WHERE list_id = 'Lab_Category' LIMIT 1";
if ($dummy = sqlQuery($query)) $pc_cat = $dummy['option_id'];	
	
// VALIDATE INSTALL
$invalid = "";
if (!$lab_id) $invalid = "No Lab Processor Identifier Provided\n";
if (!$lab_data['protocol'] || $lab_data['protocol'] == 'INT') $invalid = "This processor has no batch protocol\n";
//if (!file_exists("{$GLOBALS["OE_SITE_DIR"]}/labs/".$lab_id)) $invalid .= "No Lab Work Directory\n";
//if (!file_exists("{$GLOBALS["OE_SITE_DIR"]}/labs/".$lab_id."/backups")) $invalid .= "No Lab Backup Directory\n";
if (!file_exists("{$GLOBALS["srcdir"]}/wmt")) $invalid .= "Missing WMT Library\n";
if (!file_exists("{$GLOBALS["srcdir"]}/wmt/laboratory")) $invalid .= "Missing WMT Lab Library\n";
...
if ($invalid) { ?>
<html><head></head><body>
<h1><?php echo $lab_name ?> Interface Not Available</h1>
...
function labPnote($pid, $newtext, $assigned_to = '', $datetime = '') {
	...
	$body = date('Y-m-d H:i') . ' ('. $lab_name .' to '. $notify .') '.	$newtext;
...
if ($FROM && !$THRU) { // default thru to today if not given
	$THRU = date('Y-m-d');
}
...
//DEBUG!!!	$messages = $client->getResults(25, $DEBUG);
	$messages = $client->getResults(25, true);
}
...
foreach ($messages as $message) {
	...
	if ($DEBUG) {	
		echo "<hr/>\n";
		$mtype = ($message->order_control == 'CA')? 'Cancel Order' : 'Results';
		echo "Processing $type for Patient: ".$message->name[0].", ".$message->name[1]." ".$message->name[2]."\n";
	...
	if ($message->pid) { // check pid
		$patient = sqlQuery("SELECT pid, DOB, providerID, lname, fname, mname FROM patient_data WHERE (pubpid = ? OR pid = ?) and DOB = ?", array($message->pid, $message->pid, $pat_DOB) );
		if ($patient['pid']) {
			$pid = $patient['pid'];
	...
	if (!$pid && $message->pubpid && $pat_DOB) { // maybe they used pubpid
		$patient = sqlQuery("SELECT pid, DOB, providerID, lname, fname, mname FROM patient_data WHERE (pubpid = ? OR pid = ?) and DOB = ?", array($message->pubpid, $message->pubpid, $pat_DOB) );
		if ($patient['pid']) {
			$pid = $patient['pid'];
	...
	if (!$pid && $message->extpid) { // maybe they used extpid
		$patient = sqlQuery("SELECT pid, DOB, providerID, lname, fname, mname FROM patient_data WHERE (pubpid = ? OR pid = ?) and DOB = ?", array($message->extpid, $message->extpid, $pat_DOB) );
		if ($patient['pid']) {
			$pid = $patient['pid'];
	...
	if (!$order_data && $message->order_number && $message->lab_number) {
		$order = sqlQuery("SELECT id FROM form_laboratory WHERE order_number = ? AND lab_number = ? AND lab_id = ? ",array($message->order_number, $message->lab_number, $lab_id));
		if ($order['id']) {
			$order_data = new wmtOrder($order_type,$order['id']);
	...
	if (!$order_data && $message->lab_number && $pat_DOB) {
		$order = sqlQuery("SELECT id FROM form_laboratory WHERE lab_number = ? AND pat_DOB = ? AND lab_id = ? ",array($message->lab_number, $pat_DOB, $lab_id));
		if ($order['id']) {
			$order_data = new wmtOrder($order_type,$order['id']);
	...
	if (!$pid) { // no match
		$pid = $DUMMY_ID; // if no valid patient use  dummy
	...
	// alternate patient field naming
	if (!$provider_id && $patient['provider_id']) {
		$provider = sqlQuery("SELECT * FROM users WHERE id = ?",array($patient['provider_id']));
		if ($provider) {
			$provider_id = $provider['id'];
		} 
	}
	...
	elseif (!$message->account) {
		$site = sqlQuery("SELECT * FROM list_options WHERE list_id = 'Lab_Site_Identifiers' AND title = ?",array($facility_id));
	...
	if (!$message->dob) {
		$errors = true;
		echo "FATAL ERROR: NO PATIENT BIRTHDATE PROVIDED \n";
	}
	if (!$message->name && !$message->pid) {
		$errors = true;
		echo "FATAL ERROR: NO PATIENT IDENTIFIER OR PATIENT NAME \n";
	}
	if ($message->order_control == 'CA' && !$matched) {
		// mark result as processed
		$client->ackResult($message, $DEBUG);
		
		$errors = true;
		echo "NOTICE: NO ORDER FOUND FOR CANCEL REQUEST ... IGNORING \n";
	}
	if ($errors) {
		echo "\n";
		continue;
	...
	* --------------------------------------------------------------------------- */
	...
	if (!$encounter && $pid != $DUMMY_ID) {
		...
		addForm($encounter, "RESULTS ENCOUNTER",
			sqlInsert("INSERT INTO form_encounter SET " .
				"date = '".date('Y-m-d H:i:s',strtotime($odate))."', " .
				"onset_date = '', " .
				"reason = 'GENERATED ENCOUNTER FOR RESULT', " .
	...
	// SPECIAL FOR CERNER (use specimen number as accession number)
	if ($lab_data['npi'] == 'CERNER' && !$message->lab_number) {
		// strip leading zeros for cerner
		$message->lab_number = ltrim($message->specimen_number, '0');
	}
	...
	if (!$matched) {
		...
		$order_data->request_account = $message->account;
	...
	if (count($message->reports) > 0) { // do we have anything to process?
		...
		foreach ($message->reports as $report) {
			...
			// CPL Bug Check
			if ($report->service_id[0] == '') continue;
			
			if ($DEBUG) {
				echo "<hr/>";
				$test = ($report->service_id[1])? $report->service_id[1] : $report->service_id[4];
				echo "Test Ordered: ".$report->service_id[0]." - ".$test."\n";
				if ($report->specimen_datetime)
			...
			if ($ordered[$report->service_id[0]]) {
				...
				$item_data->procedure_name = ($report->service_id[1])? $report->service_id[1] : $report->service_id[4];
			...
			if (count($report->results) > 0) { // do we have results for this order?
				foreach ($report->results as $result) {
					...
					// determine result code and text
					if (is_array($result->observation_id)) {
						$result_code = $result->observation_id[0];
						$result_text = $result->observation_id[1];
					} else {
						$result_code = $result->observation_id;
						$result_text = '';
					}
					...
					if ($DEBUG) {
						...
						echo "Observation Code: ".$result_code." \n";
						echo "Observation Text: ".$result_text." \n";
					...
					$result_data->result_code = $result_code;
					$result_data->result_text = $result_text;
...
//					$result_data->facility = $result->producer_id; // need unique lab producer facility if available
					$result_data->facility = ($labs[$result->producer_id]) ? $labs[$result->producer_id] : $result->producer_id; // use unique number
					
					$obvalue = $result->observation_value;
					if (is_array($obvalue)) {
						if ($obvalue[3] == "Base64") $images[] = $obvalue[4]; // save document
					...
					if ($lab_data['type'] == 'radiology') $result_data->result_status = 'Final';
	...
	if ($lab_data['npi'] == 'BBPL') {
		// use BBPL document
		$document = base64_decode($images[0]);
	}
	else {
		// generate observation results document
		$document = makeResultDocument($message, $lab_data);
	}
	...
	// save document id
	$doc_id = $d->get_id(); 
	$order_data->result_doc_id = $doc_id;
	...
	if ($category['id'] && $doc_id) 
		...
		die ("\n\nMISSING DOCUMENT CATEGORY FOR $lab_name OR DOCUMENT MISSING !!");

	// save the document identifier
	$doc_id = $order_data->result_doc_id;
	if ($DEBUG && $doc_id) 
	...
	if ($final_status) {
		...
		if ($final_status == 'c') $order_data->order_status = 'cancelled';
	}
	...
	/* --------------------------------------------------------------------------- *
 	 ...
 	 * --------------------------------------------------------------------------- */
	if ($patient) {
		$link_ref = "../../forms/laboratory/update.php?id=".$order_data->id."&pid=".$pid."&enc=".$encounter;
		...
		$note .= "To review these results click on the following link: ";
  		$note .= "<a href='". $link_ref ."' target='_blank' class='link_submit' onclick='top.restoreSession()'>".$lab_name." - ". $message->order_number ."</a>\n\n";
		labPnote($pid, $note, $provider_username);
	}
	else {
		$note = "Processor results received for an unknown patient. ";
		...
		$note .= "Please use the Orphan Results Report to assign these results to a valid patient.\n\n";
		labPnote($pid, $note, $provider_username);
	}
