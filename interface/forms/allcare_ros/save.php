...
function save_form_flag(){
    ...
    //save ros status in lbf_data
       ...
       // echo "select form_id  from forms where form_name ='Allcare Encounter Forms' AND encounter='".$GLOBALS['encounter']."' AND pid='".$GLOBALS['pid']."' AND deleted=0 order by id desc"; exit();
            ...
            ///echo $lb_value;
             if($lb_value!=''){
               // echo $lb_value; 
                ...
                $ins_form=sqlStatement("INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),'".$GLOBALS['encounter']."','Allcare Encounter Forms',$new_id1,'".$GLOBALS['pid']."','".$_SESSION["authUser"]."','default',1,0,'LBF2')");
