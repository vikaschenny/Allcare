...
// Table for Template From data
echo "<div id='template_details'>";
echo "<b>Audit Note Logs Table</b>";
if($formid)
    {
      $audit_log=sqlStatement("select  * from tbl_allcare_formflag where form_id='$formid' AND form_name = 'Patient Encounter'");
   ...
   echo  "<tr>
            <th style='width:20'>UserName</th>
           <th style='width:20'>Date</th>
           <th style='width:20'>Audit Note</th>
           <th style='width:20'>Action</th>
           <th style='width:20'>IP Address</th>
          </tr>";
   while($audit_rows=sqlFetchArray($audit_log))   
   {
         $arrays=unserialize($audit_rows['logdate']);
         $count = count($arrays);
        foreach ($arrays as $value2) {
          if($value2['audit_note']!=''){  ?>
            <tr>
                <td style="width:20"><?php echo $value2['authuser'];?></td>
                <td style="width:20"><?php echo $value2['audit_date']; ?></td>
                <td style="width:20"><?php echo $value2['audit_note']; ?></td>
                <td style="width:20"><?php echo $value2['action']; ?></td>
                <td style="width:20"><?php echo $value2['ip_address']; ?></td>
           </tr>
          <?php }
          } 
