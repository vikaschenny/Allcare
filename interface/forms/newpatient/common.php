	...
	$.ajax({
		...
		{  
                ...
                if( $('#template1').html(response)) {
                   ...
                   var answer = $("#template1").dialog({resizable: false,close: function() {$(".lightbox").fadeOut();}})
...
<script>
    ...
    j(document).ready(function(){
        j('#form_audited_status').change(
            function() {
                var val1 = j('#form_audited_status option:selected').val();
                if(val1 === 'Completed'){
                    j("#form_elec_signedby").removeAttr('disabled');
                    j("#form_elec_signed_on").removeAttr('disabled');
                }else{
                    j("#form_elec_signedby").attr('disabled','disabled'); 
                    j("#form_elec_signed_on").attr('disabled','disabled'); 
                }
            }
        );
    });
...
<?php
// template from access
$get_template_from_access = sqlStatement("SELECT process_templatefrom FROM tbl_user_custom_attr_1to1 WHERE userid= ".$_SESSION['authUserID']);
while($set_template_from_access = sqlFetchArray($get_template_from_access)){
    $template_from_access = $set_template_from_access['process_templatefrom'];
}
if($template_from_access == "YES"):
    ?>
        <script>
...
<table>
    ...
    <?php          
        ...
        // Handle a data category (group) change.
          ...
          if($field_id=='elec_signed_on'){
              $date_1=$viewmode ? substr($result['date'], 0, 10) : date('Y-m-d'); 
              if($currvalue!=''){
                  $date2=explode(" ",$currvalue);
                  $datetime1 = date_create("$date2[0]");
              ...
              echo "<td >&nbsp"; echo "Duration (in days): ".$interval->days ; echo "</td>";
          }
          echo "</div>";
        }
...
if($auditN != "" || $feeR != ""):
    ...
    <script>
        ...
        if($('#form_audited_status').val()==''){ $('#form_audited_status option[value="Incomplete"]').attr("selected", "selected"); )
...
?>
<a href="javascript:;" class="link" onclick="window.open( 'https://<?php echo $_SERVER[HTTP_HOST]; ?>/interface/forms/newpatient/details_page.php?id=<?php echo $id; ?>', '', 'width=500, height=600')"> Details </a>
</form>
