...
function updateENC_forms($id, $new, $create,$ecounter1,$pid1,$field_id)
{
  ...
  return $db_id;
    ...
    $fres = sqlStatement("SELECT * FROM layout_options " .
      "WHERE form_id = 'LBF2' AND uor > 0  AND group_name LIKE '%CCM%' AND field_id IN ('ccm_stat','ccm_review')" .
      "ORDER BY seq");
    $field_id1=array();
    ...
    foreach($field_id1 as $val){
       // echo "select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$formid' AND l.form_id='LBF2' AND l.group_name LIKE '%CPO%' AND lb.field_id LIKE '%$val%' order by seq"; 
        $res1=sqlstatement("select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$formid' AND l.form_id='LBF2' AND l.group_name LIKE '%"
                . "%CCM%' AND lb.field_id LIKE '%$val%' order by seq");
