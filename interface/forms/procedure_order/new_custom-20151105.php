...
echo "select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$form_id' AND l.form_id='LBF2' AND l.group_name LIKE '%Procedure%' AND lb.field_id LIKE 'procedure_stat' order by seq";
 $res1=sqlstatement("select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$form_id' AND l.form_id='LBF2' AND l.group_name LIKE '%Procedure%' AND lb.field_id LIKE 'procedure_stat' order by seq");
 $res_row1=sqlFetchArray($res1);
 if(!empty($res_row1)){
     updateENC_forms($form_id, $newdata['lbf_data'] ,$create=false,$encounter,$pid);
 }else{
      updateENC_forms($form_id, $newdata['lbf_data'] ,$create=true,$encounter,$pid);
 }
 }
...
<form method="post" action="new_custom.php?id=<?php echo $formid ?>&encounter=<?php echo $encounter; ?>"
