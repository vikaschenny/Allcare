...
if($_REQUEST['mode']=='add'){
    ...
    function updateENC_forms($form_id, $new, $create,$ecounter1,$pid1,$field_id)
   {
        global $form_id;
        $db_id = 0;
        if ($create) {
    ...
    foreach($field_id1 as $val){
        //echo "select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$form_id' AND l.form_id='LBF2' AND l.group_name LIKE '%Procedure%' AND lb.field_id LIKE 'procedure_stat' order by seq";
        ...
        if(!empty($res_row1)){
            updateENC_forms($form_id, $newdata['lbf_data'][$val] ,$create=false,$encounter,$pid,$val);
        }else{
             updateENC_forms($form_id, $newdata['lbf_data'][$val] ,$create=true,$encounter,$pid,$val);
        }
    }
}
  ...
  if ($_POST['bn_xmit']) {
    ...
    function formJump_custom($provider,$encounter,$pid,$isSingleView,$isFromCharts,$formid,$form_id){
        if($isSingleView == 1 && $isFromCharts == 0)
            echo "<script> window.close();window.opener.location.href = '../../reports/single_view_form.php?encounter=$encounter&pid=$pid';</script>";
            ...
            echo "\n<script language='Javascript'> window.close(); window.opener.location.href = '../../reports/incomplete_charts.php?encounter=$encounter';</script>\n";
    }
 ...
 ********************************************************************/
 ...
 window.open('../../orders/find_order_popup.php' +
  '?labid=' + f.form_lab_id.value +
  '&order=' + f[ptvarname].value +
  '&formid=<?php echo $formid; ?>' +
  '&formseq=' + formseq,
...
function sel_related(varname) {
 ...
 window.open('../../patient_file/encounter/find_code_popup.php?codetype=<?php echo attr(collect_codetypes("diagnosis","csv")) ?>', '_blank', 500, 400);
}
...
<form method="post" action="new_custom.php?id=<?php echo $formid ?>&encounter=<?php echo $encounter; ?>&isSingleView=<?php echo $isSingleView; ?>&isFromCharts=<?php echo $isFromCharts; ?>"
...
//  $row['provider_id']);
 $query = "SELECT id, lname, fname FROM users WHERE ".
                                    ...
                                    while ($urow = sqlFetchArray($ures)) {
                                            ...
                                            if ($provid == $row['provider_id']) echo " selected"; }
