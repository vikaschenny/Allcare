...
class C_FormVitals extends Controller {
    ...
    function default_action($form_id,$pending,$finalized) {

        if (is_numeric($form_id)) {
    		$vitals = new FormVitals($form_id,$pending,$finalized);
    	}
    	else {
    		$vitals = new FormVitals();
    	}
    ...
    function default_action_process() {
		...
		if(empty($_POST['id']))
		{
			...
			$_POST['process'] = "";
                         $sql1=sqlStatement("select form_id  from forms where form_name ='Vitals' AND encounter='".$GLOBALS['encounter']."' AND pid='".$GLOBALS['pid']."' AND deleted=0 order by id desc");
                        $row1=sqlFetchArray($sql1);
                        if($row1['form_id']!=''){
                            $update=sqlStatement("UPDATE form_vitals SET  `finalized` = ?  ,  pending= ? WHERE pid=? AND id=?",array($_POST['finalized'][0],$_POST['pending'][0],$GLOBALS['pid'],$row1['form_id']));
                        }
		}else {
                    $update=sqlStatement("UPDATE form_vitals SET  `finalized` = ?  ,  pending= ? WHERE pid=? AND id=?",array($_POST['finalized'][0],$_POST['pending'][0],$GLOBALS['pid'],$_POST['id']));
                }
		return;
    }
