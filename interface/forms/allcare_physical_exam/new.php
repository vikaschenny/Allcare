...
if ($_POST['bn_save']) {
 ...
 if ($formid) {
  ...
  $formid = addForm($encounter, "Allcare Physical Exam", 0, "allcare_physical_exam", $pid, $userauthorized);
 ...
 foreach ($form_obs as $line_id => $line_array) {
  ...
  echo $comments  = $line_array['comments']  ? $line_array['comments'] : '';
 ...
 if($finalized || $pending){
        ...
        if($finalized=='Y' && $pending=='Y'){
         ...
         $res12=sqlstatement("select form_id  from forms where form_name ='Allcare Encounter Forms' AND encounter='".$GLOBALS['encounter']."' AND pid='$pid' AND deleted=0 order by id desc");
         ...
         if(!empty($frow_res)){
             ...
             if($lb_value!=''){
                ...
                $ins_form=sqlStatement("INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),'".$GLOBALS['encounter']."','Allcare Encounter Forms',$new_id1,$pid,'".$_SESSION['authUser']."','default',1,0,'LBF2')");
...
<script language="JavaScript">
 ...
 function seldiag(selobj, line_id) {
  ...
  if (opt.value == '*') {
   ...
   dlgopen('../../forms/allcare_physical_exam/edit_diagnoses.php?lineid=' + line_id, '_blank', 500, 400);
  }
 }
 
 function details(formid){
      dlgopen('../../forms/allcare_physical_exam/details.php?formid=' + formid, '_blank', 500, 400);
    }
...
<body class="body_top">
<form method="post" action="<?php echo $rootdir ?>/forms/allcare_physical_exam/new.php?id=<?php echo $formid ?>"
