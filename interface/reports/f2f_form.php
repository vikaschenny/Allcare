...
if($_REQUEST['delete_id']!=''){
    $sql=sqlStatement("delete from lbf_data where field_id='f2f_stat' and form_id='".$_REQUEST['delete_id']."'");
    echo "<script>window.location.href = 'f2f_encounters_report.php';</script>";
}
if ($mode && $_REQUEST['delete_id']=='') {   
...
function closeme() {
document.getElementById("delete_id").value = "<?php echo $form_id; ?>";
 //window.location.href = 'f2f_encounters_report.php'; 
 var f = document.forms['new_f2f'];
 //if (validate(f)) {
...
<form name='new_f2f' method='post' action='f2f_form.php?coid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
