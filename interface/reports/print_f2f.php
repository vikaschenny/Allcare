...
$fval = sqlStatement("SELECT l.field_id, lb.field_value
                ...
                INNER JOIN tbl_form_chartoutput_transactions ff ON ff.date_of_service=fe.date
                WHERE l.form_id =  'LBF2' AND f.deleted=0 and ff.transaction=2
                AND uor >0
                AND l.field_id !=  ''
...
/* foreach($code1 as $ssr)
 ...
 /*$care=sqlStatement("select notes from tbl_form_chartoutput_transactions where pid='".$GLOBALS['pid']."' AND id='".$id."' AND encounter='".$encounter_id."' AND date_of_service='".$date_of_service."'");
...
//printed name and sign of physician
    ...
    //f2f report   datetimestamp
         $report_ts=sqlStatement("select created_date from tbl_form_chartoutput_transactions where date_of_service='".$date_of_service." and transaction=2 and pid=$patient_id and id=$id'");
    ...
    {    
         ...
         foreach ($f2f as $key => $value) {
             if($key =='f2f_med')
             {
                 $med_val=explode("|",$value);
                 foreach($med_val as $val){
                     $sql=sqlStatement("select * from list_options where list_id='medical_problem_issue_list' and option_id='$val' ");
                     $med_row=sqlFetchArray($sql);
                     //$med_cond= str_replace( "|" , " , " ,$value);
                      $med_cond.= $med_row['title']." , " ;
                 }
                 
                $med_con1=trim($med_cond,",");
                 $s = str_replace("{".$key."}",$med_con1, $s);
...
$sql7=sqlStatement("select * from tbl_form_chartoutput_transactions where encounter=$encounter_id and pid=$patient_id and date_of_service='$date_of_service' and id=$id");
