...
<body class="body_top">
    <br>
<div id="pname">
    <?php  //$getPid=sqlStatement("SELECT * from forms where form_id='".$form_id."' AND form_name='Allcare Encounter Forms' AND formdir='LBF2'");
   
            //$resPid=sqlFetchArray($getPid);?>
    <span ><?php echo htmlspecialchars( xl('Patient Name'), ENT_NOQUOTES); ?>:</span>
        <?php 
            $getPatientName=sqlStatement("SELECT CONCAT(fname,' ',lname) AS pname FROM patient_data WHERE pid='".$patient_id."'");
...
/*if ($result = getF2FReportsByPid($pid,$encounter)) {	*/
        if ($result = getF2FByPid($patient_id)) {  
	foreach ($result as $iter) {
		...
		print "<a href='f2f_form.php?f2fid=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
			"&inmode=edit". "&encounter_id=".$iter['encounter']."&pid=".$iter['pid'].""."&form_id=".$iter['form_id']."' onclick='top.restoreSession()' class='css_button_small'><span>".
