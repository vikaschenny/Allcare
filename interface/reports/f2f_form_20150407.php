...
{  
  ...
  if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='add_f2f.php?form_id=$form_id&pid=$patient_id';";
  else
    $body_onload_code = "javascript:parent.Layout-Based Visit Forms.location.href='add_f2f.php?form_id=$patient_id';";
}
...
function imdeleted() {
 top.restoreSession();
 location.href = 'summary/add_f2f.php';
}
...
<form name='new_f2f' method='post' action='f2f_form.php?f2fid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>&form_id=<?php echo htmlspecialchars( $form_id, ENT_QUOTES); ?>&pid=<?php echo htmlspecialchars( $patient_id, ENT_QUOTES); ?>&enc=<?php echo htmlspecialchars( $encounter, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
...
<input type="hidden" name="pid" value="<?php echo $_REQUEST['pid']; ?>" >
   ...
   <?php if($ab1='add' && $inmode2!=create && $inmode!='edit') { ?>
                        ...
                        <span class='bold'><?php //echo htmlspecialchars( xl($_GLOBALS['encounter']), ENT_NOQUOTES); ?></span>-->

   <?php } else if($inmode2='create' && $inmode!='edit') { ?>
                        <span><?php echo htmlspecialchars( xl('Patient Name'), ENT_NOQUOTES); ?>:</span>
                        <span class='bold'><?php echo htmlspecialchars( xl($pname), ENT_NOQUOTES); ?></span><br>


                         <span><?php echo htmlspecialchars( xl('Encounter Id'), ENT_NOQUOTES); ?>:</span>
                        <span class='bold'><?php echo htmlspecialchars( xl($encounter), ENT_NOQUOTES); ?></span>
    <?php } else if($inmode='edit' && $inmode2!='create'){?>
                        <span><?php echo htmlspecialchars( xl('Patient Name'), ENT_NOQUOTES); ?>:</span>
                            <?php 
    ...
    <span class='bold'><?php echo htmlspecialchars( xl($resPatientName['pname1']), ENT_NOQUOTES); ?></span><br>
     ...
     <span><?php echo htmlspecialchars( xl('Encounter Id'), ENT_NOQUOTES); ?>:</span>
    <span class='bold'><?php echo htmlspecialchars( xl($GLOBALS['encounter']), ENT_NOQUOTES); ?></span>
   <?php }
?>
 ...
 if($inmode2=='create')
{
    ...
    $sel=sqlStatement("select * from lbf_data where form_id='".$form_id."' AND field_id='f2f_stat'");
    $row2=sqlFetchArray($sel);
    //print_r($row2);
      if($row2!='')
      {
          while($row2=sqlFetchArray($sel))
          {
              print_r($row2);
            if($row2['field_id']=='f2f_stat')
            {
        ...
        $res2=sqlStatement("INSERT INTO lbf_data (form_id,field_id,field_value) VALUES ($form_id,'f2f_stat','finalized')");
        $rows2=sqlFetchArray($res2);
    ...
    $res1=sqlStatement("SELECT DISTINCT(fe.date)as date,DATE_FORMAT(fe.date,'%m/%d/%Y') as fdate
                ...
                AND f.pid ='$patient_id' AND lb.form_id='".$form_id."'");
