...
$sqlGetPatients="SELECT DISTINCT pd.title,pd.fname,pd.lname,pd.mname,pd.deceased_date,pd.deceased_reason,pd.deceased_stat,pd.practice_status,pd.patient_facility,pd.cpo,pd.ccm,pd.last_visit,pd.cert_end,
     pd.hhs_days,pd.calc_next_visit,pd.refill_due,pd.insuranceID,pd.language,pd.ethnicity,pd.race,pd.financial_review,pd.family_size,pd.monthly_income,pd.Statement_cat,pd.homeless,
     ...
     pd.vfc,pd.Insured_Type,pd.annual_visit,pd.authorization,pd.Elig_Note,pd.Copay,pd.Deduct_Fam_Ann,pd.Deduct_Indiv_Ann,pd.indoutofpocket,pd.familyoutofpocket,
     pd.PCP,pd.kareo,
    ...
    pd.DOB,pd.street,pd.street_addr,pd.suite,pd.postal_code,pd.city,pd.state,pd.country_code,
    ...
    opc.pc_comments,opc.pc_counter,opc.pc_topic,opc.pc_informant,opc.pc_eid
