...
.fac_admit{ width: 9%; }
.fac_admit1{ width: 9%; }
.fac_discharge,.fac_discharge1 { width: 9%; }
.fac_isactive,.fac_isactive1 { width: 7%; }
.fac_notes,.fac_notes1 { width: 9%; }
.fac_links,.fac_links1 { width: 9%; }
...
<form method='post' action='facility_select.php' name='theform' onsubmit='return top.restoreSession()'>
...
$sqllimit = $MAXSHOW;
$given = "*";
$orderby = "facility_admitdate ASC, facility_dischargedate ASC";
...
if ($popup) {
  ...
  while ($frow = sqlFetchArray($fres)) {
    ...
    if (!empty($_REQUEST[$field_id])) {
     $value = trim($_REQUEST[$field_id]);
      if ($field_id == 'facility_admitdate') {
      echo  $where .= " AND $field_id LIKE ?";
        array_push($sqlBindArray,$value);
      }
      else if ($field_id == 'facility_dischargedate') {
 ...
 $sql = "SELECT $given FROM tbl_patientfacility " .
...
<th class="facName"><?php echo htmlspecialchars( xl('Facility name'), ENT_NOQUOTES);?></th>
<th class="fac_admit"><?php echo htmlspecialchars( xl('Admit Date'), ENT_NOQUOTES);?></th>
<th class="fac_discharge"><?php echo htmlspecialchars( xl('Discharge Date'), ENT_NOQUOTES);?></th>
<th class="fac_isactive"><?php echo htmlspecialchars( xl('Is Active'), ENT_NOQUOTES);?></th>
<th class="fac_notes"><?php echo htmlspecialchars( xl('Notes'), ENT_NOQUOTES);?></th>
<th class="fac_links"><?php echo htmlspecialchars( xl('Related Links'), ENT_NOQUOTES);?></th>
</tr>
</table>
</div>
...
if ($result) {
    foreach ($result as $iter) {
        ...
        echo  "<td class='facName1'>" . htmlspecialchars($iter['facilityid'] ) . "</td>\n";
      
        echo "<td class='fac_admit1'>" .
	    htmlspecialchars( $iter['facility_admitdate'], ENT_NOQUOTES) . "</td>\n";
        
        echo "<td class='fac_discharge1'>" . htmlspecialchars( $iter['facility_dischargedate'], ENT_NOQUOTES) . "</td>";
        
            echo "<td class='fac_isactive1'>" . htmlspecialchars( $iter['facility_isactive'], ENT_NOQUOTES) . "</td>";
        
           
        
        echo "<td class='fac_notes1'>" . htmlspecialchars( $iter['facility_notes'], ENT_NOQUOTES) . "</td>"; 
        echo "<td class='fac_links1'>" . htmlspecialchars( $iter['facility_related_links'], ENT_NOQUOTES) . "</td>"; 
        echo "</tr>";
    }
}
?>
