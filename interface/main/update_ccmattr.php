...
$get_defaulthhsdays = sqlStatement("SELECT title FROM list_options WHERE list_id= 'allcareConfig' AND option_id='defaultccmdays'");
while($setdays = sqlFetchArray($get_defaulthhsdays)){
    $hhsdays = $setdays['title'];
}


if($ccmvisit_list == "") $ccmvisit_list = '15, 16, 17, 18, 19, 20, 24, 25, 29, 44';
...
$externalClause = " AND p.deceased_stat IN(".$selectIsDeceased.")
                AND p.ccm  IN(".$selectIsActive.")";

if(isset($_POST['selectCalc']) && !empty($_POST['selectCalc'])):
    if($_POST['selectCalc'] == 6):
        $i=0;
        $log = "";
        $log = "Log recorded For CCM category on ". date('Y-m-d H:i:s') . "\nUpdate done by ".$_SESSION['authUser']. " from ipaddress: ".getRealIpAddr()."\n";
        $log .= "CCM Last Visit Dates Updated for below list of patients\n";
        $lastvisitQuery = sqlStatement("SELECT p.pid, DATE_FORMAT( MAX( fe.date ) ,  '%Y-%m-%d %H:%i:%s' ) AS last_visit
                                        ...
                                        WHERE fe.pc_catid
                                        IN ( $ccmvisit_list ) $externalClause
                                        GROUP BY p.pid
                                        ORDER BY p.pid");
        while($row1=sqlFetchArray($lastvisitQuery)):
            sqlStatement("UPDATE patient_data SET ccm_last_visit = '".$row1['last_visit']."' WHERE pid=".$row1['pid']);
            $log .= "CCM Last visit date = ". $row1['last_visit'] . " for patient id = ". $row1['pid']."\n";
            $i++;
        endwhile;
        $log .= "Total CCM Last visit dates updated =". $i ."\n\n\n";
        file_put_contents('../../../../logs/schedulingCalcualtion'.date('Y-m-d').'.log', $log, FILE_APPEND);
        echo "CCM Last Visit Dates Updated<br />";
    elseif($_POST['selectCalc'] == 18): 
        $patientid = array();
        $lastvisitQuery = sqlStatement("SELECT lfd.field_id, lfd.field_value, f.pid, pd.ccm_dur_manual
        ...
        $log = "";
        $log = "Log recorded For CCM category on ". date('Y-m-d H:i:s') . "\nUpdate done by ".$_SESSION['authUser']. " from ipaddress: ".getRealIpAddr()."\n";
        $log .= "CCM Next Home Visit Days Updated for below list of patients\n";
        while($row3=sqlFetchArray($lastvisitQuery)):
            if($row3['ccm_dur_manual'] != 'YES'):
                sqlStatement("UPDATE patient_data SET ccm_hhs_days = '".$row3['field_value']."' WHERE pid=".$row3['pid']);
        ...
        $p = implode(",",$patientid);
        if($p != ""):
            sqlStatement("UPDATE patient_data SET ccm_hhs_days=".$hhsdays." WHERE pid NOT IN (".$p.") AND ccm_dur_manual!='YES'");
        else:
            sqlStatement("UPDATE patient_data SET ccm_hhs_days=".$hhsdays." WHERE ccm_dur_manual!='YES'");
        endif;
        
        $log .= "UPDATE patient_data SET hhs_days=".$hhsdays." WHERE pid NOT IN (".$p.") AND dur_manual!='YES'";
        $log .= "Total CCM Next Home Visit Days updated =". $i ."\n\n\n";
        file_put_contents('../../../../logs/schedulingCalcualtion'.date('Y-m-d').'.log', $log, FILE_APPEND);
        echo "CCM Next Visit Days Updated<br />"; 
    elseif($_POST['selectCalc'] == 19):
        $i = 0;
        $log = "";
        $log = "Log recorded For CCM category on ". date('Y-m-d H:i:s') . "\nUpdate done by ".$_SESSION['authUser']. " from ipaddress: ".getRealIpAddr()."\n";
        $log .= "CCM Calculated Next Visit Dates Updated for below list of patients\n";
        $calculatedNextQuery = sqlStatement("SELECT DISTINCT p.pid, 
                                        ...
                                        AND p.pid = fe.pid $externalClause
                                        ORDER BY fe.date DESC 
                                        LIMIT 1 ) ,  '0000-00-00 00:00:00' ) , INTERVAL ( p.ccm_hhs_days                                    
                                        ) 
                                        DAY ) ,  '%Y-%m-%d %H:%i:%s'
                                        )
        ...
        while($row4=sqlFetchArray($calculatedNextQuery)):
            sqlStatement("UPDATE patient_data SET calc_next_ccm = '".$row4['calculated_next_visit']."' WHERE pid=".$row4['pid']);
            $log .= "CCM Calculated Next Visit Dates = ". $row4['calculated_next_visit'] . " for patient id = ". $row4['pid']."\n";
            $i++;
        endwhile;
        
        $datesQuery = sqlStatement("SELECT pid,calc_next_ccm FROM patient_data");
        while($row6 = sqlFetchArray($datesQuery)):
            if($row6['calc_next_ccm'] == '-0001-11-30 00:00:00'):
                sqlStatement("UPDATE patient_data SET calc_next_ccm = '0000-00-00 00:00:00' WHERE pid=".$row6['pid']);
                $log .= "CCM Calculated Next Visit Dates = '0000-00-00 00:00:00' for patient id = ". $row6['pid']."\n";
                $i++;
            endif;
        endwhile;
        $log .= "Total CCM Calculated Next Visit Dates Updated =". $i ."\n\n\n";
        file_put_contents('../../../../logs/schedulingCalcualtion'.date('Y-m-d').'.log', $log, FILE_APPEND);
        echo "CCM Calculated Next Visit Dates Updated";     
    endif;
endif;
