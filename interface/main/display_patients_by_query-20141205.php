...
$getOpenAppQuery = sqlStatement("SELECT pc_pid FROM openemr_postcalendar_events WHERE pc_eventDate > CURDATE() -1 OR pc_endDate > CURDATE() -1");
$openAppId = array();
if(sqlNumRows($getOpenAppQuery)>0)
{
    ...
    if($openAppIdStr != ""){
        $openAppIdStrInClause = " AND lpid NOT IN (".$openAppIdStr.")";
    }    
}
...
<script type='text/javascript'>
        ...
        columnDefs: [
            { visible: false, targets: [4,5,7,8,9,10,11,15,16,22] }
        ]
        } );
...
//            }  
        ...
        $getTemp = $getTemp. $openAppIdStrInClause;
        ...
        while($rowTemp=sqlFetchArray($showTemp))
        {
            ...
            echo "<td title='".$fname." ".$lname."'>".$fname." ".$lname." <br /><a class=\"css_button_small\" href=\"javascript:;\" onclick=\"window.open('calendar/add_edit_event.php?patientid=".$rowTemp['lpid']."','AppointmentWindow','width=650, height=320')\"><span>Add</span></a></td>";	           
        ...
        while($rowTemp2=sqlFetchArray($showTemp2))
        {
            ...
            echo "<input type='hidden' id='hdnDetailsBack_$j' name='hdnDetailsBack' 
                  ...
                  value=\"".$street.$city .$state." ".$postal_code.$country_code."\" />";
            $j++;
