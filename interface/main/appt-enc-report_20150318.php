...
//alert('ADDRS_D = '+jQuery("#hdnDetails_"+i).val());
            ...
            geocoder.geocode({ 'address': address_patient }, function (results, status) {
                if (status === google.maps.GeocoderStatus.OK) 
                {
                    var latAndLong = String(results[0].geometry.location);
                    var latAndLongResult = latAndLong.substring(1,latAndLong.length-1);
                    $.ajax({
                        type: 'get',
                        cache: false,
                        url: "http://maps.googleapis.com/maps/api/geocode/json?latlng="+latAndLongResult+"&sensor=false",
                        success: function(response) {
                           // alert(JSON.stringify(response));
                            ...
                            if(correctAddress != address_patient){
