...
$selection = "select scan_medical_charts from tbl_drivesync_authentication where email='$rs->notes'";
$sel_rows = $sqlconfCentralDB->prepare($selection);
$sel_rows->execute();
$es = $sel_rows->fetchObject();
$idvalue = str_replace('https://drive.google.com/drive/folders/', '', $es->scan_medical_charts);
$folderid = $idvalue; 
...
if(is_array($all_folders)){
    ...
    foreach($tktid as $val1){
       ...
       // $body=$val1; 
        ...
        $stmt1->bindParam(':obj_type', $category);   
        ...
        $enc='';
        $logstatus="scan medical charts message created";
        $wats='';
        $file_name=''; $ins_id12='';
