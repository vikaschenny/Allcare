...
$selection = "select scan_medical_payment from tbl_drivesync_authentication where email='$rs->notes'";
$sel_rows = $sqlconfCentralDB->prepare($selection);
$sel_rows->execute();
$es = $sel_rows->fetchObject();
$idvalue = str_replace('https://drive.google.com/drive/folders/', '', $es->scan_medical_payment);
$folderid = $idvalue; 
...
if(is_array($all_folders)){
    ...
    foreach($tktid as $val1){
       ...
       // $body=$val1; 
        ...
        $enc='';
        $logstatus="scan medical payment message created";
        $wats='';
        $file_name=''; $ins_id12='';
