...
$selection = "select scan_medical_payer from tbl_drivesync_authentication where email='$rs->notes'";
$sel_rows = $sqlconfCentralDB->prepare($selection);
$sel_rows->execute();
$es = $sel_rows->fetchObject();
$idvalue = str_replace('https://drive.google.com/drive/folders/', '', $es->scan_medical_payer);
$folderid = $idvalue; 
    ...
    foreach($tktid as $val1){
       ...
       // $body=$val1; 
		...
		/*********send email notification******/
            ...
            $today = date("Y-m-d"); 
            $enc='';
            $logstatus="scan medical payer message created";
            $wats='';
            $file_name=''; $ins_id12='';
