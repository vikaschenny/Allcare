...
//                                                  'visit_cat'=>'Visit_Category',
                                ...
                                foreach($fields as $key1 => $value1){
                                   if($key1=='patient'){
                                     ...
                                     }else if($key1!='charges' && $key1!='copays' && $key1!='billed' &&  $key1!=='error' && $key1!=='duration') {
                                          $fieldset2 .= $key1.",";
                                     }
  ...
  if($selected_fields[0]!='-1' && !empty($_POST['selectEncColmsData']))  {
    // echo "<pre>"; print_r($selected_fields); echo "</pre>";
      foreach($selected_fields as $key=> $value){
          ...
          } else if($value!='billed' &&  $value!='error' && $value!='duration'){
...
//    }
  ...
  if (!empty($form_facility)) {
   ...
   if($fac2!=''){
    $query .= "AND e.pc_facility IN ($fac2) ";
   }
  }
  ...
  if (!empty($form_facility)) {
   ...
   if($patientVisitType!='' && $currentVisitCategories!=0){
      $query .= "AND fe.pc_catid  IN ($currentVisitCategories ) ";
  }
...
//            echo "<pre>"; print_r($row['Practitioner']); echo "</pre>";
		...
		foreach($row as $key => $value) {
                ...
                if(!empty($selected_fields)){
                    ...
                    if(in_array('duration',$selected_fields)){
                         print("<th>Duration (in days) </th>");
                    }
                }else{
                     print("<th>Billed</th>");
                     print("<th>Error</th>");
       ...
       print("<tr align=left valign=top>");
		foreach($row as $key => $value) {
                ...
                $sql_d=sqlStatement("select * from form_encounter where encounter='".$row['Encounter']."' and pid= '".$row['Chart']."'");
                $dat=sqlFetchArray($sql_d);
                  
                if($dat['elec_signed_on']!=''){
                    $date2=''; $date_1='';
                    $date2=explode(" ",$dat['elec_signed_on']);
                    $date_1=explode(" ",$dat['date']);
                    $datetime1 = date_create("$date2[0]");
                    $datetime2 = date_create("$date_1[0]");
