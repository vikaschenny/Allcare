...
<html>
	...
	<link rel="stylesheet" href="../../patient_file/summary/driveassets/drive.css">
        <link rel="stylesheet" href="css/linkfoulder.css">
	...
	<script src="../../patient_file/summary/driveassets/js/jquery.webui-popover.min.js"></script>
        ...
        <script>
           ...
           $(function(){
               ...
               $('a.entrysetting').webuiPopover('destroy').webuiPopover({trigger:'click',padding:0});
              ...
              function loaddrive(rootfoulder,folderid,category,userid){
                ...
                var gerfilesurl = '<?php echo $foulderdata?>'+"/"+folderid+"/folders";
                ...
                ajaxcall(folderinfourl,null,"get",getfileinfo,ajaxdefaulterrorcallback);
                 ...
                 function getfiles(data){
                    ...
                    $.each(files,function(index,value){
                      ...
                      function getlinkedfolders(){
                        ...
                        html += '<div class="entry '+getfiletype(value['mimeType'])+' '+linkstates.linkedclass+'" data-id="'+value['id']+'" data-name="'+value['name']+'">\n\
                         ...
                         if(getfiletype(value['mimeType']) != "parentfolder" && getfiletype(value['mimeType']) != "newfolder"){
                             ...
                             // new code for not visible for rename opt
                             if(folderopt.length !=1)
                                folderopt.shift();
                       ...
                       html +='<div class="entry_name" title="'+value['name']+'">\n\
                                 <a class="entry_link">\n\
                                     <div class="entry-name-view">\n\
                                         <span>\n\
                                             <input class="entrycheck" type="checkbox" id="'+value['id']+'"/>&nbsp;&nbsp;<label for="'+value['id']+'">'+value['name']+'</label>\n\
                                         </span>\n\
                                     </div>\n\
                                 </a>\n\
                              </div>';   
                     ...
                     $('.newfolder').find(".entrycheck").remove();
                     $('.parentfolder').find(".entrycheck").remove();
                     /* toll tip*/
                     var show = false;
                    $("#checkstate").html('<span class="glyphicon glyphicon-check" aria-hidden="true"></span>Select all folders')
                    $("#checkstate").data("checkstate","selectall");
            ...
            function clearajaxcalls(callslist){
             ...
             $('#rename,#delete,#linkfolder').on("show.bs.modal",function(event){
                 ...
                 }else if(modal.attr("name") == "delete")
                 {
                    ...
                    if(target.data("linktype") == "multi"){
                        modal.find('#link').data("linktype","multi")
                       if(target.data('linkstate').split(" ")[0] == "Link"){
                            if($('.entry:not(.isselected)').find(".entrycheck:checked").length !=0){
                                modal.find('.modal-header').html("<h4>Link Folders</h4>");
                                modal.find('.modal-body').html("Do You want to Link selected folders?");
                            }else{
                               event.preventDefault();
                               event.stopPropagation();
                               alert("Plese Select folder for link.");
                            }
                           
                       }else{
                            if($('.entry.isselected').find(".entrycheck:checked").length !=0){
                                modal.find('.modal-header').html("<h4>Unlink Folders</h4>");
                                modal.find('.modal-body').html("Do You want to unlink selected folders?");
                            }else{
                                event.preventDefault();
                                event.stopPropagation();
                                alert("Plese Select linked folder for unlink.");
                            }
                       }
                       
                    }else{
                       modal.find('#link').data("linktype","single")
                       target.data('linkstate').split(" ")[0] == "Link"?modal.find('.modal-header').html("<h4>Link Folder</h4>"):modal.find('.modal-header').html("<h4>Unlink Folder</h4>")
                       target.data('linkstate').split(" ")[0] == "Link"?modal.find('.modal-body').html("Do You want to Link this folder?"):modal.find('.modal-body').html("Do You want to unlink this folder?")
                   }
             ...
             $('#link').on("click",function(){
                 ...
                 if($(this).data("linktype") == "multi"){
                     if($(this).hasClass("Link")){
                         $('.entry:not(.isselected)').find(".entrycheck:checked").each(function(){
                              targetsfolders.push($(this).parents(".entry").data("id"));
                         });
                     }else{
                         $('.entry.isselected').find(".entrycheck:checked").each(function(){
                              targetsfolders.push($(this).parents(".entry").data("id"));
                         });
                     }
                     
                 }else{
                     targetsfolders.push(targetfid);
                 }
                 
                 ajaxcall(url,{category:category,userid:userid,action:'save',status:status,folderid:targetsfolders},"post",function(data){
                      loaddrive(rootfoulder,progressid,category,userid);
                 },ajaxdefaulterrorcallback);
             ...
             $(document).on("click",".entrycheck,.entrycheck ~ label",function(evt){
                 evt.stopPropagation();
                 
                 if($('.entry').find(".entrycheck:checked").length == $('.entry').find(".entrycheck").length){
                    $("#checkstate").data("checkstate","dselectall");
                    $("#checkstate").html('<span class="glyphicon glyphicon-unchecked" aria-hidden="true"></span>Deselect all folders')
                }else if($('.entry').find(".entrycheck:checked").length != $('.entry').find(".entrycheck").length){
                    $("#checkstate").html('<span class="glyphicon glyphicon-check" aria-hidden="true"></span>Select all folders')
                    $("#checkstate").data("checkstate","selectall");
                }
             })
             $(document).on("click","#checkstate",function(evt){
                 evt.preventDefault();
                if($(this).data("checkstate") == "selectall"){
                    $('.entry').find(".entrycheck").prop("checked",true);
                    $(this).data("checkstate","dselectall");
                    $(this).html('<span class="glyphicon glyphicon-unchecked" aria-hidden="true"></span>Deselect all folders')
                }else if($(this).data("checkstate") == "dselectall"){
                    $('.entry').find(".entrycheck").prop("checked",false);
                    $(this).html('<span class="glyphicon glyphicon-check" aria-hidden="true"></span>Select all folders')
                    $(this).data("checkstate","selectall");
                }
        ...
        <style>.modal{z-index: 100000;} .modal-backdrop{z-index: 99999;}</style>
     </head>
    <body>   
       <!--<a href="javascript:void(0)" class="folder" ><strong>Start</strong></a>-->
        ...
        <div class="list-container uyd-grid">
            <div class="nav-header">
                ...
                <a href="javascript:void(0)" class="entrysetting"  data-placement="bottom-left"><span class="glyphicon glyphicon-cog" ></span></a>
                <div  class="webui-popover-content">
                    <ul class="dropdown-menu" >
                        <li class="setting"><a href="javascript:void(0)" data-checkstate="selectall" id="checkstate"><span class="glyphicon glyphicon-check" aria-hidden="true"></span>Select all folders</a></li>
                        <li class="setting"><a href="javascript:void(0)" data-linktype="multi" data-linkstate="Link Folder" data-toggle='modal' data-target='#linkfolder'><span class="glyphicon glyphicon glyphicon-link" aria-hidden="true" ></span>Link selected folders</a></li>
                        <li class="setting"><a href="javascript:void(0)" data-linktype="multi" data-linkstate="Unlink Folder" data-toggle='modal' data-target='#linkfolder'><span class="glyphicon glyphicon glyphicon-link" aria-hidden="true"></span>Unlink selected folders</a></li>
                    </ul>
                </div>
