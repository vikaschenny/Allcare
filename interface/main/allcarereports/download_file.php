...
//get file metadata
//$curl = curl_init();
//$form_url = $protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/getfileinfo_web/'.$row['notes'].'/'.$fileid;
...
$curl = curl_init();
$form_url = $protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/getaccesstoken_web/'.$row['notes'];
curl_setopt($curl,CURLOPT_URL, $form_url);
curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
$result = curl_exec($curl);
$resultant = $result;
...
if($ext=='xlsx' || $ext=='doc'){ 
    ...
    $url  = $protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/downloadfile_web/'.$row['notes'].'/'.$fileid;
    $ch = curl_init($url);
...
$category=$_REQUEST['category'];
 ...
 $individual_link='http://'.$_SERVER['HTTP_HOST'].'/interface/login/login_frame.php?param='.$id;
