...
$form_apptstatus = htmlspecialchars_decode ($_REQUEST['form_apptstatus']);
$pid = $_REQUEST['pid'];
...
$form_apptstatusClause= '';

if($form_apptstatus != ''){
    $form_apptstatusClause = " AND e.pc_apptstatus IN ($form_apptstatus) ";
}
...
$where = $where.$form_apptstatusClause.$pidClause;
...
if(isset($zirmedUsername) || isset($zirmedPassword) || isset($zirmedCustID) || isset($zirmedResponseType)){
    while($rows = sqlFetchArray($res)){
        ...
        //$resp = file_get_contents('http://qa2allcare.texashousecalls.com/api/elig-270-Draper-Charlesetta.elg');
        $resp = $printData;
