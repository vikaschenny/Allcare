...
<body class="body_top">                     
<form action='non_encounter_data.php' name='chart_form' method='post' onsubmit="">
    ...
    <a href="allcare.php?page=transaction"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
    ...
    <?php         
        $chart = sqlStatement("SELECT DISTINCT(group_name) FROM layout_options " .
                    "WHERE form_id = 'NONENC' AND uor > 0 AND field_id != '' " .
        ...
        foreach ($chart_group as $cgroup){
            ...
            echo "<div id='div_$cgroup'  class='section' style='display:$display_style;' >";
                $form=array('NONENC'=>'Default Medical Record Sections','DEM'=>'Demographics','HIS'=>'History','LBF2'=>'Allcare Encounter Forms');
                foreach($form as $key => $value){ 
                    ...
                    if($group == $cgroup && $key=='NONENC'){
                        ...
                        while($frowres = sqlFetchArray($fres1)){
                            ...
                            $title = $frowres['title'];
                            $formdata = sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND field_id = '$field_id' AND group_name = '$group' AND screen_name= '$cgroup'");
                            $fid_res = sqlFetchArray($formdata);
                            if(substr($cgroup, 1)=='Home Health' || substr($cgroup, 1) == 'Payer Audit') {
                                $cgroup1=str_replace(' ', '-', $cgroup);
                            if(empty($fid_res)):
                                $fins = sqlStatement("insert into tbl_medrecord_non_encounter (form_id,field_id,group_name,option_value,screen_name,page_break) values('$key','$field_id','$group','".$_POST[$field_id."-".$key."-".$cgroup1]."','$cgroup','".$_POST["form_pgb1"."_".$field_id."_".$key."_".$cgroup1]."')");
                            elseif( !empty($fid_res) && $_REQUEST['status'] != 'noupdate'):
                                //echo "update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup1]."' , page_break='".$_POST["form_pgb1"."_".$field_id."_".$key."_".$cgroup1]."'  where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'";
                                $update=sqlStatement("update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup1]."' , page_break='".$_POST["form_pgb1"."_".$field_id."_".$key."_".$cgroup1]."'  where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'");
                            endif;
                            $sel=sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND group_name='$group' AND field_id='$field_id' AND screen_name= '$cgroup'"); 
                           ...
                           if(empty($fid_res)):
                                $fins = sqlStatement("insert into tbl_medrecord_non_encounter (form_id,field_id,group_name,option_value,screen_name,page_break) values('$key','$field_id','$group','".$_POST[$field_id."-".$key."-".$cgroup]."','$cgroup','".$_POST["form_pgb1"."_".$key."_".$cgroup]."')");
                            elseif( !empty($fid_res) && $_REQUEST['status'] != 'noupdate'):
                                //echo "update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup]."' , page_break='".$_POST["form_pgb1"."_".$key."_".$cgroup]."' where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'";
                                $update=sqlStatement("update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup]."' , page_break='".$_POST["form_pgb1"."_".$key."_".$cgroup]."' where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'");
                            endif;
                            $sel=sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND group_name='$group' AND field_id='$field_id' AND screen_name= '$cgroup'"); 
                            $selrow = sqlFetchArray($sel);
                            if(!empty($selrow)){
                    ...
                    echo  "</div>\n";
                    echo "<br>";
                    }else if($group!= $cgroup && $key!='NONENC'){
                        
                        if($key=='LBF2'){
                            if((substr($group, 1)=='Prescription'  || substr($group, 1)=='DME' || substr($group, 1)=='Immunization' || substr($group, 1)=='Medical Problems' || substr($group, 1)=='Allergies'  || substr($group, 1)=='Medication'  || substr($group, 1)=='Surgeries'  || substr($group, 1)=='Dental Problems' ) )  {
                                 
                                 echo "<br>";echo "<b>".htmlspecialchars( xl(substr( $group, 1)), ENT_NOQUOTES)."</b></span>&nbsp&nbsp"; 
                                 if(substr($cgroup, 1)=='Home Health' || substr($cgroup, 1) == 'Payer Audit'){
                                    $cgroup1=str_replace(' ', '-', $cgroup);
                                    $group1=str_replace(' ', '-', $group);
                                    if($_REQUEST['status'] != 'noupdate'){
                                        //echo "update tbl_medrecord_non_encounter SET   layout_col='".$_POST["form_lc1"."-".$group1."-".$key."-".$cgroup1]."' ,page_break='".$_POST["form_pgb1"."-".$group1."-".$key."-".$cgroup1]."'   where form_id='$key' AND group_name='$group' AND screen_name='$cgroup'";
                                        $update1=sqlStatement("update tbl_medrecord_non_encounter SET layout_type = '".$_POST["form_ltype"."-".$group."-".$key."-".$cgroup1]."', layout_col='".$_POST["form_lc1"."-".$group1."-".$key."-".$cgroup1]."' ,page_break='".$_POST["form_pgb1"."-".$group1."-".$key."-".$cgroup1]."'   where form_id='$key' AND group_name='$group' AND screen_name='$cgroup'");
                                    }
                                    $lcsel2=sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND group_name='$group' AND screen_name= '$cgroup'"); 
                                    $lcselrow2 = sqlFetchArray($lcsel2);?>
                                    <?php 
                                        if(substr($group, 1)!='Audit Form'){
                                            if(in_array(substr($group1,1), array('Medical-Problems', 'Allergies', 'Surgeries', 'Medication', 'Dental-Problems','DME','Prescription'))){
                                                ?>    
                                            ...
                                            <i style='font-size:14px'> (Select page break YES/NO)</i><br>
                                        <?php
                                    }
                            }
                            else{  
                                    $group1=str_replace(' ', '-', $group);
                                    if($_REQUEST['status'] != 'noupdate'){
                                       // echo "update tbl_medrecord_non_encounter SET layout_type = '".$_POST["form_ltype"."-".$group1."-".$key."-".$cgroup]."',layout_col='".$_POST["form_lc1"."-".$group1."-".$key."-".$cgroup]."' ,page_break='".$_POST["form_pgb1"."-".$group1."-".$key."-".$cgroup]."'  where form_id='$key' AND group_name='$group' AND screen_name='$cgroup'";
                                        $update2=sqlStatement("update tbl_medrecord_non_encounter SET layout_type = '".$_POST["form_ltype"."-".$group1."-".$key."-".$cgroup]."',layout_col='".$_POST["form_lc1"."-".$group1."-".$key."-".$cgroup]."' ,page_break='".$_POST["form_pgb1"."-".$group1."-".$key."-".$cgroup]."'  where form_id='$key' AND group_name='$group' AND screen_name='$cgroup'");
                                    }
                                    $lcsel=sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND group_name='$group' AND screen_name= '$cgroup'"); 
                                    $lcselrow = sqlFetchArray($lcsel);?> 
                                    <?php 
                                if(substr($group, 1)!='Audit Form'){

                                    if(in_array(substr($group1,1), array('Medical-Problems', 'Allergies', 'Surgeries', 'Medication', 'Dental-Problems','DME','Prescription', 'Immunization'))){
                                    ?>    
                                    <label>Layout Type:</label> 
                             ...
                             echo "<div class='section' style='display:none;'>";
                                ...
                                while($frowres = sqlFetchArray($fres1)){
                                        ...
                                        if(!empty($selrow)){
                                         ...
                                         if(empty($fid_res)):
                                            $fins = sqlStatement("insert into tbl_medrecord_non_encounter (form_id,field_id,group_name,option_value,screen_name,page_break,layout_col, layout_type) values('$key','$field_id','$group','".$_POST[$field_id."-".$key."-".$cgroup]."','$cgroup','".$_POST["form_pgb1"."-".$field_id."-".$key."-".$cgroup]."','".$_POST["form_lc1"."-".$group."-".$key."-".$cgroup]."','".$_POST["form_ltype"."-".$group."-".$key."-".$cgroup]."')");
                                        elseif( !empty($fid_res) && $_REQUEST['status'] != 'noupdate'):
                                            //echo "update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup]."' ,page_break='".$_POST["form_pgb1"."-".$field_id."-".$key."-".$cgroup]."' , layout_col='".$_POST["form_lc1"."-".$group."-".$key."-".$cgroup]."'  where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'";
                                            $update=sqlStatement("update tbl_medrecord_non_encounter SET option_value='".$_POST[$field_id."-".$key."-".$cgroup]."' where form_id='$key' AND field_id='$field_id' AND group_name='$group' AND screen_name='$cgroup'");
                                        endif;
                                        $sel=sqlStatement("select * from tbl_medrecord_non_encounter where form_id='$key' AND group_name='$group' AND field_id='$field_id' AND screen_name= '$cgroup'"); 
                                        $selrow = sqlFetchArray($sel);
                                        if(!empty($selrow)){
