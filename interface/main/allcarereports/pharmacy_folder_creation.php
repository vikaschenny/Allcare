    ...
    if($folder_name!='') {
       //To create folder in parent folder
        if($parentid!='' && $parent_stat=='yes'){
            ...
            $curl = curl_init();
            $form_url2 = 'http://'.$_SERVER['HTTP_HOST'].'/api/DriveSync/insert_folder_web/'.$email.'/'.$parentid.'/'.trim($folder_name,"-");
            ...
            $val= explode(':',$resultant);
             ...
             //create folder api call
            $curl = curl_init();
            $form_url = 'http://'.$_SERVER['HTTP_HOST'].'/api/DriveSync/createfolder_web/'.$email.'/'.trim($folder_name,"-"); 
            curl_setopt($curl,CURLOPT_URL, $form_url);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
            $result = curl_exec($curl);
            $resultant = $result;
...
while($row1=sqlFetchArray($fac)){
    ...
    $sel_custom=sqlStatement("select pharmacy_folder from tbl_pharmacy_custom_attributes_1to1 where pharmacyid='".$row1['id']."'");
