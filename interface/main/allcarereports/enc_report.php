			...
			<td>
			   ...
			   <?php  xl('Details','e'); ?>
                         ...
                         <td>
                                ...
                                $fields1 = array( 'provider'=> 'Provider',
                                                  'f.formdir' => '',
                                                  'f.form_name'=>'',
                                                  'p.pid'=> '',
                                                  'fe.encounter'=>'',
                                ...
                                foreach($fields1 as $key => $value){
                                     if($key=='provider') {
                                          $fieldset3 .= 'CONCAT(u.lname ," " ,u.fname) AS provider,';
                                     }else 
                                     $fieldset3 .= $key.",";
                                }   
...
if($form_selected_fields[0]!='-1' && !empty($_POST['selectEncColmsData'])) {
    ...
    $query = "SELECT " ." ".$fieldparamters1.",".$fieldval1." ".
       "FROM ( form_encounter AS fe, forms AS f ) " .
      "LEFT OUTER JOIN patient_data AS p ON p.pid = fe.pid " .
      "LEFT JOIN users AS u ON u.id = fe.provider_id " .
      "WHERE f.encounter = fe.encounter AND f.formdir = 'newpatient' ";
   ...
   $query .= "ORDER BY $orderby";
    ...
    $query = "SELECT " ." ".$fieldparamters1.",".$fieldparamters." ".
       "FROM ( form_encounter AS fe, forms AS f ) " .
      "LEFT OUTER JOIN patient_data AS p ON p.pid = fe.pid " .
      "LEFT JOIN users AS u ON u.id = fe.provider_id " .
      "WHERE f.encounter = fe.encounter AND f.formdir = 'newpatient' ";
              ...
              // Fetch all other forms for this encounter.
               print("<tr align=left valign=top>");
                    foreach($row as $key=> $value) {
                          if($key!='formdir' && $key!='form_name' && $key!='pid' && $key!='encounter' ){
    //                            if($key=='provider'){ 
    //                               $provider_name=($row[$key] == $lastdocname) ? "" : $row[$key];
    //                               print("<td>$provider_name</td>\n");
    //                            } else 
                                    print("<td>$row[$key]</td>\n");
                          } 
                    }
...
$(document).ready(function() {
    ...
    // DataTable
    var table = $('#encounter_report').DataTable({ 
         dom: 'T<"clear">lfrtip',
                    "tableTools": {
