 ...
 */
 ...
 $primary_docs1 = array(
  ...
  'sch' => array(xl('Scheduling')  , 0, 'main/scheduling.php'));
 
 $fres1 = sqlStatement("SELECT title,group_name,field_id,description FROM layout_options " .
    "WHERE form_id = 'ALLCARE' AND uor > 0 AND field_id != ''  AND group_name = '1AllCare'" .
    "ORDER BY group_name, seq");
    while($frow1= sqlFetchArray($fres1)){
     $primary_docs2[$frow1['field_id']] = array(xl($frow1['title'])  , 0, $frow1['description']);
 } 

// foreach($primary_docs21 as $key => $value){
//     foreach($value as $key => $val){
//         // echo $key."===".$val;
//          print_r(array($val));
//          $primary_docs2 = array($val);
//     }
//      //$output = explode("_", $key);
//     //$primary_docs2 = array($output[0] => array(xl($value)  , 0, 'main/allcarereports/patientreports.php'));
// }
    
//  $primary_docs2 = array('reports' => array(xl('Reports')  , 0, 'main/allcarereports/patientreports.php'),
//  'transaction' => array(xl('Transactions')  , 0, 'main/allcarereports/transactions.php'),   
//  'logs' => array(xl('Logs')  , 0, 'main/allcarereports/patientlogs.php'));   
  
 $primary_docs3 = array( 'app' => array(xl('Portal Activity')  , 0, '../myportal/index.php'),
 ...
 $primary_docs=array_merge($primary_docs1,$primary_docs2,$primary_docs3);
...
<script language='JavaScript'>
 ...
 function getReminderCount(){ 
   ...
   $.post("<?php echo $GLOBALS['webroot']; ?>/library/ajax/dated_reminders_counter.php",
    ...
    // run updater every 60 (WMT CHANGED TO 300/5 MIN) seconds 
     // var repeater = setTimeout("getReminderCount()", 60000); 
      var repeater = setTimeout("getReminderCount()", 300000); 
   });
...
?>
 ...
 //It is called when a new patient is create/selected from the search menu.
  ...
  for(CountEncounter=0;CountEncounter<EncounterDateArray.length;CountEncounter++)
   {
	// WMT - assign a default category name for generated encounters
	...
	cat_name = 'GENERATED';
	if (CalendarCategoryArray[CountEncounter] != '') cat_name = CalendarCategoryArray[CountEncounter];
    str+='<option value="'+EncounterIdArray[CountEncounter]+'~'+EncounterDateArray[CountEncounter]+'">'+EncounterDateArray[CountEncounter]+'-'+cat_name+'</option>';
	// WMT - end of modification
   }
  str+='</Select>';
...
{//Removes an item from the Encounter drop down.
 ...
 // WMT - end require patient/encounter
  top.restoreSession();
  window.open('<?php echo "$web_root/interface/reports/" ?>' + aurl, '_blank', 'width=950,height=550,resizable=1,scrollbars=1');
  return false;
 }
...
$(document).ready(function(){
  if(3 == <?php echo $GLOBALS['concurrent_layout'] ?>){
    ...
    $("#navigation-slide > li  > a#sch0").prepend('<img src="../../images/config-date.png" class="nav-menu-img" />');
    $("#navigation-slide > li  > a#arpimg").prepend('<img src="../../images/reports.png" class="nav-menu-img" />');
    ...
    // WMT -- added following line for lab interfaces
...
<?php } else { // not athletic team ?>
  ...
  <?php genTreeLink('RTop','sch',xl('Scheduling')); ?> 
  <?php $fres = sqlStatement("SELECT * FROM layout_options " .
    "WHERE form_id = 'ALLCARE' AND uor > 0 AND field_id != ''  AND group_name = '1AllCare'" .
    "ORDER BY group_name, seq");
    while($frow = sqlFetchArray($fres)){
     $title[$frow['field_id']]=$frow['title']; 
     $group_name=$frow['group_name'];
     $options[$frow['field_id']]=$frow['edit_options'];
     } 
     ...
     ?>  <li><a class="collapsed" id="arpimg" ><span><?php  echo substr($group_name, 1);  ?></span></a> 
               <ul> <?php foreach($title as $key => $value){   ?> 
              <?php foreach ($current_user_groups as $value1) {
                    if(in_array("Administrators", $current_user_groups)){   ?>
                           <li><?php genTreeLink('RTop',$key,$value); ?> </li>
                    <?php  break; } else if($value1=='Accounting' || $value1=='Clinicians' || $value1=='Emergency Login' || $value1=='Front Office' || $value1=='Physicians' ){
                                     $option=explode(",",$options[$key]);?>
                             <li><?php foreach($option as $option_value) {
                                 if ($option_value== substr($value1, 0, 3)|| $options[$key]=='')  {
//                                     $fres2 = sqlStatement("SELECT title FROM layout_options " .
//                                           "WHERE form_id = 'LBFALLCARE' AND uor > 0 AND field_id != ''  AND group_name = '1AllCare' AND edit_options LIKE '%".$option_value."%'" .
//                                            "ORDER BY group_name, seq");
...
if (acl_check('admin', 'super') && $GLOBALS['wmt_lab_enable']) { // WMT LabLink enabled ?>
...
<?php }  // no WMT laboratory interface ?>
  ...
  <li><a class="collapsed" id="repimg" ><span><?php xl('Reports','e') ?></span></a>
    <ul>
	  ...
	  <?php genMiscLink('RTop','rep','0',xl('List'),'reports/patient_list.php'); ?>
          ...
          <?php // genMiscLink('RTop','rep','0',xl('Rx Import'),'../custom/import_rx_allscripts1.php'); ?>
