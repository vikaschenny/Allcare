...
require_once("../../allcarereports/objects_function.php");
...
if($action=='delete'){
    ...
    $result=deleteMessage(trim($del,"','"));
     newEvent("delete", $_SESSION['authUser'], $_SESSION['authProvider'], 1, "Custom messages: id ".$del);
...
//    }
    ...
    $asql=sqlStatement("SELECT *
                        ...
                        AND option_id = u.abook_type where   active=1 and authorized=1 and (fname!='' or lname!='')");
    ...
    $objval['agency']=$address_Book;  
    $scan['no_value']='no value';
    $objval['scan']=$scan;  
...
$sql="SELECT $all FROM tbl_allcare_custom_messages u
        LEFT JOIN users ON u.user = users.username
        WHERE u.deleted != '1' AND u.message_status!='no message' AND u.assigned_to LIKE ? "; 
if($_REQUEST['action']=='edit'){
...
$sql.=" ORDER BY u.id desc";
 ...
 while($myrow = sqlFetchArray($result)){
    $name = $myrow['from'];
    if($name!=''){
        $from=user_byname($name);
        if($from!=''){
            $myrow['from']=$from;  
        }else{
             $auser=sqlStatement("select * from tbl_allcare_agencyportal where portal_username='$name'");
             $arow=sqlFetchArray($auser);
             $myrow['from']=user_details($arow['uid']);
        }
    }
