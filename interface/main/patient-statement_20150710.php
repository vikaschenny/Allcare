...
// billing facility
 $res = sqlStatement("SELECT * FROM facility WHERE billing_location = 1");
    ...
    //
    $practiceName = $practiceAddr = $practiceCity = $practiceState = $practiceZip = "";
  ...
  if ($INTEGRATED_AR) {
    while ($row = sqlFetchArray($t_res)) {
      ...
      foreach ($codes as $code => $cdata) {
          $dispcode = $code;
          $dxQuery = sqlStatement("SELECT justify FROM billing WHERE code = '".$code."' AND pid = ".$row['pid']." AND encounter =".$row['encounter']);
...
?>
 <?php
    $patient = $row['fname'].$row['mname'].", ".$row['lname'];
    if(count($codes) > 1):
        foreach ($codes as $code => $cdata):
            $eachCharge = "";
