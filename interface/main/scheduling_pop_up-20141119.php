...
function changePatientLabel()
{

	if(jQuery('#gridPatientText').text() == 'Hide Grid')
		jQuery('#gridPatientText').text('Show Grid');
	else
		jQuery('#gridPatientText').text('Hide Grid');
}	
function changeEncLabel()
{

	if(jQuery('#gridEncText').text() == 'Hide Grid')
		jQuery('#gridEncText').text('Show Grid');
	else
		jQuery('#gridEncText').text('Hide Grid');
}	
    </script>
...
<div id='divContainer' style='background-color:#FFFFCC;'>
    ...
    <div id="divMain1" style='margin:2%;padding:2%;height:auto;background:#FFFF66'>
        ...
        <div style="float:right">
            <input type="button" value="Refresh" style="height:50px"
                           onclick="javascript:showPatients();" />                  
        </div>
        <br><br><br>
    ...
    while ($rowQueries = sqlFetchArray($getQueries)) 
    {
      echo "<option value='" . $rowQueries['name'] . "'>".$rowQueries['name']."</option>";
        ...
        <div class="lblSelect"><b>Patient Standard Data:</b></div>
            <div class='divSelect'>
                <select id="selectPatientData" multiple="multiple" onchange="javascript:showPatients();">
		...
		<div class='divSelect'>
                    <select id="selectPOSFields" multiple="multiple" onchange="javascript:showPatients();">
	...
	<div id="divSelectProvider">
		...
		<div class='divSelect'>
                    <select id='selectProviders' onchange="javascript:showPatients();" class="btn btn-default" style="text-align:left;" multiple>
                        <option value='-1'>---Select---</option>
	...
	<div id="divSelectPayer">
		...
		<div class='divSelect'>
		<select id='selectPayers' onchange="javascript:showPatients();" class="btn btn-default" style="text-align:left;" multiple>
                    <option value='-1'>---Select---</option>
                    <option value='-2' selected>All</option>
		<?php
	...
	<div id="divSelectVisitCategory">
		...
		<div class='divSelect'>
                    <select id='selectVisitCategory' onchange="javascript:showPatients();" class="btn btn-default" style="text-align:left;" multiple>
                        <option value='-1'>---Select---</option>
                        <option value='-2' selected>All</option>
                    <?php
	...
	<br>                
        ...
        <div>
            <b><a id="gridPatientText" onclick="javascript:jQuery('#divPatientsList').toggle(500);changePatientLabel();"
               style="cursor:pointer;">Hide Grid</a></b>
...
<script type="text/javascript" src="../../library/dynarch_calendar_setup.js"></script>
	...
	<br>
       ...
       value="" onchange='javascript:showPatientsByEncounters()' />
...
Calendar.setup({inputField:"date_to", ifFormat:'%Y-%m-%d', button:"img_calendar_to"});
	...
	<br>                
        ...
        <div>
			<b><a id="gridEncText" onclick="javascript:jQuery('#divPatientsListByEncounter').toggle(500);changeEncLabel();"
               style="cursor:pointer;">Hide Grid</a></b>
...
/**************************************************************************/
	//alert('add '+i+':'+jQuery("#hdnAddress_"+i).val());
            ...
            var address_patient = jQuery("#hdnAddress_"+i).val();
            var show_address=jQuery("#hdnDetails_"+i).val();
...
{
    var addresses=document.getElementsByName("hdnAddressEnc");    
    for(var i=0;i<addresses.length;i++)
    {
    ...
    jQuery.each(addresses,function(i,val){
        ...
        var geocoder = new google.maps.Geocoder();
        var address_patient = jQuery("#hdnAddressEnc_"+i).val();
        var show_address=jQuery("#hdnDetailsEnc_"+i).val();
        geocoder.geocode({ 'address': address_patient }, function (results, status) {
            if (status === google.maps.GeocoderStatus.OK) 
            {
                map.setCenter(results[0].geometry.location);
