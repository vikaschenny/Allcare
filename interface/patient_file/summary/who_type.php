...
if($type=='provider') {
    ...
    echo "<label>From:</label> <select name='form_provider1' id='form_provider1' title='provider' onchange='provider_details(this);' >";
    ...
    echo "<label>From:</label><select name='form_facility' id='form_facility' onchange='facility_details(this);'>\n";
    ...
    echo "   </select>\n";
     ...
     echo "<label>From:</label><select name='form_pharmacy' id='form_pharmacy' title='pharmacy' onchange='pharmacy_details(this);'>";
    echo "<option value=''>Select</option>";
  ...
  $name = htmlspecialchars($name, ENT_QUOTES);
  echo "<label>From:</label><select name='form_payer' id='form_payer' onchange='payer_details(this);'>\n";
  echo "<option value='' >Select</option>";
  while ($frow = sqlFetchArray($fres)) {
  ...
  echo "   </select>\n";
    ...
    if($fid=='incomplete') {
        ...
        $ures = sqlStatement("SELECT id, fname, lname, organization, username FROM users " .
            ...
            <label>From:</label>  
       <select name="refer_to" id="refer_to" title="" onchange="addr_bk(this);" >
...
function addr_bk(target){
    ...
    $.ajax({
        ...
        success: function(response)
        {
            $(target).next('#addr_bk').html(response);
...
function provider_details(target){
...
function facility_details(target){
...
function pharmacy_details(target){
    var value3 = $(target).val();
    $.ajax({
        type: 'POST',
        url: url,	
...
function payer_details(target){
    var value4 =$(target).val();
    $.ajax({
        type: 'POST',
        url: url,	
        data:{payer:value4},
