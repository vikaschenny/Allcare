...
$trow = $_REQUEST['id'] ?getNonencChartById($_REQUEST['id'],substr($_REQUEST['chartgroupshidden'],1)) : array();
  ...
  $empty_name=' ', $class='', $onchange='', $tag_id = '', $custom_attributes = null )
{
  ...
  $selectEmptyName = htmlspecialchars( xl($empty_name), ENT_NOQUOTES);
    if(($tag[2] == 'demographics') || ($tag[2] == 'dem') || ($tag[2] == 'demographic')){
...
<div id="chart_output">
    <ul class="tabNav">
        ...
        $fres = sqlStatement("SELECT * FROM layout_options " .
          "WHERE form_id = 'NONENC' AND uor > 0 AND group_name = '".$_POST['chartgroupshidden']."'" .
      ...
      while ($frow = sqlFetchArray($fres)) {
        ...
        $defaultval = sqlStatement("SELECT option_value FROM tbl_medrecord_non_encounter " .
        "WHERE form_id = 'NONENC' AND group_name = '".$this_group."' AND screen_name='".$this_group."' AND field_id='$field_id'");
