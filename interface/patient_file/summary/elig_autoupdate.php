...
function geteditrecords($data,$get_fields_names,$pid,$provider){
   ...
   foreach($data as $key_name =>$valuearray){
    ...
    $save_elig_data = sqlStatement("UPDATE tbl_eligibility_response_data SET `$field_id` = '".addslashes($field_val)."', updated_date = NOW(),`domain` = 'EMR_Eligibility_Response' , `user` = '$provider' WHERE id = $id");
