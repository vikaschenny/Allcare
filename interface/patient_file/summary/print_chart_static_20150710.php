...
while ($eidval = sqlFetchArray($geteid)){ 
    ...
    if(!empty($eidval['elec_signedby'])  && !empty($eidval['elec_signed_on'])){
        $getSignImg="SELECT signature_image, CONCAT(fname,', ', lname) as name, CONCAT(fname,' ', lname) as fname,
        ...
        echo "</td></tr>";
        echo "<tr><td>";
        echo "<br/><br/><u>".ucwords($rowSignImg['fname']).", ".$rowSignImg['designation']."</u> Electronically Signed by ". ucwords($rowSignImg['name'])." ".$rowSignImg['designation']." on ".$eidval['elec_signed_on'];
