...
function submitme() {
    ...
    if (validate(f)) {
        var errorno=0;
        if(($("#single_sel_dos").parents('.tab').hasClass('current')==true || $("#muti_sel_dos").parents('.tab').hasClass('current')==true )&& $("#dos").val()==""){
            ...
            <div class="tab" id="tab2">
                ...
                <?php   $sql12 = sqlStatement("SELECT * FROM tbl_form_chartoutput_transactions WHERE id = $id ");
                        ...
                        $date1  =  $sql23['date_of_service'];
                                ?>
                <label>Transaction Type:</label><select id="transtype" name="transtype" onchange="transaction();" ><option value="1" <?php if($sql23['transaction']==1) echo "selected"; ?>>Medical Record</option><option value="2" <?php if($sql23['transaction']==2) echo "selected"; ?> >Face To Face</option></select>
