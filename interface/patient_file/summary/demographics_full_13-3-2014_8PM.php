...
$existingPOSSsql = "SELECT fg.Recordset_ID,fg.Recordset_Name,fg.POS_id
                    ...
                    INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id 
                      where fg.POS_id=(select p2.pos_id from tbl_allcare_patients1ton p2 
                      INNER JOIN tbl_pos_types pt ON pt.id = p2.pos_id and p2.pid=".$GLOBALS['pid']." 
                          group by p2.pos_id order by pt.title asc limit 0,1)
...
$(document).ready(function(){
        ...
        if($existingPOSSRows>0)
 {
            while($rowRecSql=sqlFetchArray($existingPOSSQry))
             {
                ?>
                        var  oTable =$("#example<?php echo $rowRecSql['POS_id'].$rowRecSql['Recordset_ID'];?>").dataTable( {
