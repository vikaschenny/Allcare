...
<?php
  // WMT - lab interface specific functionality
  if ($GLOBALS['wmt_lab_enable'] && acl_check('patients', 'med')) { ?>
	// Initialize the lab form if it is registered and user is authorized.
	$("#labdata_ps_expand").load("labdata_fragment.php");
<?php 
  } 
  // WMT - end modification
...
|
<a href="../summary/add_facility.php?active=all" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Service Facility'),ENT_NOQUOTES); ?></a>
|
<a href="../summary/add_agency.php?active=all" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Agencies'),ENT_NOQUOTES); ?></a>
<!--|
<a href="../summary/add_face_to_face.php?active=all" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Face To Face'),ENT_NOQUOTES); ?></a>-->
|
<a href="../summary/add_chartoutput.php?active=all" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Medical Record'),ENT_NOQUOTES); ?></a>
|
<a href="../../reports/patient_full_encounters_single_view.php?pid=<?php echo $pid; ?>" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Charts'),ENT_NOQUOTES); ?></a>
<!--|
<a href="../summary/eligibility_response_view.php?pid=<?php echo $pid; ?>" class='iframe large_modal' onclick='top.restoreSession()'>
<?php echo htmlspecialchars(xl('Eligibility Response'),ENT_NOQUOTES); ?></a>-->
  </td>
 </tr>
</table> <!-- end header -->
...
<?php
  // WMT -- lab interface specific section
  if ($GLOBALS['wmt_lab_enable'] && acl_check('patients', 'med')) { ?>
							<tr>
								<td width='650px'>
<?php // labdata expand collapse widget
  ...
  $widgetLabel = "labdata";
  $widgetButtonLabel = xl("Trend");
  $widgetButtonLink = "../../reports/laboratory/lab_analysis.php";
  $widgetButtonClass = "";
  $linkMethod = "html";
  $bodyClass = "notab";

  // check to see if any labdata exist
  $spruch = "SELECT rpt.date_collected AS date " .
		"FROM procedure_report rpt " .
		"JOIN procedure_order ord ON  rpt.procedure_order_id = ord.procedure_order_id " .
		...
		"ORDER BY rpt.date_collected DESC ";
