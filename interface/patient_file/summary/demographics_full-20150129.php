...
function abooktypeselected(){
    
        var selectedabookval= $('#selectabooktype option:selected').val() ;
        
        $.ajax({
            url: 'fetchOrgByAbooktype.php?callback=?',
            type: 'GET',
            data:'type='+selectedabookval, 
...
<?php
/*
   $existingPOSSsql = "SELECT fg.Recordset_ID,fg.Recordset_Name,fg.POS_id
                    from tbl_allcare_patients1ton_fieldmapping fg
                    INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id 
                      where fg.POS_id in(select p2.pos_id from tbl_allcare_patients1ton p2 
                      INNER JOIN tbl_pos_types pt ON pt.id = p2.pos_id and p2.pid=".$GLOBALS['pid']." 
                          group by p2.pos_id order by pt.id desc)
                    group by fg.Recordset_ID order by fg.Recordset_Name";

     
*/   
   $existingPOSSsql ="select distinct p2.pos_id from tbl_allcare_patients1ton p2 
           where p2.pid=".$GLOBALS['pid'];
   
   
//echo $existingPOSSsql;//die;
...
?>
 ...
 <?php
            ...
            while($rowRecSql=sqlFetchArray($existingPOSSQry))
             {
                        $showGroupSql =  "SELECT fg.id, fg.POS_id, fg.Recordset_ID,fg.Recordset_Name, pt.title from tbl_allcare_patients1ton_fieldmapping fg INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id where POS_id=".$rowRecSql['pos_id']." group by fg.Recordset_ID";
                        $showGroupQry =  sqlStatement($showGroupSql);
                        while($POS1to1Rec = sqlFetchArray($showGroupQry)) 
                        {
 
                ?>
                        var  oTable =$("#example<?php echo $POS1to1Rec['POS_id'].$POS1to1Rec['Recordset_ID'];?>").dataTable( {
    ...
    oTable.$('td.editable_class').editable( 'recordset_editable.php', {
...
function deleteRecordsetRow(tblRowId)
{
    $.ajax({
                     type: 'POST',
                     url: 'delete_recordset_row.php',
                     data:{tblRowId:tblRowId},
