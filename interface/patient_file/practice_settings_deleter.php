...
function form_delete($formdir, $formid) {
  ...
  }else if($formdir == 'auditform'){
        row_delete("tbl_form_audit", "id = '$formid'");
  }
  else if ($formdir == 'cpo') {
...
<?php
 ...
 //
  ...
  else if($_REQUEST['insurance']!=''){
       
        row_delete("insurance_companies", "id = '".$_REQUEST['insurance']."'"); 
        $sql=sqlStatement("select * from tbl_patientinsurancecompany where insuranceid='".$_REQUEST['insurance']."'");
        while($row=sqlFetchArray($sql)) {
            row_delete("tbl_patientinsurancecompany", "id = '".$row['id']."'");
        }
