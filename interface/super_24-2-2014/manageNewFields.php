...
function newEvt(selectedBox)
{
     ...
     {
  dlgopen('getTableFields.php?table_name='+document.getElementById(selectedBox).value+'&selectedBox='+selectedBox,
   '_blank', 550, 270);
     }
  return false;
}
...
var totalFields=0;
/*
function receivedFromChild(tableName,checkedFields,checkedFieldsType)
{            
    ...
    {              
        ...
        //alert('ff= '+jQuery('#related_tables'+i).val());
...
function validateStep1()
{
    ...
    {
        if(relTables[j].value==='none' && jQuery('#txtFieldName').val()==='')
        {
            flag=0;
        }
        else
...
function validateDataTypes()
{
    for(var i=0;i<20;i++)
    {            
        if(jQuery('#txtFieldName'+i).val()!=='' && (jQuery('#data_types'+i).val()==='' || jQuery('#txtDataLength'+i).val()===''))
        {
            alert('Please enter Data Type & Data Length for the field '+jQuery('#txtFieldName'+i).val()+'');
            return false;                            
        }
	if(isNaN(jQuery('#txtDataLength'+i).val()))
        {
            alert('Please enter Data Length in numbers for the field '+jQuery('#txtFieldName'+i).val());
...
{ 
        ...
        var finalURL='editGroupRecordset.php';
        var pos_id=jQuery("#lstpostype").val();
...
//            jQuery('#txtDataLength'+i).attr('readonly','');
    ...
    */
        ...
        if(delList[i].checked===true)
        {
            ...
            */
            ...
            for(var j=i;j<(totalFields);j++)
            {
                ...
                if(jQuery('#txtFieldName'+(j+1)).attr('readonly'))
                {
                    jQuery('#txtFieldName'+j).attr('readonly','readonly');                            
                }
                
                if(jQuery('#data_types'+(j+1)).attr('disabled'))
                {
                    jQuery('#data_types'+j).attr('disabled','disabled');                          
                }
                
                if(jQuery('#txtDataLength'+(j+1)).attr('readonly'))
                {
                    jQuery('#txtDataLength'+j).attr('readonly','readonly');                      
                }                  
                
                if(jQuery('#boolFieldRequired'+(j+1)).is(':checked'))
                {
                    jQuery('#boolFieldRequired'+j).attr('checked', true);
                }
    ...
    //totalFields=0;
    //alert('DL = '+checkedNum);
    totalFields=Math.abs(totalFields-checkedNum);
