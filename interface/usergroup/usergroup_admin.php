...
//  echo "<br>9090";
  if($_FILES['imgSignatureEdit'])
  {
      ...
      $sql2 = "SELECT objectid,objectref FROM tbl_centralobjectref WHERE objectid=".$_POST['id']." AND objecttype = 'User'";
        $r = sqlStatement($sql2);
        $objref = "";
        ...
        $str = "SELECT email,cangroup FROM tbl_user_custom_attr_1to1 WHERE userid='". $_POST['id'] ."'";
        ...
        if($datasync == 1):
            if($count):
                $sql = "UPDATE allcareobjects SET userid=".$_POST['id'].", 
                                                  username='".$user_data["username"]."',
                                                  fname='".$_POST['fname']."',
                                                  mname='".$_POST['mname']."',
                                                  lname='".$_POST['lname']."',
                                                  uemail='".$email."',    
                                                  ufacilityid='".$_POST['facility_id']."',
                                                  cangroup='".$cangroup."',    
                ...
                $sql = "INSERT INTO allcareobjects (userid,practiceId,username,fname,mname,lname,uemail,ufacilityid,cangroup,objecttype) 
                    VALUES(".$_POST['id'].",'".$practiceId."','".$user_data["username"]."','".$_POST['fname']."','".$_POST['mname']."','".$_POST['lname']."','".$email."','".$_POST['facility_id']."','".$cangroup."','User')";
                $stmt = $sqlconfCentralDB->prepare($sql) ;
                $stmt->execute();
            ...
            if($count == 0):
                sqlStatement("INSERT INTO tbl_centralobjectref (objectid,objectref,objecttype) VALUES(".$_POST['id'].",".$id.",'User')");
            endif;
         endif;   
  ...
  if ($_POST["mode"] == "new_user") {
    ...
    if($success)
    {
      ...
      $ws = new WSProvider($prov_id);
        ...
        $sql2 = "SELECT objectid,objectref FROM tbl_centralobjectref WHERE objectid=".$prov_id." AND objecttype = 'User'";
        $r = sqlStatement($sql2);
        $objref = "";
        while($row2 = sqlFetchArray($r)):
        $objref = $row2['objectref'];
        ...
        // Get user email from user custom attribute table
        $str = "SELECT email FROM tbl_user_custom_attr_1to1 WHERE userid='". $_POST['id'] ."'";
        $query = sqlStatement($str);
        $email = $cangroup = '';
        while($row = sqlFetchArray($query)):
            $email = $row['email'];
        ...
        if($datasync == 1):
            if($count):
                $sql = "UPDATE allcareobjects SET userid=".$prov_id.", 
                                                  username='".trim(formData('rumple'))."',
                                                  fname='".trim(formData('fname'))."',
                                                  mname='".trim(formData('mname'))."',
                                                  lname='".trim(formData('lname'))."',
                                                  uemail='".$email."',    
                                                  ufacilityid='".trim(formData('facility_id'))."',
                                                  cangroup='".$cangroup."',    
                                                  practiceId='".$practiceId."'    
                                                  WHERE id=".$objref;
                $stmt = $sqlconfCentralDB->prepare($sql) ;
                ...
                $sql = "INSERT INTO allcareobjects (userid,practiceId,username,fname,mname,lname,uemail,ufacilityid,cangroup,objecttype) 
                    VALUES(".$prov_id.",'".$practiceId."','".trim(formData('rumple'))."','".trim(formData('fname'))."','".trim(formData('mname'))."','".trim(formData('lname'))."','".$email."','".trim(formData('facility_id'))."','".$cangroup."','User')";
                $stmt = $sqlconfCentralDB->prepare($sql) ;
                $stmt->execute();
                $id = $sqlconfCentralDB->lastInsertId();
            endif;
            ...
            if($count == 0):
                sqlStatement("INSERT INTO tbl_centralobjectref (objectid,objectref,objecttype) VALUES(".$prov_id.",".$id.",'User')");
            endif;
        endif;   
    }
...
//get user id 
$uid=sqlStatement("select max(id)as id from users");
...
if($sel_rows['provider_folder_trigger']=='yes'){
       ...
       // to get configured email
                ...
                {
                    $query = $sel_rows['provider_folder'] . " and id=" . $uidrow['id'] ;
                    $fsql = sqlStatement("$query");
                    $frow = sqlFetchArray($fsql);
                    $folder_name = str_replace(" ", "_", $frow['provider_folder']);  
                }
                $curl = curl_init();
                ...
                if($val[0]!=''){
                         ...
                         $today = date("Y-m-d"); 
                         $ins=sqlStatement("insert into tbl_user_custom_attr_1to1 (userid,drive_sync_folder,created_date)values('".$uidrow['id']."','".$val[0]."','$today')"); 
                        // $ins=sqlStatement("update patient_data SET patient_folder='$link' where pid=".$newpid);
                         $ins_log=sqlStatement("insert into DriveSync_log (date,user,email,encounter,patient_id,google_folder,file_name,file_id,status,watsID,category)values(now(),'".$_SESSION['authUser']."','".$row['notes']."','','".$uidrow['id']."','$link','','','folder_created(during User Creation in EMR)','','user')");
                    //subfolders
                    if($sel_rows['provider_sub_folder']=='yes'){
                        $subsql = sqlStatement("select * from list_options where list_id='AllcareUsersSubfolders'");
                        $link1='';
                        ...
                        while($subrow = sqlFetchArray($subsql)){
                         ...
                         $ins_log=sqlStatement("insert into DriveSync_log (date,user,email,encounter,patient_id,google_folder,file_name,file_id,status,watsID,category)values(now(),'".$_SESSION['authUser']."','".$row['notes']."','','".$uidrow['id']."','$link1','','','subfolder_created(during user Creation in EMR)','','user')");
                        }
                    }
    ...
    <div>
	  ...
	  <tr >
		...
		<td><a href="facility_user.php" class="css_button"><span><?php xl('View Facility Specific User Information','e'); ?></span></a>
                ...
                <td><a href="#" class="css_button" id="userwelcomenotifiy"><span><?php xl('User Welcome Notification','e'); ?></span></a>
		</td>
...
?>
    ...
    $('#userwelcomenotifiy').click(function(){
        var url = '../../modules/user_notification/user_creation.php';
        $.ajax({
            url:url,
            type:'post',
            data:{type:'user'},
