...
if ($mode) {   
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_userpayroll SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
    ...
    $sets .= " userid = ?";
    $id = sqlInsert("INSERT INTO tbl_userpayroll SET $sets", $sqlBindArray);
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="payroll_1ton.php?userid="<?php echo $_REQUEST['userid']; ?>  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
