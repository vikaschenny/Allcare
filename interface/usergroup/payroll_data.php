...
$newdata = array();
$newdata['tbl_userpayroll']['userid'] = $userid;
...
if($id)
{ 
  updatePayroll1ton($id, $newdata['tbl_userpayroll'] ,$create=false);
}
else
{  
    updatePayroll1ton($id, $newdata['tbl_userpayroll'],$create=true);

}

if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='payroll_1ton.php?userid=$userid';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='payroll_1ton.php?userid=$userid';";
}
...
$trow = $id ? getPayrollDatabyId($id) : array();
...
if(usertype=="add")
   window.parent.hidepenal(["save",usernumber,"payroll_data_list"]);
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('PAYROLL'); ?>
...
<body class="body_top" onload="<?php echo $body_onload_code; ?>" >
    <form name='new_userdata' method='post' action='payroll_data.php?id=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="payroll_1ton.php?userid="<?php echo $_REQUEST['userid']; ?>  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="canceladd(event)">
...
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'PAYROLL' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
