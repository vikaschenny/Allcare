...
if ($result = getUserDatabyUserid($userid)) {
	...
	echo "<tr height='25'><td>";
        print "<a href='extra_user_data.php?userid=$userid&id=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
                "&inmode=edit' class='css_button_small' onclick='userstable(event,this,\"User Credentials\")'><span>".
                htmlspecialchars( xl('Edit'), ENT_NOQUOTES)."</span></a>";
        echo "</td><td>";
        if (acl_check('admin', 'super')) {
                echo "<a href='../patient_file/deleter.php?userid=".
        ...
        foreach($field_id as $attr){
            $fres2=sqlStatement("SELECT data_type FROM layout_options WHERE form_id = 'USERS' AND uor > 0 AND field_id = '$attr'");
