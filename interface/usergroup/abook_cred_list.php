...
if ($result = getAbookCredDatabyUserid($abookuserid)) {
	...
	echo "<tr height='25'><td>";
        print "<a href='abook_cred.php?abookuserid=$abookuserid&id=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
                "&inmode=edit' onclick='addresbooktable(event,this,\"Address Book Credentials\")' class='css_button_small'><span>".
                htmlspecialchars( xl('Edit'), ENT_NOQUOTES)."</span></a>";
        echo "</td><td>";
        if (acl_check('admin', 'super')) {
                echo "<a href='../patient_file/deleter.php?abookcredid=".
                        htmlspecialchars( $iter{"id"}, ENT_QUOTES).
                        "' onclick='deleteaddressbookrow(event,this,\"abookcredid\")' class='css_button_small'><span>".
        ...
        foreach($field_id as $attr){
            $fres2=sqlStatement("SELECT data_type FROM layout_options WHERE form_id = 'ABOOKCRED' AND uor > 0 AND field_id = '$attr'");
