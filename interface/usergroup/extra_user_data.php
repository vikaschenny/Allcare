...
$newdata = array();
$newdata['tbl_patientuser']['userid'] = $userid;
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'USERS' AND uor > 0 AND field_id != '' " .
...
if($id)
{ 
  updateUserCred1ton($id, $newdata['tbl_patientuser'] ,$create=false);
}
else
{  
    updateUserCred1ton($id, $newdata['tbl_patientuser'],$create=true);

}
  
if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='user_cred_1ton.php?userid=$userid';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='user_cred_1ton.php?userid=$userid';";
}
...
$trow = $id ? getUserDatabyId($id) : array();
...
var usertype = '<?php echo $mode; ?>' || "";
var usernumber = '<?php echo $userid; ?>' || "";
if(usertype=="add")
   window.parent.hidepenal(["save",usernumber,"extra_userdata_list"]);
...
// onclick='return deleteme()'
 dlgopen('../patient_file/deleter.php?userid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>', '_blank', 500, 450);
 return false;
}
...
function imdeleted() {
 top.restoreSession();
 location.href = 'usergroup/add_extrausersdata.php';
}
...
<form name='new_userdata' method='post' action='extra_user_data.php?id=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="add_extrausersdata.php?userid="<?php echo $_REQUEST['userid']; ?>  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="canceladd(event)">
