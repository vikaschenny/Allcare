...
if ($mode) {   
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_patientuser SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
    ...
    $sets .= " userid = ?";
    $id = sqlInsert("INSERT INTO tbl_patientuser SET $sets", $sqlBindArray);
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="add_extrausersdata.php?userid="<?php echo $_REQUEST['userid']; ?>  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
