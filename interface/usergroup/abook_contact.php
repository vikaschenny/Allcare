...
if ($mode) {   
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_user_abookcontact SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
    ...
    $sets .= " userid = ?";
    $id = sqlInsert("INSERT INTO tbl_user_abookcontact SET $sets", $sqlBindArray);
  
  }   */
$newdata = array();
$newdata['tbl_user_abookcontact']['userid'] = $abookuserid;
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'ABOOKCONTACT' AND uor > 0 AND field_id != '' " .
  "ORDER BY group_name, seq");
while ($frow = sqlFetchArray($fres)) {
...
if($id)
{ 
 updateAbookData1ton($id, $newdata['tbl_user_abookcontact'] ,$create=false);
}
else
{  
    updateAbookData1ton($id, $newdata['tbl_user_abookcontact'],$create=true);

}
  
if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='abook_contacts_1ton.php?abookuserid=$abookuserid';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='abook_contacts_1ton.php?abookuserid=$abookuserid';";
}
...
$trow = $id ? getAbookDatabyId($id) : array();
...
var addressbooktype = '<?php echo $mode; ?>' || "";
var addressbooknumber = '<?php echo $abookuserid; ?>' || "";
if(addressbooktype=="add")
   window.parent.hidepenal(["save",addressbooknumber,"abook_contact_list"]);
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('ABOOKCONTACT'); ?>
...
<body class="body_top" onload="<?php echo $body_onload_code; ?>" >
    <form name='new_userdata' method='post' action='abook_contact.php?id=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="abook_contacts_1ton.php?abookuserid=<?php echo $abookuserid; ?>"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="canceladd(event)" >
...
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'ABOOKCONTACT' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
