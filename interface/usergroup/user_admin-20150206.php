...
//echo "<script>alert('actvar user_admin===".$_GET["active"]."');</script>";
...
if ($_GET["mode"] == "update") {        
  ...
  if($_GET['imgSignatureEdit'])
  {
      //echo "imgS=".$iter['imgSignatureEdit'];
    //move_uploaded_file($_FILES['imgSignatureEdit']['tmp_name'],"../pic/user_sign/".$_FILES['imgSignatureEdit']['name']);  
    //$tqvar = formData('signature_image','G');
    //sqlStatement("update users set info = '$tqvar' where id = {$_GET["id"]}");
      $tqvar = formData('imgSignatureEdit','G');
    sqlStatement("update users set signature_image = '$tqvar' where id = {$_GET["id"]}");
  }
  /******************************/
...
<li class='current'>
<a onclick="javascript:jQuery('#table_user_edit').show();
                       jQuery('#table_provider_signature_edit').hide();
...
<a onclick="javascript:jQuery('#table_user_edit').hide();
                       jQuery('#table_provider_signature_edit').show();
                       jQuery('.tabNav > li').removeClass('current');
                       jQuery(this).parent('li').addClass('current');" style='cursor:pointer' id="tabProviderSignature_edit"> User Signature</a>
</li>
</ul>    
    <br>    
...
?>
  ...
  <!--<tr>
      ...
      <td><input type="file" id="imgSignatureEdit" name="imgSignatureEdit" accept="image/*"
  ...
  <tr>
      ...
      <td>
        ...
        src="../pic/user_sign/<?php echo $iter["signature_image"];  ?>" />
              <input type="hidden" id="hdnPreviousImg" name="hdnPreviousImg"
                     value="<?php echo $iter["signature_image"];  ?>" />
              <script>
                  
                  jQuery('#img_sign').on("load",function(){
                      this.attr("src","../pic/user_sign/<?php echo $iter["signature_image"];  ?>");
                  });
		 ...
		 function readURL(input,imageID,hdnImageID)
		 {
                ...
                jQuery('#img_sign').attr('src',jQuery('#imgSignatureEdit').val());
              </script>
          </div>      
      </td>
  </tr>-->
...
<INPUT TYPE="HIDDEN" NAME="secure_pwd" VALUE="<?php echo $GLOBALS['secure_password']; ?>">
    ...
    <!--------------------------------------->
    <TABLE border=0 cellpadding=0 cellspacing=0 id="table_provider_signature_edit">
		<tr>
			<td><span class=text><?php xl('Signature Image','e'); ?>:</span></td>
			<td><input type="file" id="imgSignatureEdit" name="imgSignatureEdit" accept="image/*"
           onchange="javascript:readURL(this,'img_sign','');" />
...
$(document).ready(function(){
	$('#table_provider_signature_edit').hide();
    $("#cancel").click(function() {
		  parent.$.fn.fancybox.close();
	 });

           if(!$('#authorized').prop("checked"))
           {
