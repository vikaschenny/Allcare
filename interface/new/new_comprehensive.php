...
function getLayoutRes1() {
  global $SHORT_FORM; 
  return sqlStatement("SELECT * FROM layout_options " .
    "WHERE form_id = 'SERVICEFAC' AND uor > 0 AND field_id != '' " .
    ($SHORT_FORM ? "AND ( uor > 1 OR edit_options LIKE '%N%' ) " : "") .
    "ORDER BY group_name, seq");
}

function getLayoutRes2() {
  global $SHORT_FORM;
  return sqlStatement("SELECT * FROM layout_options " .
...
function searchit() {
 ...
 var url = '../main/finder/facility_select.php?popup=1';

<?php
$lres1 = getLayoutRes1();
while ($lrow1 = sqlFetchArray($lres1)) {
  
  $field_id1  = $lrow1['field_id'];
  $data_type1 = $lrow1['data_type'];
  $fldname1 = "form_$field_id1";
  switch(getSearchClass($data_type1)) {
    case  1:
      echo
      " if (f1.$fldname1.style.backgroundColor != '' && trimlen(f1.$fldname1.value) > 0) {\n" .
      "  url += '&$field_id1=' + encodeURIComponent(f1.$fldname1.value);\n" .
      " }\n";
      break;
    case 2:
      echo
      " if (f1.$fldname1.style.backgroundColor != '' && f1.$fldname1.selectedIndex > 0) {\n" .
      "  url += '&$field_id1=' + encodeURIComponent(f1.$fldname1.options[f1.$fldname1.selectedIndex].value);\n" .
      " }\n";
      break;
  }
}
...
function searchagen() {
 ...
 var url2 = '../main/finder/agency_select.php?popup=1';

<?php
$lres2 = getLayoutRes2();
while ($lrow2 = sqlFetchArray($lres2)) {
  $field_id2  = $lrow2['field_id'];
  $data_type2 = $lrow2['data_type'];
  $fldname2 = "form_$field_id2";
  switch(getSearchClass($data_type2)) {
    case  1:
      echo
      " if (f2.$fldname2.style.backgroundColor != '' && trimlen(f2.$fldname2.value) > 0) {\n" .
      "  url2 += '&$field_id2=' + encodeURIComponent(f2.$fldname2.value);\n" .
      " }\n";
      break;
    case 2:
      echo
      " if (f2.$fldname2.style.backgroundColor != '' && f2.$fldname2.selectedIndex > 0) {\n" .
      "  url2 += '&$field_id2=' + encodeURIComponent(f2.$fldname2.options[f2.$fldname2.selectedIndex].value);\n" .
      " }\n";
      break;
  }
}
...
<!-- for facility-->
<div id="service_facility">
    
<form action='new_comprehensive_save.php' name='service _facility form' method='post' onsubmit='return validate(this)'>

<span class='title'><?php xl('Search Service Facility','e'); ?></span>
...
while ($frow1 = sqlFetchArray($fres1)) {
  ...
  $this_group = $frow1['group_name'];
  $titlecols  = $frow1['titlecols'];
  $datacols   = $frow1['datacols'];
  $data_type  = $frow1['data_type'];
  $field_id   = $frow1['field_id'];
  $list_id    = $frow1['list_id'];
  $currvalue  = '';
  ...
  if (strcmp($this_group, $last_group) != 0) {
    ...
    if (!$SHORT_FORM) {
      ...
      $group_name = substr($this_group, 1);
      if (strlen($last_group) > 0) echo "<br />";
      echo "<span class='bold'><input type='checkbox' name='form_cb1_$group_seq' id='form_cb1_$group_seq' value='1' " .
        "onclick='return divclick(this,\"div1_$group_seq\");'";
      if ($display_style == 'block') echo " checked";
        
      // Modified 6-09 by BM - Translate if applicable  
      echo " /><b>" . xl_layout_label($group_name) . "</b></span>\n";
        
      echo "<div id='div1_$group_seq' class='section' style='display:$display_style;'>\n";
  ...
  if ($titlecols > 0) {
    end_cell1();
    echo "<td colspan='$titlecols'";
    echo ($frow1['uor'] == 2) ? " class='required'" : " class='bold'";
    if ($cell_count == 2) echo " style='padding-left:10pt'";
    echo ">";
    $cell_count += $titlecols;
  }
  ...
  if($field_id!="POSid")
  {
  if ($frow1['title']) echo (xl_layout_label($frow1['title']).":"); else echo "&nbsp;";
  }
  echo "</b>";
  ...
  ++$item_count;
  generate_form_field($frow1, $currvalue);
}
...
<br />
<?php if ($WITH_SEARCH) { ?>
<input type="button" id="search_fac" value=<?php xl('Search','e','\'','\''); ?>
 style='background-color:<?php echo $searchcolor; ?>' />
&nbsp;&nbsp;
<?php } ?>
...
<form action='new_comprehensive_save.php' name='agency form' method='post' onsubmit='return validate(this)'>

<span class='title'><?php xl('Search Agency','e'); ?></span>
...
while ($frow2 = sqlFetchArray($fres2)) {
  ...
  $this_group = $frow2['group_name'];
  $titlecols  = $frow2['titlecols'];
  $datacols   = $frow2['datacols'];
  $data_type  = $frow2['data_type'];
  $field_id   = $frow2['field_id'];
  $list_id    = $frow2['list_id'];
  $currvalue  = '';
  ...
  if (strcmp($this_group, $last_group) != 0) {
    ...
    if (!$SHORT_FORM) {
      ...
      $group_name = substr($this_group, 1);
      if (strlen($last_group) > 0) echo "<br />";
      echo "<span class='bold'><input type='checkbox' name='form_cb2_$group_seq' id='form_cb2_$group_seq' value='1' " .
        "onclick='return divclick(this,\"div2_$group_seq\");'";
      if ($display_style == 'block') echo " checked";
        
      // Modified 6-09 by BM - Translate if applicable  
      echo " /><b>" . xl_layout_label($group_name) . "</b></span>\n";
        
      echo "<div id='div2_$group_seq' class='section' style='display:$display_style;'>\n";
  ...
  if ($titlecols > 0) {
    end_cell2();
    echo "<td colspan='$titlecols'";
    echo ($frow2['uor'] == 2) ? " class='required'" : " class='bold'";
    if ($cell_count == 2) echo " style='padding-left:10pt'";
    echo ">";
    $cell_count += $titlecols;
  }
  ...
  if($field_id!="POSid")
  {
  if ($frow2['title']) echo (xl_layout_label($frow2['title']).":"); else echo "&nbsp;";
  }
  echo "</b>";
  ...
  ++$item_count;
  generate_form_field($frow2, $currvalue);
}
...
<br />
<?php if ($WITH_SEARCH) { ?>
<input type="button" id="search_agen" value=<?php xl('Search','e','\'','\''); ?>
 style='background-color:<?php echo $searchcolor; ?>' />
&nbsp;&nbsp;
<?php } ?>
...
// Set onclick/onfocus handlers for toggling background color.for facility

$lres1 = getLayoutRes1();
while ($lrow1 = sqlFetchArray($lres1)) {
  $field_id1  = $lrow1['field_id'];
  switch(getSearchClass($lrow1['data_type'])) {
    case 1:
        echo "    \$('#form_$field_id1').click(function() { toggleSearch(this); });\n";
      break;
    case 2:
      echo "    \$('#form_$field_id1').click(function() { selClick(this); });\n";
      echo "    \$('#form_$field_id1').blur(function() { selBlur(this); });\n";
      break;
 }
}
...
while ($lrow2 = sqlFetchArray($lres2)) {
  $field_id2  = $lrow2['field_id']; 
  switch(getSearchClass($lrow2['data_type'])) {
   case 1: 
      echo "    \$('#form_$field_id2').click(function() { toggleSearch(this); });\n";
      break;
    case 2:  
      echo "    \$('#form_$field_id2').click(function() { selClick(this); });\n";
      echo "    \$('#form_$field_id2').blur(function() { selBlur(this); });\n";
      break;
  }
}
?>
