...
<?php
  ...
  if ($display_style == 'block') echo " checked";
  echo " /><b>" . xl('Patient\'s Attributes') . "</b></span>\n";
  echo "<div id='div_8' class='section' style='display:$display_style;'>\n";
   /*$POSsql ="SELECT fg.id, fg.POS_id, fg.Recordset_ID, fg.Recordset_Name, fg.Table_ID, fg.Field_ID, pt.title, tb.Field_Name, tb.Table_Name
...
/*  $outerPOSSql = "SELECT pt.id,pt.title from tbl_pos_types pt 
                                INNER JOIN tbl_allcare_patients1to1_fieldmapping fg ON fg.POS_id=pt.id
                                group by pt.title order by pt.title
                  UNION
                  SELECT pt.id,pt.title from tbl_pos_types pt 
                                INNER JOIN tbl_allcare_patients1ton_fieldmapping fg1 ON fg1.POS_id=pt.id
                                group by pt.title order by pt.title               
                 ";
  */
  $outerPOSSql = "SELECT pt.id,pt.title
FROM tbl_pos_types pt
INNER JOIN tbl_allcare_patients1to1_fieldmapping fg ON fg.POS_id=pt.id
GROUP BY pt.title
...
GROUP BY pt.title";
           ...
           $outerPOSqry =  sqlStatement($outerPOSSql);
                                 
           echo "<ul id='posnavigation' class='treeview-red treeview'>";
           while ($outerPOSrow = sqlFetchArray($outerPOSqry)) 
           {
                
                //echo  "<li class='expandable'><div class='hitarea expandable-hitarea'></div><span>POS : ".$outerPOSrow['title']."</span>";
                echo  "<li class='expandable'><div class='hitarea expandable-hitarea'></div><span>Patient Attribute : ".$outerPOSrow['title']."</span>";
                                         
                /*
                  $mostinnerRecordsetSql ="SELECT fg.id, fg.POS_id, fg.Recordset_ID, fg.Recordset_Name, pt.title
from tbl_allcare_patients1ton_fieldmapping fg
INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id where POS_id=".$outerPOSrow['id']."
group by fg.Recordset_ID";
                  */
                  $mostinnerRecordsetSql ="SELECT fg1.id, fg1.POS_id, fg1.Grouping_ID, fg1.Grouping_Name,fg1.table_ID, pt.title
from tbl_allcare_patients1to1_fieldmapping fg1
INNER JOIN tbl_pos_types pt ON pt.id = fg1.POS_id where POS_id=".$outerPOSrow['id']."
group by fg1.Grouping_ID
UNION
SELECT fg.id, fg.POS_id, fg.Recordset_ID, fg.Recordset_Name,fg.table_ID, pt.title
from tbl_allcare_patients1ton_fieldmapping fg
INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id where POS_id=".$outerPOSrow['id']."
group by fg.Recordset_ID";
                                    
                 $mostinnerRecordsetqry =  sqlStatement($mostinnerRecordsetSql);
           
                 echo "<ul style='display: none;'>";
                 
                while ($mostinnerRecordsetrow = sqlFetchArray($mostinnerRecordsetqry)) 
                {
                                         
//           echo "<li class='collapsable'><div class='hitarea collapsable-hitarea'></div><span class=''>Recordset : ".$mostinnerRecordsetrow['Recordset_Name']."</span>";
echo "<li class='collapsable'><div class='hitarea collapsable-hitarea'></div>
    <span class=''>";
echo (($mostinnerRecordsetrow['table_ID']==1)?"Group":"Recordset").":".$mostinnerRecordsetrow['Grouping_Name']."</span>";
           
           /*
              $innerRecordsetSql ="SELECT fg.id, fg.POS_id, fg.Recordset_ID, fg.Recordset_Name, fg.Table_ID, fg.Field_ID, pt.title, tb.Field_Name, tb.Table_Name
FROM tbl_allcare_tablemeta tb
INNER JOIN tbl_allcare_patients1ton_fieldmapping fg ON tb.Field_ID = fg.Field_ID
INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id and fg.Recordset_ID=".$mostinnerRecordsetrow['Recordset_ID']."
group by fg.Recordset_ID,fg.Field_ID order by pt.title ";
              */
                            
              $innerRecordsetSql ="SELECT fg.id, fg.POS_id, fg.Grouping_ID, fg.Grouping_Name, fg.Table_ID, fg.Field_ID, pt.title, tb.Field_Name, tb.Table_Name
FROM tbl_allcare_tablemeta tb
INNER JOIN tbl_allcare_patients1to1_fieldmapping fg ON tb.Field_ID = fg.Field_ID
INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id 
and fg.Grouping_ID=".$mostinnerRecordsetrow['Grouping_ID']."    
UNION
SELECT fg.id, fg.POS_id, fg.Recordset_ID, fg.Recordset_Name, fg.Table_ID, fg.Field_ID, pt.title, tb.Field_Name, tb.Table_Name
FROM tbl_allcare_tablemeta tb
INNER JOIN tbl_allcare_patients1ton_fieldmapping fg ON tb.Field_ID = fg.Field_ID
INNER JOIN tbl_pos_types pt ON pt.id = fg.POS_id and fg.Recordset_ID=".$mostinnerRecordsetrow['Grouping_ID']." ";
                            
                $innerRecordsetqry =  sqlStatement($innerRecordsetSql);
                 
                echo "<ul style='display: block;'>";
                while ($innerRecordsetrow = sqlFetchArray($innerRecordsetqry)) 
                {
                    $textboxname =$innerRecordsetrow['Field_Name'];
  	 //   echo "<li><span>".$innerRecordsetrow['Field_Name']."</span></li>";
                  echo "<li><span>";
                  echo "<table><tr><td width=40% align=right><b>$textboxname :</b></td>";
                  echo "<td width=60% align=left> <input type='text' size='20' name='".$textboxname."' value=''/>";
                  echo "</td></tr></table></span></li>";
