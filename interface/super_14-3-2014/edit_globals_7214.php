  ...
  foreach ($grparr as $fldid => $fldarr) {
   if ( $_GET['mode'] != "user" || ($_GET['mode'] == "user" && in_array($fldid, $USER_SPECIFIC_GLOBALS)) ) {
    ...
    if (is_array($fldtype)) {
      ...
      echo "<table border='1' id='tbl_grid' name='tbl_grid'>
                <tr>
      ...
      echo "<select id='related_tables0' name='related_tables[]' onchange='javascript:return newEvt(this.id);' style='width:125px'>          
            <option value='none'>none</option>";      
      while ($row = sqlFetchArray($res)) 
      {
        echo "<option value='" . $row['Tables_in_openemr'] . "'>".xl($row['Tables_in_openemr'])."</option>";
      }

            echo "</select></td>
            <td><input type='text' id='txtFieldName0' name='txtFieldName[]' style='width:125px' /></td>                    
            <td>";
//                echo "<select id='data_types' name='data_types' onchange=''>
...
//                </select>";
                echo "<input type='text' id='txtFieldType0' name='txtFieldType[]' style='width:125px' />";
            echo "</td>
            <td><input type='text' id='txtDataLength0' name='txtDataLength[]' style='width:125px' /></td>
            <td align='center'>
                <input type='checkbox' id='boolFieldRequired0' name='boolFieldRequired[]' />
            </td>
            <td><input type='text' id='txtDefaultValue0' name='txtDefaultValue[]' style='width:125px' /></td>
        </tr>
        </table>
        <br>
        <center>        
            <input type='button' id='btnNext' name='btnNext' value='Next' 
            onclick='javascript:insertFields(document.theform.form_201.value,document.theform.form_201.form_202,
                                             document.theform.related_tables,
                                             document.theform.txtFieldName,
                                             document.theform.txtDataLength,
                                             document.theform.boolFieldRequired,
                                             document.theform.txtDefaultValue)' />
...
function receiveFromChild(tableName,checkedFields,checkedFieldsType)
{
    ...
    {      
        ...
        if(checkedFieldsType[i].indexOf('(') !== -1)  // datatype contains '('  e.g. varchar(100)
        {
            split0 = checkedFieldsType[i].split("(");
            split1=split0[0];
            split2 = split0[1].split(")");
        }
        else
        {
            split1 = checkedFieldsType[i];   
            split2='-';
        }
        ...
        if(i===0)
        {
            jQuery('#txtFieldName0').val(checkedFields[i]);            
            jQuery('#txtFieldType0').val(split1);   
            jQuery('#txtDataLength0').val(split2[0]);
        }
        else if(i>0)
        {
           addRow(i,tableName,checkedFields[i],split1,split2[0]);
        }
    }
}
...
function addRow(numRow,tableName,checkFieldName,checkFieldType,checkFieldSize)
{
	...
	var tbl=document.getElementById('tbl_grid');
	...
	var cell6=document.createElement('TD');	
        ...
        var cell0value='';
        cell0value+='<input type="checkbox" id="" name="" />';

        var cell1value='';
        cell1value+='<select name="related_tables'+numRow+'" id="related_tables'+numRow+'" onchange="javascript:return newEvt(this.id);" style="width:125px">\n\
                     <option value="none">none</option>\n\
                        <option value="'+tableName+'" selected>'+tableName+'</option>';                    
          <?php
$res1 = sqlStatement('SHOW TABLES'); 

while ($row = sqlFetchArray($res1)) 
{ ?>        
cell1value+='<option value="<?php echo $row['Tables_in_openemr'];?>" \n\
   ><?php echo $row['Tables_in_openemr'];?></option>';
<?php }
          ?>
      
        cell1value+='</select>';
        //cell1value+='<script>jQuery("#related_tables"'+numRow+').val("'+tableName+'");';
        
var cell2value='';
cell2value+='<input type="text" id="txtFieldName'+numRow+'" name="txtFieldName[]" style="width:125px" value="'+checkFieldName+'" />';

//        var cell3value='';
//        cell3value+='<select name="DataType'+numRow+'" id="DataType[]" onchange="">';
//        cell3value+='<option value="0">none</option>';
//        cell3value+='<option value="1">Number</option>';
//        cell3value+='<option value="2">Character</option>';
//        cell3value+='<option value="3">Text</option>';
//        cell3value+='<option value="4">Boolean</option>';
//        cell3value+='<option value="5">Date/Time</option>';
//        cell3value+='<option value="6">Binary</option>';
//        cell3value+='</select>';

var cell3value='';
cell3value+='<input type="text" id="txtFieldType'+numRow+'" name="txtFieldType[]" style="width:125px" value="'+checkFieldType+'" />';

var cell4value='';
cell4value+='<input type="text" id="txtDataLength'+numRow+'" name="txtDataLength[]" style="width:125px" value="'+checkFieldSize+'" />';

var cell5value='';        
cell5value+='<center><input type="checkbox" id="boolFieldRequired'+numRow+'" name="boolFieldRequired'+numRow+'" /></center>';       

var cell6value='';
cell6value+='<input type="text" id="txtDefaultValue'+numRow+'" name="txtDefaultValue[]" style="width:125px" />';

cell0.innerHTML=cell0value;
...
function insertFields(chk1to1,chk1ton,related_tables,FieldName,DataLength,FieldRequired,DefaultValue)
{
    var finalURL='addNewFields.php';  
        alert('tbl='+related_tables);
        alert('fn='+FieldName);
        ...
        $.ajax({
                type: 'POST',
		url: finalURL,	
		data: {chk1to1:chk1to1,chk1ton:chk1ton,related_tables:related_tables,FieldName:FieldName,
                        DataLength:DataLength,FieldRequired:FieldRequired,DefaultValue:DefaultValue},
