    ...
    {        
        foreach($FieldName as $key=>$value)   //foreach($relatedTables as $key=>$value)
        {
            if($value!='none')
            {
                ...
                $lastRecordFieldId=  sqlStatement('SELECT field_ID FROM tbl_allcare_tablemeta_insurance
                                                   WHERE id IN (SELECT MAX(id) FROM tbl_allcare_tablemeta_insurance)');
                
                //$lastInsertedFieldId=  sqlStatement('SELECT field_ID FROM tbl_allcare_tablemeta_insurance WHERE id='.mysql_insert_id().'');
                $lastFieldId=sqlFetchArray($lastRecordFieldId);
                
                $tableId=($tableName=='tbl_allcare_insurance1to1')?1:2;
                 
                $table_original=($relatedTables[$key]!='none') ? $relatedTables[$key] : '' ;
//                
//sqlStatement("INSERT INTO tbl_allcare_tablemeta_insurance(table_ID, table_Name,field_ID,field_Name,is_Field_Editable,created_Date) 
...
{
    insert_in_tables('tbl_allcare_insurance1to1');
}

if(isset($_POST['chk1ton']) && $_POST['chk1ton']==1)
{
    insert_in_tables('tbl_allcare_insurance1ton');
}
