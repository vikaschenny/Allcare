                ...
                <div class= "row">
                    <div class= "col-lg-12 col-sm-12 col-xs-12" style='padding-top:100px !important;'>
                        ...
                        // Element data seperator		
                            ...
                            $where  = "e.pc_pid IS NOT NULL AND e.pc_eventDate >= '$from_date'";
                                        ...
                                        DATE_FORMAT(e.pc_eventDate, '%Y-%m-%d') as dos,
                                              e.pc_facility,
                                               ...
                                               c.name as payer_name, 
                                                DATE_FORMAT(e.pc_eventDate, '%m-%Y') as month_value,(SELECT elig_verify_type FROM tbl_inscomp_custom_attr_1to1 WHERE insuranceid = i.provider) as verify_type
                                    FROM openemr_postcalendar_events AS e
                                    LEFT JOIN users AS d on (e.pc_aid is not null and e.pc_aid = d.id)
                                    LEFT JOIN facility AS f on (f.id = e.pc_facility)
                                    LEFT JOIN patient_data AS p ON p.pid = e.pc_pid
...
//                            $res3                   = sqlStatement($query3, $sqlBindArray);
                            ...
                            $query = "SELECT DISTINCT DATE_FORMAT(e.pc_eventDate, '%Y%m%d') as pc_eventDate,
                                               ...
                                               f.facility_npi,
                                               f.name as facility_name,
                                               c.name as payer_name,(SELECT ter.id FROM tbl_eligibility_response_data ter WHERE ter.pid= p.pid AND ter.month = DATE_FORMAT(e.pc_eventDate, '%%m-%%Y') LIMIT 1) as month_check,(SELECT elig_verify_type FROM list_options WHERE tbl_inscomp_custom_attr_1to1 WHERE insuranceid = i.provider) as verify_type,
