...
//      echo htmlspecialchars( xl('Provider Portal is turned off'), ENT_NOQUOTES);
//      exit;
//    }
...
<head>
	...
	function validate() {
            var clearPass=true;            
	    if (document.getElementById('authUser').value == "") {
		document.getElementById('authUser').style.border = "1px solid red";
                clearPass=false;
	    }
	    if (document.getElementById('clearPass').value == "") {
		document.getElementById('clearPass').style.border = "1px solid red";
                clearPass=false;
	    }
            return clearPass;
	}
        function process_new_clearPass() {

            if (!(validate_new_clearPass())) {
                alert ('<?php echo addslashes( xl('Field(s) are missing!') ); ?>');
                return false;
            }
            if (document.getElementById('clearPass_new').value != document.getElementById('clearPass_new_confirm').value) {
                alert ('<?php echo addslashes( xl('The new clearPassword fields are not the same.') ); ?>');
                return false;
            }
            if (document.getElementById('clearPass').value == document.getElementById('clearPass_new').value) {
                alert ('<?php echo addslashes( xl('The new clearPassword can not be the same as the current clearPassword.') ); ?>');
                return false;
            }
        }

        function validate_new_clearPass() {
            var clearPass=true;
            if (document.getElementById('authUser').value == "") {
                document.getElementById('authUser').style.border = "1px solid red";
                clearPass=false;
            ...
            if (document.getElementById('clearPass_new').value == "") {
                document.getElementById('clearPass_new').style.border = "1px solid red";
                clearPass=false;
            }
            if (document.getElementById('clearPass_new_confirm').value == "") {
                document.getElementById('clearPass_new_confirm').style.border = "1px solid red";
                clearPass=false;
            }
            return clearPass;
        }
...
<br><br>
	...
	<form action="get_provider_info.php" method="POST" onsubmit="return process()" >
		...
		<tr>
		    ...
		    <td><input name="authUser" id="authUser" type="text" autocomplete="off" /></td>
		</tr>
		<tr>
		    <td class="algnRight"><?php echo htmlspecialchars( xl('Password'), ENT_NOQUOTES);?></>
		    <td>
			<input name="clearPass" id="clearPass" type="password" autocomplete="off" />
		    </td>
		</tr>
