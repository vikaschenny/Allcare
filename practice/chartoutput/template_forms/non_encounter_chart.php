...
if($transid!=''){
    ...
    function patientdata_not_related_to_encounters($rowChartOutputTrans,$result1,$minValue,$groupName,$pid){
         $j = 0;
        foreach($result1 as $key => $value){

            $display_title = '';
            $get_display_field_label = sqlStatement("SELECT title FROM layout_options WHERE form_id = 'NONENC' AND field_id = '".$value."'");
                 ...
                 if ($rowChartOutputTrans[$value] == "YES"){
                    echo "<div id='show_div_med' style='display:none'>";
                            ...
                            echo "<div style='clear:both;'>";
                                ...
                                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                                echo "<ul class='1".$idName.$j."' >"; 
                                display_medications($pid,$groupName,$setpagebr_allr['layout_type']);
                                echo "<ul/></div>";
                            echo "</div>";
                     echo "</div>";
                 }
...
//        echo "No Insurance data for this patient.";
    ...
    function display_medications($pid,$groupName,$layout_type){
    global $tempvalue;
   ...
   /* ============================ */
    $getChartOutputTrans="SELECT *  FROM tbl_nonencounter_data
                          WHERE pid='$patientid'
                          AND id='$transid'";
    ...
    foreach($result as $key1 => $value1){
        $getselected = sqlStatement("SELECT $value1   FROM tbl_nonencounter_data WHERE pid='$patientid' AND $value1='YES' AND id='$transid'");
    ...
    for($i=0;$i<count($setselected);$i++){
        foreach($setselected as $key=>$value){
             foreach($value as $key=> $val){
                $getlayout_fields1 = sqlStatement("SELECT * FROM layout_options WHERE form_id = 'NONENC' AND uor > 0 AND field_id != ''  AND group_name='$group' AND field_id='$key'  ORDER BY seq");
    ...
    static_patient_data($group, $result1, $patientid, $minValue, $rowChartOutputTrans);
}
