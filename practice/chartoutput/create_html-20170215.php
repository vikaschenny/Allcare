...
if($_REQUEST['authorize']=='true'){
    ...
    if($patient_folderid==''){
        ...
        $curl = curl_init();
        $form_url = $protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/listAllFoldersmed/'.$row['notes'].'/'.$id.'/folder/Charts';
        curl_setopt($curl,CURLOPT_URL, $form_url);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
    ...
    if($med_folder!=''){
        $curl = curl_init();
        $form_url = 'https://'.$_SERVER['HTTP_HOST'].'/api/DriveSync/uploadHTMLWeb/'.$row['notes'].'/'.$page[0].'/'.trim($med_folder," ").'/'.$url; 
        curl_setopt($curl,CURLOPT_URL, $form_url);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
    ...
    $view='https://'.$_SERVER['HTTP_HOST'].'/interface/patient_file/summary/ggl_drive_folders.php?folder='.str_replace('https://drive.google.com/drive/folders/','',$row2['folder1']).'&category=patients&action=view';
    $ins_log1=sqlStatement("insert into DriveSync_log (date,user,email,encounter,patient_id,google_folder,file_name,file_id,status,watsID,category)values(now(),'".$_REQUEST['user']."','".$row['notes']."','".$_REQUEST['encounter']."','$patient_id','".$row2['folder1']."','".$_REQUEST['page_name']."','$resultant','file Created','','patient')");
