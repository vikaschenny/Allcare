...
function display_demographics($patient_id, $groupName){
    ...
    while($setgroupnames=sqlFetchArray($getgroupnames)){
        ...
        while($settitles=sqlFetchArray($gettitles)){
            ...
            if($idlist !=''):
                ...
                if($check == 1):
                    $getgroupval2 = sqlStatement("SELECT ". $idlist." FROM patient_data p LEFT JOIN employer_data e ON e.pid= p.pid WHERE e.pid = $patient_id ");
                else:
                    $getgroupval2 = sqlStatement("SELECT ".$getname. $idlist." FROM patient_data p WHERE pid = $patient_id ");
                endif;
...
{    
    ...
    foreach($priroty_type as $pri_val){
        $getInsuranceData="SELECT insd.type AS Priority,
                           ...
                           WHERE insd.pid='".$patient_id."' and insd.date < '$rowdos' and insd.type='$pri_val' order by insd.id desc limit 0,1";        
...
function display_cpo($encounterid,$groupName,$pid){
    global $tempvalue;
    ...
    $date3=sqlStatement("select cp.cpo_data, cp.provider_id  from tbl_form_cpo cp INNER JOIN form_encounter fe on fe.pid=cp.pid INNER JOIN forms f on fe.encounter = f.encounter AND f.form_id=cp.id  where cp.pid=".$pid." AND fe.encounter='$encounterid' AND form_name='cpo' and deleted=0 group by cp.id ");
...
ksort($keyarr); $j = 1;
$groupName = $group;
$geteid = sqlStatement("SELECT  fe.encounter,DATE_FORMAT(fe.date, '%Y-%m-%d') as date,openemr_postcalendar_categories.pc_catname FROM form_encounter AS fe left join openemr_postcalendar_categories on fe.pc_catid=openemr_postcalendar_categories.pc_catid  WHERE fe.pid = ".$patient_id." AND  DATE( date ) = '$date_of_service2' order by fe.date desc");
    ...
    foreach($result1Value as $key => $value){
        ...
        if($value=='form_mobile_allergy' || $value=='form_homehealth_allergy' || $value=='form_payeraudit_allergy' || $value=='form_referral_allergy' || $value=='form_appeal_allergy'){
            if($_REQUEST[$value] == "YES") {
                ...
                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                echo "<ul class='1".$idName.$j."' >"; 
                display_allergy(0,$setpagebr_allr['layout_type'],$groupName,$pid);
                echo "<ul/></div></div></div>";
            }
        }
        if($value=='form_mobile_surgery' || $value=='form_homehealth_surgery' || $value=='form_payeraudit_surgery' || $value=='form_referral_surgery' || $value=='form_appeal_surgery'){
            if($_REQUEST[$value] == "YES") {
                ...
                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                echo "<ul class='1".$idName.$j."' >"; 
                display_surgery(0,$setpagebr_allr['layout_type'],$pid);
                echo "<ul/></div></div></div>";
            }
        }
        if($value=='form_mobile_dental' || $value=='form_homehealth_dental' || $value=='form_payeraudit_dental' || $value=='form_referral_dental' || $value=='form_appeal_dental'){
            if ($_REQUEST[$value] == "YES"){
                ...
                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                echo "<ul class='1".$idName.$j."' >"; 
                display_dental(0,$setpagebr_allr['layout_type'],$pid);
                echo "<ul/></div></div></div>";
            }
         }
        if($value=='form_mobile_mproblem' || $value=='form_homehealth_mproblem' || $value=='form_payeraudit_mproblem' || $value=='form_referral_mproblem' || $value=='form_appeal_mproblem'){
            if ($_REQUEST[$value] == "YES"){
                ...
                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                echo "<ul class='1".$idName.$j."' >"; 
                display_mproblem(0,$pid,$groupName,$setpagebr_allr['layout_type']);
                echo "<ul/></div></div></div>";
            }
        }
        if($value=='form_mobile_dme' || $value=='form_homehealth_dme' || $value=='form_payeraudit_dme' || $value=='form_referral_dme' || $value=='form_appeal_dme'){
            if ($_REQUEST[$value] == "YES"){
                ...
                echo " <div id='1".$idName.$j."'  style='clear:both;'>"; 
                echo "<ul class='1".$idName.$j."' >"; 
                display_dme(0,$setpagebr_allr['layout_type'],$pid);
                echo "<ul/></div></div></div>";
            }
        }   
        if($value=='form_mobile_med' || $value=='form_homehealth_med' || $value=='form_payeraudit_med' || $value=='form_referral_med' || $value=='form_appeal_med'){
            if ($_REQUEST[$value] == "YES"){
                ...
                echo "<ul class='1".$idName.$j."' >"; 
                echo $patient_id;
                display_medications(0,$pid,$groupName,$setpagebr_allr['layout_type']);
                echo "<ul/></div></div></div>";
            }
        }  
...
function display_div_function($gname,$encounter,$pid){
    if($gname == 'Patient Information')
        ...
        echo "<div class='page' style='clear:both'>";
    //echo "SELECT CONCAT(fname,' ',lname) AS pname ,pid,DATE_FORMAT(DOB,'%m-%d-%Y') as DOB ,ss, providerID, street,city,state,country_code,postal_code FROM patient_data WHERE pid=".$pid."";
    ...
    $provider       = $resPatientName['providerID']; 
//    echo "SELECT f.name as faclityname, f.street, f.city, f.state, f.postal_code, f.country_code, f.email,f.website,f.fax, f.phone
//                                        FROM facility f
