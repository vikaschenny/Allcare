...
function display_insurance($patient_id,$rowChartOutputTrans) 
{    
    $datacheck = '';
    ...
    $priroty_type=array('primary','secondary','tertiary'); 
    foreach($priroty_type as $pri_val){
        $getInsuranceData="SELECT insd.type AS Priority,
                           ...
                           WHERE insd.pid='".$patient_id."' and insd.date < '".$rowChartOutputTrans['date_of_service']."' and insd.type='$pri_val' order by insd.id desc limit 0,1";        
...
function display_allergy($encounterid,$layout_type,$groupName,$patient_id){
    ...
    if($encounterid != 0){
       ...
       $allegrysqlquery .= " SELECT DISTINCT l.id, l.title AS Title, 
                                    ...
                                    FROM lists AS l
                                    WHERE l.type =  'allergy'
                                    AND l.pid =$patient_id AND l.id NOT IN ( SELECT DISTINCT list_id FROM issue_encounter WHERE pid =$patient_id ) 
                                    ORDER BY is_Issue_Active ASC , begdate DESC";
    }
...
function display_prescription($layout_type,$patient_id){
    global $tempvalue;
    $pressql = sqlStatement("SELECT  * FROM  `prescriptions` WHERE  `patient_id` =$patient_id order by active desc ,date_added desc ");
...
function display_ccm($encounterid,$groupName,$patient_id){
    global $tempvalue;
    ...
    $date3=sqlStatement("select cp.ccm_data, cp.provider_id  from tbl_form_ccm cp INNER JOIN form_encounter fe on fe.pid=cp.pid INNER JOIN forms f on fe.encounter = f.encounter AND f.form_id=cp.id  where cp.pid=".$patient_id." AND fe.encounter='$encounterid' AND form_name='ccm' and deleted=0 group by cp.id ");
    $fdate2=sqlFetchArray($date3);
    if(!empty($fdate2)){
        foreach($fdate2 as $key=> $value){
            if($key == 'ccm_data'){
...
if($who_type=='provider'){
    ...
    if($pro_res['specialty']!='') { echo "<tr><td><b>Specialty:</b></td><td>".$pro_res['specialty']."</td></tr>"; } 
    if($pro_res['phone']!='') { echo "<tr><td><b>Home Phone:</b></td><td>".$pro_res['phone']."</td></tr>"; } 
    if($pro_res['phonecell']!='') { echo "<tr><td><b>Mobile:</b></td><td>".$pro_res['phonecell']."</td></tr>"; } 
    if($pro_res['phonew1']!='') { echo "<tr><td><b>Work Phone:</b></td><td>".$pro_res['phonew1']."</td></tr>"; }
    if($pro_res['email']!='') { echo"<tr><td><b>Email:</b></td><td>".$pro_res['email']."</td></tr>"; } 
    if($pro_res['fax']!='') {  echo "<tr><td><b>Fax:</b></td><td>".$pro_res['fax']."</td></tr>"; } 
    if($pro_res['street']!='') { echo "<tr><td><b>Street:</b></td><td>".$pro_res['street']."</td></tr>"; } 
    if($pro_res['city']!='') { echo "<tr><td><b>City:</b></td><td>".$pro_res['city']."</td></tr>"; } 
    echo "</table>";
    echo "</td></tr>";
    echo "</table>";
    ...
    echo "<tr><td><h3> To</h3> </td></tr><tr><td>";
    echo "<table  border='0' style='padding-left:50px;'>";
    if($fres['name']!='')   { echo "<tr><td><b>Name:</b></td><td>".$fres['name']."</td></tr>"; }
    if($fres['phone']!='')  { echo "<tr><td><b>Mobile:</b></td><td>". $fres['phone']."</td></tr>"; } 
    if($fres['email']!='')  { echo "<tr><td><b>Email:</b></td><td>".$fres['email']."</td></tr>"; } 
    if($fres['fax']!='')    { echo "<tr><td><b>Fax:</b></td><td>".$fres['fax']."</td></tr>"; } 
    if($fres['street']!='') { echo "<tr><td><b>Street:</b></td><td>".$fres['street']."</td></tr>"; } 
    if($fres['city']!='')   { echo "<tr><td><b>City:</b></td><td>".$fres['city']."</td></tr>"; } 
    if($fres['state']!='')  { echo "<tr><td><b>State:</b></td><td>".$fres['state']."</td></tr>"; } 
    if($fres['postal_code']!='') { echo "<tr><td><b>Postal Code:</b></td><td>".$fres['postal_code']."</td></tr>"; } 
    if($fres['country_code']!=''){ echo"<tr><td><b>Country Code:</b></td><td>".$fres['country_code']."</td></tr>"; } 
    echo "</table>";
    echo "</td></tr>";
    echo "</table>";
    ...
    echo "<tr><td><h3> To</h3> </td></tr><tr><td>";
    echo "<table  border='0' style='padding-left:50px;'>";
    if($phres['name']!='') { echo"<tr><td><b>Name:</b></td><td>".$phres['name']."</td></tr>"; } 
    if($phres['email']!='') { echo "<tr><td><b>Email:</b></td><td>".$phres['email']."</td></tr>"; } 
    if($phres['city']!='') { echo "<tr><td><b>City:</b></td><td>".$phres['city']."</td></tr>"; } 
    if($phres['state']!='') { echo "<tr><td><b>State:</b></td><td>".$phres['state']."</td></tr>"; } 
    if($phres['zip']!='') {  echo "<tr><td><b>Postal Code:</b></td><td>".$phres['zip']."</td></tr>"; } 
    if($phres['country']!='') { echo "<tr><td><b>Country Code:</b></td><td>".$phres['country']."</td></tr>";} 
    echo "</table>";
    echo "</td></tr>";
    echo "</table>";
    ...
    echo "<tr><td><h3> To</h3> </td></tr><tr><td>";
    echo "<table  border='0' style='padding-left:50px;'>";
    if($ires['name']!='') { echo "<tr><td><b>Name:</b></td><td>".$ires['name']."</td></tr>"; }
...
while ($eidval = sqlFetchArray($geteid)){ 
        ...
        if($value=='mobile_ccm' || $value=='homehealth_ccm' || $value=='payeraudit_ccm' || $value=='referral_ccm' || $value=='appeal_ccm'){
            if($rowChartOutputTrans[$value] == "YES"){
                ...
                if(substr($setpagebr5['group_name'],1)=='CCM'){
                    ...
                    echo " <div id='".$idName.$j."'  style='clear:both;'>";   
                    echo "<ul class='".$idName.$j."' >";
                    display_ccm($eidval['encounter'],$groupName,$patient_id);
                    echo "<ul/></div></div>";
                    $i++;
                } 
                echo "</div>";
    ...
    if(!empty($eidval['elec_signedby'])  && !empty($eidval['elec_signed_on']) && $eidval['elec_signed_on'] != '0000-00-00 00:00:00'){
        $getSignImg="SELECT signature_image,  CONCAT(fname,' ', lname) as fname,
                (SELECT provider_credentials FROM tbl_patientuser where userid = '".$eidval['elec_signedby']."' LIMIT 1) as designation
                FROM users
                WHERE id='".$eidval['elec_signedby']."' and username!=''";
        $resSignImg=sqlStatement($getSignImg);
        $rowSignImg=sqlFetchArray($resSignImg);
        $newval="../../interface/pic/user_sign/".$rowSignImg['signature_image'];
                    ...
                    if($rowChartOutputTrans[$value] == "YES" ){
                        ...
                        if(sqlNumRows($result)==0){
                            ...
                            <script>
                                ...
                                obj.push({"id":"show_div_immunizations", "value":"none"});  
                            </script> 
                            <?php
                        }else{
        ...
        if($value=='mobile_insurance' || $value=='homehealth_insurance' || $value=='payeraudit_insurance' || $value=='referral_insurance' || $value=='appeal_insurance'){  
            if ($rowChartOutputTrans[$value] == "YES"){
                ...
                if($key!=$minValue){
                    ...
                    display_insurance($pid,$rowChartOutputTrans);
                    echo "</div>"; 
                }else { 
            ...
            if (response.indexOf('$') > - 1){
                ...
                if (confirm(res[1]) == true) {
                     ...
                     $.ajax({
                        type: 'POST',
                        url: "create_html.php",
                        data:{data:drivehtml,  page_name:page, pid:<?php echo $patient_id; ?>, authorize:true, encounter:'<?php echo $encounter_id; ?>', modify:1,transid:'<?php echo $trans_form_id; ?>',user:'<?php echo $_REQUEST['provider'];?>'},
                        success: function(response)
                        {
                            var res = response.split('@');
                     ...
                     $.ajax({
                        type: 'POST',
                        url: "create_html.php",
                        data:{data:drivehtml,  page_name:page, pid:<?php echo $patient_id; ?>, authorize:true, encounter:'<?php echo $encounter_id; ?>', modify:0,transid:'<?php echo $trans_form_id; ?>',user:'<?php echo $_REQUEST['provider'];?>'},
                        success: function(response)
                        {
                            var res = response.split('@');
