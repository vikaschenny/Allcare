...
// Print Heading .. to have better Understanding of the Listed Transactions -- starts here Dec 07,09
	 print "<tr class='showborder_head'><th style='width=10px;'>&nbsp;</th><th style='width=10px;'>&nbsp;</th><th style='width=10px;'>&nbsp;</th>";
		...
		// Print Heading .. to have better Understanding of the Listed Transactions   -- ends here
          $res = sqlStatement("select * from tbl_patientfacility where patientid=? order by facility_admitdate DESC , facility_dischargedate DESC", array($pid) );
  for ($iter = 0; $row = sqlFetchArray($res); $iter++) 
        $all[$iter] = $row;

   foreach ($all as $iter) {
                echo "<tr height='25'><td>";
		print "<a href='service_facility.php?id=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
			"&inmode=edit' class='css_button_small'><span>".
			htmlspecialchars( xl('Edit'), ENT_NOQUOTES)."</span></a>";
		echo "</td><td>";
		/*
                 if (acl_check('admin', 'super')) {
			echo "<a href='../deleter.php?id=".
				htmlspecialchars( $iter{"id"}, ENT_QUOTES).
				"' class='css_button_small'><span>".
		...
		echo "</td>";
			...
			foreach($field_id as $attr){
                            $fres2=sqlStatement("SELECT data_type FROM layout_options WHERE form_id = 'SERVICEFAC' AND uor > 0 AND field_id = '$attr'");
