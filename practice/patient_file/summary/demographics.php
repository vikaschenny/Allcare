...
<script type="text/javascript" language="JavaScript">
 ...
 function oldEvt(eventid) {
  window.open('../../calendar/add_edit_event.php?eid=' + eventid, '', 'width=550, height=350');
 }
...
?>
 ...
 function newEvt() {
  window.open('../../calendar/add_edit_event.php?patientid=<?php echo htmlspecialchars($pid,ENT_QUOTES); ?>',  '', 'width=550, height=350');
  return false;
 }
...
<?php
 ...
 if ($thisauth) {
  ...
  if($GLOBALS['erx_enable']){
	echo '<td style="padding-left:1em;"><a class="css_button" href="../../eRx.php?page=medentry" >';
	echo "<span>".htmlspecialchars(xl('NewCrop MedEntry'),ENT_NOQUOTES)."</span></a></td>";
	echo '<td style="padding-left:1em;"><a class="css_button iframe1" href="../../soap_functions/soap_accountStatusDetails.php" >';
  ...
  if($GLOBALS['portal_onsite_enable'] && $GLOBALS['portal_onsite_address']){
    ...
    if ($portalStatus['allow_patient_portal']=='YES') {
      ...
      echo "<td style='padding-left:1em;'><a class='css_button iframe small_modal' href='create_portallogin.php?portalsite=on&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' >";
  ...
  if($GLOBALS['portal_offsite_enable'] && $GLOBALS['portal_offsite_address']){
    ...
    if ($portalStatus['allow_patient_portal']=='YES') {
      ...
      echo "<td style='padding-left:1em;'><a class='css_button iframe small_modal' href='create_portallogin.php?portalsite=off&patient=" . htmlspecialchars($pid,ENT_QUOTES) . "' >";
      if (empty($portalLogin)) {
...
|
<a href="../summary/add_facility.php?active=all" class='iframe large_modal' >
<?php echo htmlspecialchars(xl('Service Facility'),ENT_NOQUOTES); ?></a>
|
<a href="../summary/add_agency.php?active=all" class='iframe large_modal' > 
<?php echo htmlspecialchars(xl('Agencies'),ENT_NOQUOTES); ?></a>
|
<a href="../summary/add_chartoutput.php?active=all" class='iframe large_modal' >
<?php echo htmlspecialchars(xl('Medical Record'),ENT_NOQUOTES); ?></a>
|
<a href="../../reports/patient_full_encounters_single_view.php?pid=<?php echo $pid; ?>" class='iframe large_modal' >
<?php echo htmlspecialchars(xl('Charts'),ENT_NOQUOTES); ?></a>
  </td>
 </tr>
</table> <!-- end header -->
...
if ( $insurance_count > 0 ) {
  ...
  $widgetButtonLabel = xl("Edit");
  $widgetButtonLink = "insurancedata_full.php";
  $widgetButtonClass = "";
  $linkMethod = "html";
  $bodyClass = "";
  $widgetAuth = acl_check('patients', 'demo', '', 'write');
...
?>
        ...
        <ul class="tabNav"><?php
						...
						$enddate = 'Present';
						  ...
						  while( $row = sqlFetchArray($res) ) {
							if ($row['provider'] ) {
								...
								?>
								<li <?php echo $first ? 'class="current"' : '' ?>><a href="patient_file/summary/play/javascript-tabbed-navigation/">
					...
					echo "<li><a href='patient_file/summary/play/javascript-tabbed-navigation/'>" .
