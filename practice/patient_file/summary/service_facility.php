...
require_once("$srcdir/service_facility_lib.php");
...
if ($mode) {   
 ...
 //$sets1 = "facilityid =?, facility_admitdate = ?, facility_dischargedate = ?, facility_isactive = ?,facility_notes = ?, facility_related_links = ?";
 //$sqlBindArray = array($_POST['form_facilityid'], $_POST['form_facility_admitdate'], $_POST['form_facility_dischargedate'], $_POST['form_facility_isactive'],$_POST['form_facility_notes'],$_POST['form_facility_related_links']);
 /* $sqlBindArray=array();
  ...
  //print_r($_SESSION);print_r($_POST);
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_patientfacility SET $sets1 WHERE id = ?", $sqlBindArray);
  }
  else {
    //use sql placemaker
    ...
    $sets .= " patientid = ?";
    $id = sqlInsert("INSERT INTO tbl_patientfacility SET $sets", $sqlBindArray);
...
if($id)
{ 
  updateServiceFac($id, $newdata['tbl_patientfacility'] ,$create=false);
}
else
{  
    updateServiceFac($id, $newdata['tbl_patientfacility'],$create=true);

}  
  if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='add_facility.php';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='add_facility.php';";
}
...
// onclick='return deleteme()'
 dlgopen('../deleter.php?id=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>', '_blank', 500, 450);
 return false;
}
...
function imdeleted() {
 top.restoreSession();
 location.href = 'summary/add_facility.php';
}
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('SERVICEFAC'); ?>
...
function submitme() {
 var f = document.forms['new_facility'];
 if (validate(f)) {
  f.submit();
 }
}
...
<form name='new_facility' method='post' action='service_facility.php?id=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                <a href="add_facility.php"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
...
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'SERVICEFAC' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
