...
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept,$encounter=0,$copay=0,$patcopay=0,$getdeduct=0,$billingNote) {
  ...
  echo "  <td class='detail' id='td_billingnote_$var_index' align='center'>" . htmlspecialchars($billingNote, ENT_QUOTES) . "</td>\n";
...
<table border='0' id="table_display" cellpadding='0' cellspacing='0' width='635'>
 <tr bgcolor="#cccccc" id="tr_head">
  ...
  <td class="dehead" width="65">
   <?php echo htmlspecialchars( xl('Billing Note'), ENT_QUOTES) ?>
  </td>
...
<?php
  ...
  $gottoday = false;
  foreach ($encs as $key => $value) {
    $billingNote =   $value['billing_note'];  
    $enc = $value['encounter'];
    $dispdate = $value['date'];
    if (strcmp($dispdate, $today) == 0 && !$gottoday) {
      $dispdate = date("Y-m-d");
