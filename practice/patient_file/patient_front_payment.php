...
//
$var_index=0;
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept,$encounter=0,$copay=0,$patcopay=0,$getdeduct=0,$billingNote) {
  global $var_index;
  if($billingNote == "") $billingNote="--NIL--";
  $var_index++;
  ...
  echo "  <td class='detail' id='td_billingnote_$var_index' align='center'><span 'lbl_".attr($var_index)."' class='editable' data-encounterid='".$encounter."'>" . htmlspecialchars($billingNote, ENT_QUOTES) . "</span></td>\n";
  ...
  echo "  <td class='detail' align='center' id='td_deduct_$var_index' >" . htmlspecialchars(bucks($getdeduct), ENT_QUOTES) . "</td>\n";
...
function getDeductible($patient_id, $encounter) {
        $resMoneyDeduct = sqlStatement("SELECT memo FROM ar_activity where ".
          "pid = ? and encounter = ? and payer_type=0 and account_code='Deduct' ORDER BY post_time DESC LIMIT 1",
          array($patient_id,$encounter));
        $rowPayment = sqlFetchArray($resMoneyDeduct);
			...
			if($_REQUEST['radio_type_of_payment']=='copay')//copay saving to ar_session and ar_activity tables
			 {
				$session_id=idSqlStatement("INSERT INTO ar_session (payer_id,user_id,reference,check_date,deposit_date,pay_total,".
				 ...
				 array($_SESSION['authId'],$form_source,$amount,$form_pid,$form_method));
				  ...
				  $insrt_id=idSqlStatement("INSERT INTO ar_activity (pid,encounter,code_type,code,modifier,payer_type,post_time,post_user,session_id,pay_amount,account_code,reason_code)".
				   " VALUES (?,?,?,?,?,0,now(),?,?,?,'PCP',3)",
				 ...
				 frontPayment($form_pid, $enc, $form_method, $form_source, $amount, 0, $timestamp);//insertion to 'payments' table.
                         ...
                         if($_REQUEST['radio_type_of_payment']=='deduct')//copay saving to ar_activity table
			 {
                                $deductible = 'Deductable $'.$amount;
                                $session_id=idSqlStatement("INSERT INTO ar_session (payer_id,user_id,reference,check_date,deposit_date,pay_total,".
				 " global_amount,payment_type,description,patient_id,payment_method,adjustment_code,post_to_date) ".
				 " VALUES ('0',?,?,now(),now(),?,'','patient','Deduct',?,?,'patient_payment',now())",
				 array($_SESSION['authId'],$form_source,$amount,$form_pid,$form_method));
                                
				$insrt_id=idSqlStatement("INSERT INTO ar_activity (pid,encounter,code_type,code,modifier,payer_type,post_time,post_user,session_id,memo,account_code,reason_code)".
				   " VALUES (?,?,?,?,?,0,now(),?,?,?,'Deduct',1)",
					 array($form_pid,$enc,$Codetype,$Code,$Modifier,$_SESSION['authId'],$session_id,$deductible));
...
$(".editable").each(function () {
    ...
    label.after("<textarea style = 'display:none'></textarea>");
    ...
    textbox.focusout(function () {
        ...
        $(this).prev().show();
        $.ajax({
            url: "updatebillnote.php",
            type: "POST",
            data:{encId:$(this).prev().data('encounterid'),bnote:$(this).val()},
            success:function(res){console.log(res);},
            error:function(err){console.log("Update failed due to: "+ $.parseJSON(err));}
        });
    });
});
});    
...
function make_it_hide_enc_pay()
 {
  	...
  	document.getElementById('td_head_co_pay').style.display="none";
        document.getElementById('td_head_deduct').style.display="";
...
<table border='0' cellspacing='0' cellpadding="0">
 ...
 <tr id="tr_radio2"><!-- For radio self -->
  ...
  <td class='text' colspan="2" >
      <input type="radio" name="radio_type_of_payment" id="radio_type_of_payment1" value="copay" checked="checked" onClick="make_visible_row();cursor_pointer();"/><?php echo htmlspecialchars(xl('Co Pay'), ENT_QUOTES); ?>
      <input type="radio" name="radio_type_of_payment" id="radio_type_of_payment3" value="deduct" onClick="make_visible_row();cursor_pointer();"/><?php echo htmlspecialchars(xl('Deductible'), ENT_QUOTES); ?>
      <input type="radio" name="radio_type_of_payment" id="radio_type_of_payment2" value="invoice_balance"  onClick="make_visible_row();"/><?php echo htmlspecialchars(xl('Invoice Balance'), ENT_QUOTES); ?>
      <br/><input type="radio" name="radio_type_of_payment" id="radio_type_of_payment4" value="pre_payment" onClick="make_hide_row();"/><?php echo htmlspecialchars(xl('Pre Pay'), ENT_QUOTES); ?></td>
 </tr>
...
<table border='0' id="table_display" cellpadding='0' cellspacing='0' width='890'>
 <tr bgcolor="#cccccc" id="tr_head">
  ...
  <td class="dehead" width="65">
   <?php echo htmlspecialchars( xl('Billing Note'), ENT_QUOTES) ?> <span style="color:red;font-size:10px;"><i>Click to edit</i></span>
  </td>
  ...
  <td class="dehead" align="center" width="55" id="td_head_deduct" >
   <?php echo htmlspecialchars( xl('Deductible'), ENT_QUOTES) ?>
  </td>
...
<?php
  ...
  //
  $query = "SELECT fe.encounter,fe.billing_note, b.code_type, b.code, b.modifier, b.fee, " .
  ...
  //
  $query = "SELECT fe.encounter,fe.billing_note, s.drug_id, s.fee, " .
    "LEFT(fe.date, 10) AS encdate,fe.last_level_closed " .
 ...
 //------------------------------------------------------------------------------------
	...
	$patcopay = getPatientCopay($pid, $enc);
      $getdeduct = getDeductible($pid, $enc);
//Insurance Payment
//-----------------
...
//---------------
 ...
 //------------------------------------------------------------------------------------
    ...
    // If there is patient balance then show the line   
    // Removed check on 20170509
    //if(round($duept,2) != 0 || $inscopay != 0):
        echoLine("form_upay[$enc]", $dispdate, $value['charges'],$dpayment_pat, ($dpayment + $dadjustment), $duept,$enc,$inscopay,$patcopay,$getdeduct,$billingNote);
    //endif;     
