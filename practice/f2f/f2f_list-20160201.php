...
//                           <span>".htmlspecialchars( xl('Delete'), ENT_NOQUOTES)."</span></a>";
                           echo "<a class='various' data-fancybox-type='iframe' href='chartoutput/deleter.php?f2fid=".
                                    htmlspecialchars( $iter{"id"}, ENT_QUOTES).
                                   "&location=provider_portal&patient_id=$pid&provider=$provider&trans=f2f' style='background-color:#49C1DC;
                ...
                <?php  foreach($field_id as $attr)
                        { 
                         if($attr=='refer_to'){
                                ...
                                $users1=sqlStatement("select organization from users where id=$iter[$attr]");
                                $res2=sqlFetchArray($users1);
                                if(!empty($res2)){
                                  echo " <td style='width:200px;'>&nbsp;&nbsp;&nbsp;&nbsp;" .
                                   htmlspecialchars( ($res2{organization}), ENT_NOQUOTES). "&nbsp;</td>";
                                }else {
                                  $users=sqlStatement("select concat(lname,'',fname) as name from users where id=$iter[$attr]");
                                  $res=sqlFetchArray($users);
                                   echo " <td style='width:200px;'>&nbsp;&nbsp;&nbsp;&nbsp;" .
                                   htmlspecialchars( ($res{name}), ENT_NOQUOTES). "&nbsp;</td>";
                                }
                            }else if($attr=='date_of_service'){
                                $ex=explode(" ",$iter{$attr});
                                 echo " <td style='width:150px;'>" .
                                htmlspecialchars( ($ex[0]), ENT_NOQUOTES). "&nbsp;</td>";
                            }else {
                                echo " <td style='width:150px;'>" .
