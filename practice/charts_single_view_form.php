...
if(isset($_SESSION['portal_username']) !=''){
    //$provider    = $_SESSION['portal_username'];
    $provider = $refer = $_SESSION['refer'] = $_GET['refer'];
}else {
    //$provider                    = $_REQUEST['provider'];
    $provider = $refer = $_SESSION['refer'] = $_GET['refer'];
    $_SESSION['portal_username'] = $_REQUEST['provider'];
...
<script>
    var targetid;
    function datafromchildwindow(id,formid,status) {
        var target = $('#'+targetid+" a");
        var url = target.attr("href");
        var urlpermstart = target.attr("href").indexOf('?');
        var splitstring = (url.slice(urlpermstart+1,url.length)).split("&");
        var rebuildurl="";
        for(var i = 0; i<splitstring.length; i++){
            if(splitstring[i]=="id=0")
               rebuildurl += splitstring[i].split("=")[0]+"="+id+"&";
            else if(splitstring[i]=="formid=0" && formid != undefined)
               rebuildurl += splitstring[i].split("=")[0]+"="+formid+"&";
            else if(splitstring[i].split("=")[0]+"=" == 'status=')
                rebuildurl += splitstring[i].split("=")[0]+"="+status+"&";
            else
               rebuildurl += splitstring[i]+"&";
        }
        target.attr("href",url.slice(0,urlpermstart+1)+rebuildurl.slice(0,rebuildurl.length-1));
        if(formid !=0 && formid != undefined){
            $("#lbf_form_id").val(formid)
            updateallformids(formid);
       }
            
    }
    function updateallformids(newformid){
        $(".newwindow a").each(function(){
            var url = $(this).attr("href")
            var newurl = url.replace("formid=0",("formid="+newformid))
...
//        if(substr($fuv_row1['screen_group'],1) == 'Dictation'){
            ...
            $visitcategory_id   = $fuv_row1['visitcategory_id'];
            $dos                = $fuv_row1['dos'];
...
//    echo "<br />";
    ...
    $grpm   = sqlStatement("select * from tbl_visitcat_chartgrp_mapping where visit_category=$visitcategory_id");
    ...
    if($grpres['chart_group'] != ''){ 
        ...
        $datastring     = $res.'patientid'.'='.$pid.'&'.'encounter_id'.'='.$encounter.'&'.'dos'.'='.$dos.'&'.'chartgroupshidden'.'='.$grpres['chart_group']; ?>
        <div style="text-align: center;"><a href='javascript:; ' onclick="window.open('chartoutput/preview_charts.php?'+'<?php echo $datastring; ?>','popup','width=900,height=900,scrollbars=no,resizable=yes');" title='<?php echo substr($grpres['chart_group'],1); ?>' class='btn btn-info btn-md'><span><?php echo  htmlspecialchars( xl('Preview'), ENT_NOQUOTES); ?></span></a></div> 
       <?php 
    }else { ?>
        <div style="text-align: center;"><a class='btn btn-info btn-md' data-toggle='modal' data-target='#preview' data-urlhref = "chartoutput/preview_group.php?enc=<?php echo $encounter; ?>&pid=<?php echo $pid; ?>&date=<?php echo $dos; ?>&grp=0" href="#"><span><?php echo  htmlspecialchars( xl('Preview'), ENT_NOQUOTES); ?></span></a></div>
   <?php } 
    ?>
    ...
    $get_audit_dropdown = sqlStatement("SELECT * FROM form_encounter f INNER JOIN users u 
                                        ON f.rendering_provider = u.id WHERE f.encounter = '$encounter'");
    while($set_audit_dropdown = sqlFetchArray($get_audit_dropdown)){
        $audit_dropdown = $set_audit_dropdown['audited_status'];
        $fullname = $set_audit_dropdown['fname']. " " . $set_audit_dropdown['mname'] . " " . $set_audit_dropdown['lname'];
    }
    if($audit_dropdown != 'Completed') $audit_dropdown = 'Incomplete';
    echo "<span style='font-size:15px;'><b>Rendering Provider: </b>".$fullname."</span><br />";
    echo "<span style='font-size:15px;'><b>Audit Status: </b>".$audit_dropdown."</span><br /><br />";
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('Issues/stats_full_custom.php?active=all&category=$grouptitle&encounter=$encounter&pid=$pid&formid=$lbf_form_id&location=provider_portal&provider=$provider&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$grouptitle'><a href='Issues/stats_full_custom.php?active=all&category=$grouptitle&encounter=$encounter&pid=$pid&formid=$lbf_form_id&location=provider_portal&provider=$provider&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
            ...
            <div id='f2fdiv'>
                ...
                $fres = sqlStatement("SELECT * FROM layout_options WHERE form_id IN('HIS','LBF2') AND field_id IN($history_required_fields,$required_fields) AND ((group_name = '$groupname') OR (group_name LIKE '%$group'))  ORDER BY group_name, seq");
                $last_group = '';
                $cell_count = 0;
                $item_count = 0;
                $display_style = 'block';
        ...
        <?php    
            ...
            if($group != 'auditform')
                ...
                $checkstring = '';
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new_custom.php?encounter=$encounter&pid=$pid&id=$static_form_id".$checkstring."&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new_custom.php?encounter=$encounter&pid=$pid&id=$static_form_id".$checkstring."&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new.php?encounter=$encounter&pid=$pid&id=$static_form_id&formid=$lbf_form_id&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new.php?encounter=$encounter&pid=$pid&id=$static_form_id&formid=$lbf_form_id&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
        else if($group == 'codes'){
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('codes/feesheet_custom.php?pid=$pid&encounter=$encounter&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='codes/feesheet_custom.php?pid=$pid&encounter=$encounter&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
        else if($group == 'ros'){
            $group = "allcare_ros";
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/view_custom.php?formname=$group&encounter=$encounter&pid=$pid&id=$ros_form_id&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".$form_name."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/view_custom.php?formname=$group&encounter=$encounter&pid=$pid&id=$ros_form_id&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>".$form_name."</span></a></div>";
        }
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new_custom.php?formname=$group&edit=custom_pe&encounter=$encounter&pid=$pid&id=$allcare_physical_exam_form_id&menu_val=&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new_custom.php?formname=$group&edit=custom_pe&encounter=$encounter&pid=$pid&id=$allcare_physical_exam_form_id&menu_val=&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/view_custom.php?formname=$formname&encounter=$encounter&pid=$pid&id=$vitals_form_id&&status=$status&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/view_custom.php?formname=$formname&encounter=$encounter&pid=$pid&id=$vitals_form_id&&status=$status&provider=$provider&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
//        else{
