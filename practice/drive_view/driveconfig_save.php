...
$ignoreAuth=true; // ignore the standard authentication for a regular OpenEMR user
 ...
 foreach($result as $val1){
     ...
     if($result1[0]=='patient-folder'){
         ...
         $practice=$result1[1];
     }else if($result1[0]=='email-parent-folder'){
         $email_attach=$result1[1];
     }else if($result1[0]=='imap-user'){
         $imap_user=$result1[1];
     }else if($result1[0]=='imap-pwd'){
         $imap_pwd1=$result1[1];
     }else if($result1[0]=='to_user'){
         $to_users=serialize($result1[1]);
     ...
     }else if($result1[0]=='upload_user[]'){
         $upload_user['user']=serialize($result1[1]);
     }else if($result1[0]=='upload_text'){
         $upload_user['text']=$result1[1];
     }else if($result1[0]=='upload_attach'){
         $upload_user['attach']=$result1[1];
     }else if($result1[0]=='download_user[]'){
         $download_user['user']=serialize($result1[1]);
     }else if($result1[0]=='download_text'){
         $download_user['text']=$result1[1];
     }else if($result1[0]=='download_attach'){
         $download_user['attach']=$result1[1];
     }else if($result1[0]=='store_user[]'){
         $store_user['user']=serialize($result1[1]);
     }else if($result1[0]=='store_text'){
         $store_user['text']=$result1[1];
     }else if($result1[0]=='move_user[]'){
         $move_user['user']=serialize($result1[1]);
     }else if($result1[0]=='move_text'){
         $move_user['text']=$result1[1];
     }else if($result1[0]=='scan-parent-folder'){
         $scan_folder=$result1[1];
     }else if($result1[0]=='scan-medical-charts'){
         $scan_charts=$result1[1];
     }else if($result1[0]=='scan-medical-cpo'){
         $scan_cpo=$result1[1];
     }
 }

$upload=serialize($upload_user);
$download=serialize($download_user);
$store=serialize($store_user);
$move=serialize($move_user);
 $patient_query='';
 if($patientfolder!=''){
     ...
     $col1=trim($col,','.'"_"'.',');
     $patient_query="select CONCAT($col1) as Patient_folder from patient_data";
 ...
 if($providerfolder!=''){
     ...
     $pcol1=trim($pcol,','.'"_"'.',');
     $provider_query="select CONCAT($pcol1) as provider_folder from users where (fname<>'' or lname<>'') and username<>''";
 }
 if($patientfile!=''){ 
     ...
     $pfcol1=trim($pfcol,','."'_'".',');
     if($dos==1){
         $patient_file_query='"'."select CONCAT($pfcol1) as patient_file from patient_data p INNER JOIN  tbl_form_chartoutput_transactions c  ON c.pid=p.pid".'"';
     }else{
         $patient_file_query='"'."select CONCAT($pfcol1) as patient_file from patient_data p".'"';
     }
...
if($providerfolder!='' || $patientfolder!='' || $patientfile!='' || $insfolder!='' || $facfolder!='' || $phfolder!=''){
      ...
      if(empty($sel_row)){
            $imap_pwd= GibberishAES::enc($imap_pwd1, 'rotcoderaclla');
            //base64_encode($imap_pwd1);
            $sql=sqlStatement("insert into tbl_drivesync_authentication (date,email,status,user,patient_folder,provider_folder,patient_folder_format,provider_folder_format,patient_file,patient_file_format,parent_folder,user_parent_folder,ins_parent_folder,insurance_folder_format,insurance_folder_query,
                               pharmacy_parent_folder,pharmacy_folder_format,pharmacy_folder_query,facility_parent_folder,facility_folder_format,facility_folder_query,addrbk_parent_folder,org_folder_format,org_folder_query,patient_folder_trigger,practice_parent_folder,ins_folder_trigger,pharmacy_folder_trigger,addrbk_folder_trigger,email_parent_folder,imap_email,imap_pwd,pwd_encrypt,email_to_users,facility_folder_trigger,provider_folder_trigger,patient_sub_folder,provider_sub_folder,insurance_sub_folder,pharmacy_sub_folder,addrbk_sub_folder,facility_sub_folder,upload_settings,download_settings,store_email_settings,movefile_settings,scan_parent_folder,scan_medical_charts,scan_medical_cpo)values(now(),'$email','Authenticated','".$_SESSION['authUser']."','".$patient_query."','".addslashes($provider_query)."','".$patientfolder."','".$providerfolder."',".$patient_file_query.",'".$patientfile."','".$parfolder."','".$usrparfolder."','".$insparfolder."','".$insfolder."','".$ins_query."','".$phparfolder."','".$phfolder."','".$ph_query."','".$facparfolder."','".$facfolder."','".$fac_query."','".$addrparfolder."','".$orgfolder."','".$org_query."','".$autotriggerr."','".$practice."','$insautotriggerr','$phautotriggerr','$addrautotriggerr','$email_attach','$imap_user','$imap_pwd',1,'$to_users','$facautotriggerr','$userautotriggerr','$psubfolder','$prosubfolder','$inssubfolder','$phsubfolder','$addrsubfolder','$facsubfolder','$upload','$download','$store','$move','$scan_folder','$scan_charts','$scan_cpo')");
      }else {
          ...
          if($sel_row['imap_pwd']!=$imap_pwd1){
              //echo $sel_row['imap_pwd']."++".$imap_pwd1;
           // $imap_pwd= base64_encode($imap_pwd1); 
               $imap_pwd= GibberishAES::enc($imap_pwd1, 'rotcoderaclla');
          }else{
              $imap_pwd=$imap_pwd1;
         ...
         //echo "update tbl_drivesync_authentication set date=now(),status='updated',user='".$_SESSION['authUser']."',patient_folder='".$patient_query."',provider_folder='".addslashes($provider_query)."',patient_folder_format='".$patientfolder."',provider_folder_format='".$providerfolder."' ,patient_file=".$patient_file_query.", patient_file_format='".$patientfile."' , parent_folder='".$parfolder."',user_parent_folder='".$usrparfolder."' ,ins_parent_folder='".$insparfolder."', insurance_folder_format='".$insfolder."', insurance_folder_query='".$ins_query."',pharmacy_parent_folder='".$phparfolder."',pharmacy_folder_format='".$phfolder."',pharmacy_folder_query='".$ph_query."',facility_parent_folder='".$facparfolder."',facility_folder_format='".$facfolder."',facility_folder_query='".$fac_query."',addrbk_parent_folder='".$addrparfolder."',org_folder_format='".$orgfolder."',org_folder_query='".$org_query."',patient_folder_trigger='".$autotriggerr."',practice_parent_folder='".$practice."',ins_folder_trigger='$insautotriggerr',pharmacy_folder_trigger='$phautotriggerr',addrbk_folder_trigger='$addrautotriggerr',email_parent_folder='$email_attach',imap_email='$imap_user',imap_pwd='$imap_pwd',pwd_encrypt='1' where email='".$row['notes']."'";
            $sql=sqlStatement("update tbl_drivesync_authentication set date=now(),status='updated',user='".$_SESSION['authUser']."',patient_folder='".$patient_query."',provider_folder='".addslashes($provider_query)."',patient_folder_format='".$patientfolder."',provider_folder_format='".$providerfolder."' ,patient_file=".$patient_file_query.", patient_file_format='".$patientfile."' , parent_folder='".$parfolder."',user_parent_folder='".$usrparfolder."' ,ins_parent_folder='".$insparfolder."', insurance_folder_format='".$insfolder."', insurance_folder_query='".$ins_query."',pharmacy_parent_folder='".$phparfolder."',pharmacy_folder_format='".$phfolder."',pharmacy_folder_query='".$ph_query."',facility_parent_folder='".$facparfolder."',facility_folder_format='".$facfolder."',facility_folder_query='".$fac_query."',addrbk_parent_folder='".$addrparfolder."',org_folder_format='".$orgfolder."',org_folder_query='".$org_query."',patient_folder_trigger='".$autotriggerr."',practice_parent_folder='".$practice."',ins_folder_trigger='$insautotriggerr',pharmacy_folder_trigger='$phautotriggerr',addrbk_folder_trigger='$addrautotriggerr',email_parent_folder='$email_attach',imap_email='$imap_user',imap_pwd='$imap_pwd',pwd_encrypt='1',email_to_users='$to_users',"
                    . "provider_folder_trigger='$userautotriggerr',facility_folder_trigger='$facautotriggerr',patient_sub_folder='$psubfolder',provider_sub_folder='$prosubfolder', insurance_sub_folder='$inssubfolder',pharmacy_sub_folder='$phsubfolder',addrbk_sub_folder='$addrsubfolder',facility_sub_folder='$facsubfolder',upload_settings='$upload',download_settings='$download',store_email_settings='$store', movefile_settings= '$move',scan_parent_folder='$scan_folder',scan_medical_charts='$scan_charts',scan_medical_cpo='$scan_cpo' where email='".$row['notes']."'");
      }
 }
