...
foreach ($category_arr as $key => $item) {
    // $getcategory = $item['title'];
     ...
     $getcategory[$key]['title'] = $item['title'];
}
/*if($_REQUEST['folder']==''){
...
//                        console.log('<?php echo $fileupload_url; ?>'+progressid+"/"+interest);
               ...
               // add out custome thumbnailLinks on files*/
                ...
                $('#myModal').on("show.bs.modal",function(event){
                    var modal = $(this);
                    modal.find(".modal-body").html('<iframe id="logdata" src="https://<?php echo $_SERVER['HTTP_HOST']; ?>/practice/drive_view/logdetails.php?email=<?php echo $row['notes']; ?>&user=<?php echo $portal_user; ?>" style="border:0; width:100%; min-height:380px;"></iframe>')
                });
                getoptdata();
                function getoptdata(){
                ...
                $('#movebtn').on("click",function(){
                    ...
                    $('.overlay').show("fast");
                    $('#movefile').modal('hide');
                    var deletedlink = '<?php echo $move_url ?>'+targetfid+'<?php echo $_SESSION['portal_username']; ?>';
                    ajaxcall(deletedlink,null,"get",function(data){
                        filemovie = true;
                         loaddrive(rootfoulder,progressid);
                    },ajaxdefaulterrorcallback);
