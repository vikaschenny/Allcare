...
foreach($id as $key => $val){
    if($key<$to){
        ...
        Create_folder($row['notes'],$val,$folder_name,$parentid,'',$parent_stat,'create',$sel_rows['provider_sub_folder']);
         $cnt=$key;
     }else {
    ...
    foreach($id as $key => $val){
        ...
        if($key<$to){
            ...
            if(in_array($res1[1],$child_folders)){
                     ...
                     //to get folder info
                    $curl = curl_init();
                    $form_url = $protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/getfileinfo_web/'.$row['notes'].'/'.$res1[1];
                    curl_setopt($curl,CURLOPT_URL, $form_url);
                    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);  
                    $result = curl_exec($curl);
                    $resultant = $result;
