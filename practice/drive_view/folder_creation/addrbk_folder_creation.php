            ...
            if($action=='create' && $type=='abook'){
                ...
                if($subfolder=='yes'){
                    ...
                    while($subrow = $subsql->fetchObject()){
                        $subresultant=getdata($protocol.$_SERVER['HTTP_HOST'].'/api/DriveSync/insert_folder_web/'.$email.'/'.$val[0].'/'.str_replace(" ","_",$subrow->title));
...
$list_sql = $sqlconfCentralDB->prepare("select option_id ,title from list_options where list_id='abook_type' order by `seq` asc ");
$list_sql->execute();
while($data_row = $list_sql->fetchObject()){
    ...
    while($urow=$user->fetchObject()) {
        ...
        $sel_custom=$sqlconfCentralDB->prepare("select addressbook_folder,addrbk_folder from tbl_addrbk_custom_attr_1to1 where addrbk_type_id='".$urow->id."'");
        $sel_custom->execute();
        $row12=$sel_custom->fetchObject();
        if($row12->addressbook_folder=='' && $i==0){
            $name=str_replace("/","_",str_replace(" ","_",$data_row->title));
            $abook_type[]=$name;
        }elseif($row12->addressbook_folder!='' && $i==0){
            $name=str_replace("/","_",str_replace(" ","_",$data_row->title))."*".$row12->addressbook_folder;
        }
        
        if($row12->addrbk_folder==''){
           $t= str_replace(" ","_",$data_row->title);
           $org[$name][$urow->id]= $folder_name;
        }
    $i++;
    }
}
