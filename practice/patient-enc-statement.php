...
/*
if(empty($encId)):
    echo "<span style='color:red;'>This patient has not arrived yet...</span>";
    exit;
endif;
 * 
 */
...
<script type='text/javascript'>
    $(document).ready(function() {
        $('#patlistselect').hide();
       var datatable = $('#patientstatement').dataTable( {
        iDisplayLength: 100,
        dom: 'T<\"clear\">lfrtip',
        columnDefs: [
            {
...
<center>
<form method='post' action='patient-enc-statement.php' enctype='multipart/form-data'>
    <div class="accordion" style="margin-top:2px; margin-bottom: 10px;">Filters</div>
    <div class="panel">
...
<?php } ?>
 ...
 <tr><td></td>
     ...
     <input type="checkbox" onchange="showbillingaddr()" checked>Show/hide Billing Address<br/>
     <input type="checkbox" onchange="fnShowHide(6)">Show/hide Claim History</td>
 </tr>
</table>
...
//if ($_POST['form_search'] || $_POST['form_print']) {
  ...
  if ($INTEGRATED_AR) {
    ...
    $query = "SELECT DISTINCT f.encounter, f.id, f.pid, f.date,f.billing_note, " .
      ...
      "WHERE $where $payerClause $insuranceClause $faciltyClause $livingfaciltyClause $patlistselectClause AND bb.provider_id !=0 AND bb.payer_id !='' AND f.pid= $pid " .
...
$a = 0;
  if ($INTEGRATED_AR) {
    while ($row = sqlFetchArray($t_res)) {
      ...
      $isdueany = ($balance > 0);
      $pids = $row['pid'];
      //echo "$a) $pids .  1isdueany: " . $isdueany. " 1duncount: ".$duncount."<br/>";
...
//    $isdue = ($row['duedate'] <= $today && $row['amount'] > $row['paid']) ? " checked" : "";
      ...
      // for this.
      //
      //echo "2isdueany: " . $isdueany. " 2duncount: ".$duncount."<br/>";
