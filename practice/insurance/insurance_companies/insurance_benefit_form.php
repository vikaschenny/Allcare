...
$result  = getBenefits($benefitid);



$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'BENEFITS' AND uor > 0 " .
...
$(document).ready(function(){
	...
	$(".medium_modal").fancybox( {
        ...
        $(".tabContainer input").parent("td").prev().hide();
        $(".tabContainer input").parent("td").hide();
        $('body').hide();
        $("#save").hide();
        $.ajax({url:"../benefit_fields_assign.php",type:"post",data:{action:"change",ins_type:'<?php echo $_GET['instype'] ?>'},success: function (data, textStatus, jqXHR) {
                  //console.log(data);      
                 var fielddata = JSON.parse(data)|| [];
                 $('body').show();
                 $.each(fielddata,function(index,value){
                     $(".tabContainer input[id$="+value+"]").parent('td').prev().show()
                     $(".tabContainer input[id$="+value+"]").parent('td').show();
                 });
                 //$(".tabContainer").children("div").hide();
                 $(".tabNav").children("li").hide();
                 var isfirst = false;
                 $.each($(".tabContainer").children("div"),function(eindex,evalue){
                      var self = $(this);
                      self.removeClass("current");
                      $.each(self.find("td"),function(index,value){
                          if($(value).css("display") != "none"){
                              $(".tabNav li").eq(eindex).show();
                              if(!isfirst){
                                 $(".tabNav li").removeClass("current");
                                 $(".tabNav li").eq(eindex).addClass("current");
...
<?php generate_layout_validation('BENEFITS'); ?>
...
<body class="body_top" style="display: none;">
<form action='insurance_save_benefit.php' name='insurance_benefit_form' method='post' onsubmit='return validate(this)'>
...
<br>
  <div class="section-header">
   <span class="text"><b> <?php xl("Add New Benefit", "e" )?></b></span>
</div>
<div id="insurance_custom_attr" >
	<ul class="tabNav">
	   <?php display_layout_tabs('BENEFITS', $result, $result2); ?>
	</ul>
	<div class="tabContainer" style="height: 300px;background:#FFF; overflow-y: scroll; width: 80%;">
		<?php display_layout_tabs_data_editable('BENEFITS', $result, $result2); ?>
	</div>
</div>
<br>
<input type="hidden" name="formid" id="formid" value="<?php echo $_GET['benefitid']; ?>"/>
<input type="hidden" name="planid" id="planid" value="<?php echo $_GET['planid']; ?>"/>
<input type="hidden" name="formaction" id="formaction" value="<?php echo $_GET['formaction']; ?>"/>
