...
// document.getElementById("formid").value = $(parent.document.getElementById("inscop")).attr("data-formid");
 if (validate(f)) {
  f.submit(); 
//    $.ajax({url:"insurance_save_plan.php",data:$('form').serializeArray(),type:"post",success: function (data, textStatus, jqXHR) {                 
...
<body class="body_top">
<form action='insurance_save_plan.php' name='insurance_plan_form' method='post' onsubmit='return validate(this)'>
...
<br>
  <div class="section-header">
   <span class="text"><b> <?php xl("Add New Plan", "e" )?></b></span>
</div>
<div id="insurance_custom_attr" >
	<ul class="tabNav">
	   <?php display_layout_tabs('INSUCOMP', $result, $result2); ?>
	</ul>
	<div class="tabContainer" style="height: 300px;background:#FFF; overflow-y: scroll; width: 80%;">
		<?php display_layout_tabs_data_editable('INSUCOMP', $result, $result2); ?>
	</div>
</div>
<br>
<input type="hidden" name="formid" id="formid" value="<?php echo $_GET['planid']; ?>"/>
<input type="hidden" name="insid" id="insid" value="<?php echo $_GET['insid']; ?>"/>
