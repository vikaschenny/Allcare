...
//$provider = $_SESSION['portal_username'];
    $provider = $_REQUEST['provider']; // By Subhan
}
else {
        session_destroy();
...
<script>
    jQuery(document).ready(function() {
        jQuery('#form_audited_status').change(
            function() {
                var val1 = jQuery('#form_audited_status option:selected').val();
                if(val1 === 'Completed'){
                    jQuery("#form_elec_signedby").removeAttr('disabled');
                    jQuery("#form_elec_signed_on").removeAttr('disabled');
                }else{
                    jQuery("#form_elec_signedby").attr('disabled','disabled'); 
                    jQuery("#form_elec_signed_on").attr('disabled','disabled'); 
                }
            }
        );
    });
...
<script language="javascript">
                ...
                jQuery('div',"#"+group).each(function(){
                    if(jQuery(this).attr('id').substring(1).startsWith("_") === true){
                        ...
                        if(jQuery(this).attr('id') === 'd_risk'){
                            ...
                            //dataarray4.push(jQuery('#hiddend_acd').text());
                        }else{
   ...
   function showDiv (fieldname) { 
    ...
    function audit_form_check(){
        ...
        if(audit_time === ''  || typeof audit_time === 'undefined' ) audit_time = 0;
        var selected_data = '';
        var check_audit_time    = "T:"+audit_time;
        selected_data = 'H:'+history_data+';E:'+exam_group_value+';D:'+risk_data +";T:"+audit_time;
...
<body class="body_top">
    ...
    echo "<form method='post' name='my_form' id='my_form' action='save_custom.php?id=" . attr($formid) ."&encounter=$encounter&pid=$pid&provider=$provider&isSingleView=$isSingleView&isFromCharts=$isFromCharts'>\n";
    ...
    echo "<input type='hidden' name='encounter' id='encounter' value='".$_REQUEST['encounter']."' />";
                            ...
                            while($set_list_fields = sqlFetchArray($get_list_fields)){
                                ...
                                if( $list_id == 'Diagnosis_Management_Options'){
                                    ...
                                    if( $list_id == 'Audit_CC_Options'){
                                        if($option_id != '5'){
                                            ?><label><input type='radio' id='<?php echo $list_id.'radio'; ?>' name='<?php echo $list_id.'radio'; ?>' value= '<?php echo $option_id; ?>' onclick='checkbox_selected("<?php echo $field_id ?> ", "<?php echo $group_id; ?> " , "<?php echo $option_id; ?>");' <?php if($option_id == $audit_data[$groupname_checkbox]['Audit_CC_Optionsradio']) echo checked; ?> > <?php echo $list_title."</label></br>";
                                        }else{
                                            ?><label><input 
                                                type      = 'checkbox' 
...
<!--                    <script>
    ...
    $get_cert_doc_name = sqlStatement("SELECT lbf.*,l.title FROM lbf_data lbf   
                                                         ...
                                                         WHERE lbf.form_id = $lbf_form_id AND l.field_id IN ( 'cert_recert_doc_link',  'cert_recert_doc_name'
                                                            ) ORDER by l.seq" );
    $datachecking = 0;
    ?>
    <div id='displaycert' style='display: none;clear:both;'>
        ...
        <div id='cert' style='border:1px solid black;margin: 0px 20px 10px 20px; overflow: auto;display: none;clear:both;'>
            <table id='tablecert'>
                <tr>
                    <td>
                    <?php 
...
//    $login_userid=$_SESSION['authId'];
     $login_userid = $provider;
   
    ?>
    <div class="tabContainer">							
        <?php
        $getlbfformid = sqlStatement("SELECT id FROM form_encounter WHERE pid = $pid AND encounter = $encounter");
        $lbfformid = 0;
        while($setlbfformid = sqlFetchArray($getlbfformid)){
            $lbfformid = $setlbfformid['id'];
        }
        ?>
        <input type="hidden" value ="<?php echo $lbfformid; ?>" name="lbf_formid">
        <table>
        <tr>
        <?php          
            $trow = $lbfformid ? formDataid($lbfformid) : '';
        ...
        //$encounter = isset($encounter)? $encounter : $SESSION['encounter']; 
        ...
        endwhile;
        $providerid = 0;
        $selectquery2 = sqlStatement("SELECT id FROM users WHERE username = '$provider'");
        if(!empty($selectquery2)){
            while($setquery2 = sqlFetchArray($selectquery2)){
               $providerid = $setquery2['id'];
            }
        }
        //echo "Audit code: ". $auditN . " --- " . "Fee Sheet code: ". $feeR."<br />";
        $providerselect = '';
        $isprovider = sqlStatement("SELECT id FROM `users`  WHERE active=1 AND authorized=1 AND id = ".$providerid);
        while($isprovider2 = sqlFetchArray($isprovider)){
            $providerselect = $isprovider2['id'];
        }
        if($providerselect == '' || $_SESSION['refer'] != ''){
            ?>
            <script>
                jQuery("#form_audit_note").removeAttr('disabled');
            </script>
            <?php
        }else{
            ?>
            <script>
                // jQuery("#form_audit_note").attr('disabled','disabled');
            </script>
            <?php
        }
        $providercheck = sqlStatement("SELECT processproviders  FROM tbl_user_custom_attr_1to1 WHERE userid= ".$providerid);
        ...
        if($auditN != "" || $feeR != ""):
            ...
            ?>
            <script>
                jQuery("#form_audited_status").removeAttr('disabled');
            </script>
            <?php
            else:
                //echo "Audit Failed";
            ?>
            <script>
                jQuery("#form_audited_status").attr('disabled','disabled');
                jQuery('#form_audited_status option[value="Incomplete"]').attr("selected", "selected");
            </script>
            <?php
            endif;
        endif;

        $processaudit = sqlStatement("SELECT processaudit FROM tbl_user_custom_attr_1to1 WHERE userid= ".$providerid);
        while($auditcheck2 = sqlFetchArray($processaudit)){
            $authAudit = $auditcheck2['processaudit'];
        }
