...
function display_db_table($provider,$page_id,$filters) { //echo $provider. " -- " . $page_id;
              ...
              <div class="col-md-12">
                    <div id="content">
                        ...
                        $get_providers_list = sqlStatement("SELECT id,CONCAT (fname, lname, mname) as providername,username FROM users WHERE authorized != 0 AND active = 1 AND username <> '' ORDER BY fname,lname");
                            ...
                            $getdatafilter = sqlStatement("SELECT id,af_providers FROM tbl_providerportal_filters WHERE userid = '".$_SESSION['portal_userid']."' and screen_name = 'my_patients'");
                            
                            while($setdatafilter = sqlFetchArray($getdatafilter)){
                               $af_providers = $setdatafilter['af_providers'];
                               $uid = $setdatafilter['id'];
                            }
                            $set_query_num     = sqlNumRows($getdatafilter);
                            if($set_query_num == 0){
                                if($provider != "") $sql = sqlStatement("INSERT INTO tbl_providerportal_filters (userid,screen_name,date,af_providers) values('".$_SESSION['portal_userid']."','my_patients',CURDATE(),'$provider')");
                            }else{
                                if($provider != "") $sql = sqlStatement("UPDATE tbl_providerportal_filters SET date= CURDATE(),af_providers ='$provider' WHERE id = '".$uid."' ");
                            }
                            

                           if($af_providers != "") $provider = $rendprovider = $af_providers;
                           if(!empty($_POST['rendering_provider'])){
                               $provider = $rendprovider = $_POST['rendering_provider'];
                           }
                           if($page_id=='my_patients'){
                               $title='My Patients';
                       ...
                       <form action="patient_data.php" method="post" id="patients_filtersid" name="patients_filters">
                           <input type="hidden" id="sidenavep" value="<?php echo $page_id; ?>"/>
                       <?php
                       if($page_id=='my_patients'){
                                  $sql_mypatients1=sqlStatement("SELECT * FROM `list_options`  where list_id='Mobile_My_Patients_Filters' order by seq");
                                   ...
                                   echo "</select></div>";
                                   echo "<div class='input-group-sm'><label for='pro' style='float:left;'>Filter: </label><select name='my_filters' id='my_filters' onchange='javascript:dropdownchange();' class='form-control'  style='width:150px;margin: 0 75px 10px 75px;'>";
                       ...
                       <?php }
                       if($page_id=='by_appointment'){ //echo "select id from users where username='".$provider."'";
                          $sql=sqlStatement("select id from users where username='".$provider."'");
                         $row=sqlFetchArray($sql);
                         $id=$row['id'];
                            $sql_apppatients3=sqlStatement("SELECT visit_categories FROM tbl_allcare_facuservisit WHERE  `users`  REGEXP ('".":\"".$id."\"')");
                             ...
                             $dataArray = array();
                              ...
                              $today1=date('Y-m-d');
                               echo "<div class='input-group-sm'><label for='pro' style='float:left;'>Provider:</label>
                                   ...
                                   echo "</select></div>";
                               echo "<div class='input-group-sm'><label for='appdates' style='float:left;'>Appointment Dates:</label><select id='app_date' name='app_date'  class='form-control' style='width:150px;margin: 0 75px 10px 150px;' onchange='javascript:dropdownchange();'>";
