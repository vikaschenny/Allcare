...
if($_POST['allSelectedFields'][0] == '-1' || empty($_POST['allSelectedFields']) ){
     ...
     pd.living_facility,pd.living_facility_org,pd.interpretter,pd.migrantseasonal,pd.contrastart,pd.referral_source,pd.referlink, pd.referred_date,pd.hhagency,
     pd.vfc,pd.Insured_Type,pd.authorization,pd.Copay,pd.Deduct_Fam_Ann,pd.Deduct_Indiv_Ann,pd.indoutofpocket,pd.familyoutofpocket,
    ...
    pd.sex,CONCAT(u1.fname,' ',u1.lname) AS Actual_Provider,CONCAT(u2.fname,' ',u2.lname) AS Referrer_Provider,pd.email,op_cat.pc_catname,opc.pc_hometext,opc.pc_counter,opc.pc_topic,opc.pc_informant,opc.pc_eid,pd.pid,pd.latitude,pd.longitude,pd.openAppdate
    ";
    ...
    $fieldparamters2 .= "pd.latitude,pd.longitude,pd.openAppdate";
...
#encAddrVal_wrapper{background-color:red; height:500px; width:100%; overflow:scroll; overflow-x: hidden;}
#example2_wrapper{ height:1500px; width:100%; overflow:scroll;}
#hd1{background:#FF99FF !important;}
</style>
...
";
    ...
    ?>
    <script>
        if($('#encaddrvalshow').val() == 0){
            $('#encAddrVal_wrapper').hide();
            $('#lblHideEnc').hide();
            $('#lblShowEnc').show();
        }    
        if($('#encaddrvalshow').val() == 1 || $('#encaddrvalshow').val().length === 0){
            $('#encAddrVal_wrapper').show();
            $('#lblHideEnc').show();
            $('#lblShowEnc').hide();
        }        
        </script>
    <div id='hd1'>
      <a onclick="javascript:jQuery('#example2_wrapper').toggle(500);
                              jQuery('#lblHideEnc1').hide();
                              jQuery('#lblShowEnc1').show();jQuery('#encpatshow').val('0');"
       style="cursor:pointer;">
           <label id="lblHideEnc1"><b>Hide Patient Data</b></label>
       </a>

       <a onclick="javascript:jQuery('#example2_wrapper').toggle(500);
                              jQuery('#lblHideEnc1').show();
                              jQuery('#lblShowEnc1').hide();jQuery('#encpatshow').val('1');"
       style="cursor:pointer;">
           <label id="lblShowEnc1" style="display:none;"><b>Show Patient Data</b></label>
       </a>
        <script>
        if(jQuery('#encpatshow').val() == 0){
            jQuery('#example2_wrapper').hide();
            jQuery('#lblHideEnc1').hide();
            jQuery('#lblShowEnc1').show();
        }    
        if(jQuery('#encpatshow').val() == 1 || jQuery('#encpatshow').val().length === 0){
            jQuery('#example2_wrapper').show();
            jQuery('#lblHideEnc1').show();
            jQuery('#lblShowEnc1').hide();
        }        
        </script>
    </div>
    <?php 
...
//                  ".$phone_home.$phone_biz.$phone_contact.$phone_cell."</td>";
        ...
        while($rowTemp=sqlFetchArray($showTemp2))
        {
            ...
            $openAppdate=(isset($rowTemp['openAppdate']) && $rowTemp['openAppdate']!='') ? $rowTemp['openAppdate'] : '';
            
            echo "<input type='hidden' id='hdnDetailsEncBack_$j' name='hdnDetailsEncBack' 
                  value=\"".$fname." ".$lname."-<br>".$street.", ".$city.", ".$state.", ".$country_code."<br>".$phone_home.$phone_biz.$phone_contact.$phone_cell."<br />Appointment Date: ".$openAppdate."\" />
