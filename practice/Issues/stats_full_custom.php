...
$formid     = trim($_REQUEST['formid']);
 ...
 function updateENC_forms($id, $new, $create,$ecounter1,$pid1,$provider){
    global $id;
    $db_id = 0;
   // print_r($new);
    if($id == 0 && !empty($new)) {
        ...
        $ins_form   = sqlStatement("INSERT INTO  forms (date,encounter,form_name,form_id,pid,user,groupname,authorized,deleted,formdir) VALUES (NOW(),$ecounter1,'Allcare Encounter Forms',$new_id1,$pid1,'$provider','default',1,0,'LBF2')");
    ...
    if(!empty($res_row1)){
        updateENC_forms($formid, $newdata['lbf_data'] ,$create=false,$encounter,$patient_id,$provider);
        if($_REQUEST['isSingleView'] == 1 && $isFromCharts == 0)
            echo "<script> window.close();window.opener.location.href = '../single_view_form.php?encounter=$encounter&pid=$patient_id';</script>";
        ...
        updateENC_forms($formid, $newdata['lbf_data'] ,$create=true,$encounter,$patient_id,$provider);
        if($_REQUEST['isSingleView'] == 1 && $isFromCharts == 0)
            echo "<script> window.close();window.opener.location.href = '../single_view_form.php?encounter=$encounter&pid=$patient_id';</script>";
        else if($_REQUEST['isSingleView'] == 1 && $isFromCharts == 1)
            echo "<script>window.opener.datafromchildwindow('',$id);window.close();</script>";
...
function dopclick(id,category,pid,location,enc) {
    <?php //if (acl_check('patients','med','','write')): ?>
           
    if (category == 0) category = '';
    window.open('add_edit_issue_custom.php?issue=' + encodeURIComponent(id) + '&thistype=' + encodeURIComponent(category) + '&thispid='+pid+'&thislocation='+location+'&thisenc='+enc, '_blank', 550, 400);
    <?php //else: ?>
...
<?php   if($_REQUEST['isSingleView'] == 1)
            ...
            $isSingleView = 0;
?>
<form method='post' action='stats_full_custom.php?isSingleView=<?php echo $isSingleView ?>&isFromCharts=<?php echo $isFromCharts; ?>' onsubmit=''>
...
foreach ($ISSUE_TYPES as $focustype => $focustitles) {
  ...
  echo "<a href='javascript:;' class='css_button_small' onclick='dopclick(0,\"" . htmlspecialchars($focustype,ENT_QUOTES)  . "\",$patient_id,\"" . htmlspecialchars($location,ENT_QUOTES)  . "\",$encounter)'><span>" . htmlspecialchars( xl('Add'), ENT_NOQUOTES) . "</span></a>\n";
  echo "  <span class='title'>" . htmlspecialchars($disptype,ENT_NOQUOTES) . "</span>\n";
  echo " <table id='transaction' style='margin-bottom:1em;text-align:center'>";
  ?>
  <tr class='head'>
  ...
  while ($row = sqlFetchArray($pres)) {
    ...
    echo "  <td data-th='Title' style='text-align:left'>" . htmlspecialchars($disptitle,ENT_NOQUOTES) . "</td>\n";
    echo "  <td data-th='Begin' >" . htmlspecialchars($row['begdate'],ENT_NOQUOTES) . "&nbsp;</td>\n";
    echo "  <td data-th='End' >" . htmlspecialchars($row['enddate'],ENT_NOQUOTES) . "&nbsp;</td>\n";
    // both codetext and statusCompute have already been escaped above with htmlspecialchars)
    echo "  <td data-th='Diag' >" . $codetext . "</td>\n";
    echo "  <td data-th='Status' >" . $statusCompute . "&nbsp;</td>\n";
    echo "  <td data-th='Occurrence' class='nowrap'>";
    echo generate_display_field(array('data_type'=>'1','list_id'=>'occurrence'), $row['occurrence']);
    echo "</td>\n";
    if ($focustype == "allergy") {
      echo "  <td data-th='Reaction' >" . htmlspecialchars($row['reaction'],ENT_NOQUOTES) . "&nbsp;</td>\n";
    }
    if ($GLOBALS['athletic_team']) {
        echo "  <td class='center' data-th='Missed' >" . $row['extrainfo'] . "</td>\n"; // games missed
    }
    else {
        echo "  <td data-th='Referred By' >" . htmlspecialchars($row['referredby'],ENT_NOQUOTES) . "</td>\n";
    }
    echo "  <td data-th='Comments' >" . htmlspecialchars($row['comments'],ENT_NOQUOTES) . "</td>\n";
    echo "  <td data-th='Enc' id='e_$rowid' class='noclick center' title='" . htmlspecialchars( xl('View related encounters'), ENT_QUOTES) . "'>";
...
var GoBack = function () {
     window.opener.location.href = '../provider_incomplete_charts.php';
}
var GoBack_provider = function () {
    window.close();
//      window.opener.location.href='../../../practice/provider_incomplete_charts.php?provider=<?php echo $provider; ?>';
}
