...
$location=$_REQUEST['thislocation'];
...
?>
 ...
 function closeme() {
    ...
    window.opener.location.href = 'stats_full_custom.php?pid='+<?php echo $_REQUEST['thispid']; ?>+'&category='+'<?php echo $_REQUEST['thistype']; ?>'+'&encounter='+'<?php echo $_REQUEST['thisenc']; ?>';
 }
function closeme_provider() {
//    if (parent.$) parent.$.fancybox.close();
    window.close();
 }
...
function sel_diagnosis() {
  ...
  {
  ?>
 dlgopen('find_code_popup.php?codetype=<?php echo attr(collect_codetypes("medical_problem","csv")) ?>', '_blank', 500, 400);
  <?php
  }
  else{
  ?>
...
<form method='post' name='theform'
 action='add_edit_issue_custom.php?issue=<?php echo attr($issue); ?>&thispid=<?php echo attr($thispid); ?>&thisenc=<?php echo attr($thisenc); ?>&location=<?php echo attr($location); ?>'
...
<?php } else { ?>
    <input type='button' value='<?php echo xla('Cancel'); ?>' onclick='closeme_provider();' />
<?php } ?>
