...
$squery  = sqlStatement("SELECT pc_apptstatus FROM openemr_postcalendar_events WHERE pc_eid = '$pc_eid' AND pc_pid = '".$patientid."'");
...
while($set_visit_details = sqlFetchArray($get_visit_details)){
    ...
    //echo "SELECT encounter FROM form_encounter WHERE date='$dos' AND facility_id='$facility' AND pc_catid = '$visit_category' AND billing_facility='$billing_facility' AND rendering_provider='$rendering_provider' AND pid=$patientid";
    if($sFetch['pc_apptstatus'] == '@'):
        $encquery  = sqlStatement("SELECT encounter FROM form_encounter WHERE date='$dos' AND pc_catid = '$visit_category' AND rendering_provider='$rendering_provider' AND pid=$patientid");
        $encFetch = sqlFetchArray($encquery);
        $encounter = $encFetch['encounter'];
    endif;
    
    // Create new encounter if status is not arrived
    if($sFetch['pc_apptstatus'] != '@'):
