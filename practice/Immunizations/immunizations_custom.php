...
while ($frow = sqlFetchArray($fres)) {
 ...
 $res1=sqlstatement("select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$formid' AND l.form_id='LBF2' AND l.group_name LIKE '%Immunization%'  order by seq");
 $res_row1=sqlFetchArray($res1);
 if(!empty($res_row1)){
     updateENC_forms($formid, $newdata['lbf_data'] ,$create=false,$encounter,$pid,$provider);
     
     echo "<script>  window.close();
   window.opener.location.href='../provider_incomplete_charts.php';</script>";
 }else{
      updateENC_forms($formid, $newdata['lbf_data'] ,$create=true,$encounter,$pid,$provider);
      echo "<script>  window.close();
   window.opener.location.href='../provider_incomplete_charts.php';</script>";
 }
 }
...
<!-- NEEDS WORK -->
        ...
        <tr>
          <td align="right" class="text">
              ...
              (<a href="https://www.cdc.gov/vaccines/pubs/vis/default.htm" title="<?php echo htmlspecialchars( xl('Help'), ENT_QUOTES); ?>" target="_blank">?</a>)          </td>
          <td>
...
var PrintForm = function(typ) {
    //top.restoreSession();
    newURL='shot_record_custom.php?output='+typ+'&sortby=<?php echo $sortby; ?>&pid=<?php echo $pid; ?>&encounter=<?php echo $encounter; ?>';
...
var ErrorImm = function(imm) {
   // top.restoreSession();
	location.href='immunizations_custom.php?mode=added_error&pid=<?php echo $pid; ?>&form_id=<?php echo $formid; ?>&id='+imm.id+'&isError='+imm.checked;
}
...
function sel_cvxcode(e) {
 ...
 dlgopen('../Issues/find_code_popup.php?codetype=CVX', '_blank', 500, 400);
}
