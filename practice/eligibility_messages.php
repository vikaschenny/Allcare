...
<?php
  ...
  echo "</option>\n";
$ures = sqlStatement("SELECT username, fname, lname, email FROM users " .
 "WHERE username != '' AND active = 1 AND " .
 "( info IS NULL OR info NOT LIKE '%Inactive%' ) " .
 "ORDER BY lname, fname");
 while ($urow = sqlFetchArray($ures)) {
     //if($urow['email'] != '') {
        echo "    <option data-email='" . htmlspecialchars( $urow['email'], ENT_QUOTES) . "' value='" . htmlspecialchars( $urow['username'], ENT_QUOTES) . "'";
...
$(document).ready(function(){
    ...
    pat_ids.forEach(function(item){
       var pat_name = window.opener.$("[data-pid="+item+"]").html();
       msg_note+=i+". "+pat_name+"\n";
       msg_note+="\t Elig : $123 \n";
       msg_note+="\t Elig2 : $1234 \n\n";
       i++;
