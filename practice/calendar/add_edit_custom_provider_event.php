...
function DOBandEncounter()
 {
   global $event_date,$info_msg;
         ...
         if ($GLOBALS['auto_create_new_encounters'] && $_POST['form_apptstatus'] == '@' && $event_date == date('Y-m-d'))
	 {
         ...
         */
         // 2015-11-05: Subhan: To create encounter for past as well as current date appointments. 
         // Also to update RENDERING PROVIDER field in encounter summary
         if ($GLOBALS['auto_create_new_encounters'] && $_POST['form_apptstatus'] == '@' && $event_date <= date('Y-m-d'))
	 {
    ...
    if(!empty($encounter)){
        ...
        while($set_is_Single_view = sqlFetchArray($get_is_Single_view)){
            if(trim($set_is_Single_view['title']) == 'yes' || trim($set_is_Single_view['title']) == 'Yes' || trim($set_is_Single_view['title']) == 'YES')
                echo " window.open('../../reports/single_view_form.php?encounter=$encounter&pid=$pid', '_blank', 500, 400);";
        }
    }
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 function sel_patient() {
  window.open('find_patient_popup.php', '', 'width=500,height=400');
 }
 ...
 function setappt(year,mon,mday,hours,minutes) {
  ...
  f.form_minute.value = ('' + (minutes + 100)).substring(1);
	...
	var formDate = document.forms[0].form_date;
        window.open('<?php echo $GLOBALS['web_root']; ?>/practice/calendar/find_appt_popup.php' +
                '?providerid=' + s +
...
<form method='post' name='theform' id='theform' action='add_edit_custom_provider_event.php?eid=<?php echo attr($eid) ?>' />
...
<!--<tr><th><ul class="tabNav">
		 ...
		 <a href='add_edit_custom_provider_event.php?prov=true&eid=<?php echo attr($eid);?>&startampm=<?php echo attr($startm);?>&starttimeh=<?php echo attr($starth);?>&userid=<?php echo attr($uid);?>&starttimem=<?php echo attr($starttm);?>&date=<?php echo attr($dt);?>&catid=<?php echo attr($cid);?>'>
