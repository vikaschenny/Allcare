...
if(!empty($_POST['patient_dropdown']) && !empty($_POST['dos']) && !empty($_POST['visit_category'] )){
    ...
    while($setquery = sqlFetchArray($query)){
        ...
        $insert_encounter = sqlStatement("INSERT INTO form_encounter (date, facility, facility_id, pid, encounter, pc_catid, provider_id, billing_facility,rendering_provider)
            VALUES ('$dos', 'test','$facility','$patientid','$encounter','$visit_category','$rendering_provider','$billing_facility','$rendering_provider')");
    ...
    echo "<script>
        window.location.href = '../../../practice/provider_incomplete_charts.php?provider=".$rendering_provider." &form_patient=$patientid&form_to_date=$dos';
    </script>";
}
    ...
    $(document).ready(function() {
        jQuery('#patient_search_button').click(function(){
            ...
            function setNavigation() {
                 $('#sidenave li').eq(7).addClass('active');
                 $('#sidenave li').eq(7).find('a').removeAttr("href");
            }
        });
                        ...
                        <?php  $sql_vis=sqlStatement("SELECT provider_plist_links from tbl_user_custom_attr_1to1 where userid='".$id['id']."'");
                                ...
                                if(!empty($row1_vis)){
                                    ...
                                    <?php }if(in_array('create_patient',$links)){ 
                                                ?>
                                                   <li class=""><a href='javascript:;' onclick=DoPost_patient('new_comprehensive.php?provider=<?php echo $provider; ?>')>Create Patient</a></li>
