...
if($_POST['search_plan']==''){
    ...
    if($term=="")
        ...
        $sql_post=$conn->query("select post_title from wp_posts where post_type='payer' and post_title LIKE '%$term%'");
        echo "select post_title from wp_posts where post_type='payer' and post_title LIKE '%$term%'";
    ...
    //                                $string = 'Payer List,,X = Requires Enrollment "Application: All",,O = Does not require enrollment "Group By: None",,(Blank) = Application not active for this payer "Filter(s): Payer Class = BC/BS -- Tricare -- Medicare -- Medicaid -- Commercial -- DMERC "';
