...
if ($show_all=='yes') {
    $showall = "yes";
    $lnkvar="'messages.php?show_all=no&$activity_string_html' name='Just Mine' onclick=\"\"> (".htmlspecialchars( xl('Just Mine'), ENT_NOQUOTES).")";
}
else {
    $showall = "no";
    $lnkvar="'messages.php?show_all=yes&$activity_string_html' name='See All' onclick=\"\"> (".htmlspecialchars( xl('See All'), ENT_NOQUOTES).")";
}
//}
?>
<br>
...
if (empty($task) || $task=="add" || $task=="delete") { ?>
  ...
  <?php } else { ?>
    <a href="messages.php" class="link" onclick=""><span><?php echo xlt('Show All'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '1') { ?>
    <span><?php echo xlt('Show Active'); ?></span>
  <?php } else { ?>
    <a href="messages.php?form_active=1" class="link" onclick=""><span><?php echo xlt('Show Active'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '0') { ?>
    <span><?php echo xlt('Show Inactive'); ?></span>
  <?php } else { ?>
    <a href="messages.php?form_inactive=1" class="link" onclick=""><span><?php echo xlt('Show Inactive'); ?></span></a>
  <?php } ?>
<?php } ?>
...
echo "
<form name=new_note id=new_note action=\"messages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&provider=".attr($provider)."&$activity_string_html\" method=post>
 ...
 function sel_patient() {
  ...
  function addtolist(sel){
    ...
    for($i = 0; $i < count($sort); $i++) {
        $sortlink[$i] = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sort[$i])."&sortorder=asc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>";
    }
    for($i = 0; $i < count($sort); $i++) {
        if($sortby == $sort[$i]) {
            switch($sortorder) {
                case "asc"      : $sortlink[$i] = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=desc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortup.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>"; break;
                case "desc"     : $sortlink[$i] = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=asc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Down'), ENT_QUOTES)."\"></a>"; break;
            } break;
        }
    }
    ...
    if($prev >= 0) {
        $prevlink = "<a href=\messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($prev)."&$activity_string_html\" onclick=\"\"><<</a>";
    }
    else {
        $prevlink = "<<";
    }
    ...
    if($next < $total) {
        $nextlink = "<a href=\"messages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($next)."&$activity_string_html\" onclick=\"\">>></a>";
    }
    else {
        $nextlink = ">>";
    }
    ...
    <table border=0 cellpadding=5 cellspacing=0 width=90%>
        <tr>
            <td class=\"text\"><a href=\"messages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&task=addnew&$activity_string_html\" onclick=\"\">" .
