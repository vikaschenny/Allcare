...
foreach (array('primary','secondary','tertiary') as $instype) {
    ...
    if($instype == 'primary'){
        if(formData("i1plan_name-id") == '' && formData("i1plan_name") !== ''){
            $planbody = addslashes(" New Plan type '".formData("i1plan_name") ."' is added to Patient name $patient_name ($pid) from provider portal.  ");
            $sql = sqlStatement("INSERT INTO pnotes (date, body, pid, user, groupname, activity, authorized, title, assigned_to, message_status)
        ...
        if(formData("i2plan_name-id") == '' && formData("i2plan_name") !== ''){
            $planbody = addslashes(" New Plan type '".formData("i2plan_name") ."' is added to Patient name $patient_name ($pid) from provider portal. ");
             $sql = sqlStatement("INSERT INTO pnotes (date, body, pid, user, groupname, activity, authorized, title, assigned_to, message_status)
        ...
        if(formData("i3plan_name-id") == '' && formData("i3plan_name") !== ''){
            $planbody = addslashes(" New Plan type '".formData("i3plan_name") ."' is added to Patient name $patient_name ($pid) from provider portal. ");
             $sql = sqlStatement("INSERT INTO pnotes (date, body, pid, user, groupname, activity, authorized, title, assigned_to, message_status)
