...
$renQ = sqlStatement("SELECT username FROM users u INNER JOIN form_encounter fen ON u.id = fen.rendering_provider WHERE fen.encounter = $encounter");
...
//    echo "<br /><br />";
    ...
    $get_audit_dropdown = sqlStatement("SELECT * FROM form_encounter f INNER JOIN users u 
                                        ON f.rendering_provider = u.id WHERE f.encounter = '".$_REQUEST['encounter']."'");
    ...
    echo "<tr><td style='border:0 !important'><b>Encounter: </b>".$_REQUEST['encounter']."</td><td style='border:0 !important'><b>Date Of Service: </b>".$dos[0]."</td><td style='border:0 !important'><b>Visit Category: </b>".$res_cat['pc_catname']."</td></tr>";
    echo "<tr><td style='border:0 !important'><b>Rendering Provider: </b>".$fullname."</td><td style='border:0 !important'><b>Audit Status: </b>".$audit_dropdown."</td></tr>";
    echo "</table><br>";
    
    $pid_sql    = sqlStatement("select pid from forms where encounter='$encounter' AND form_name='New Patient Encounter' AND formdir='newpatient'");
...
//            ) >1 order by fe.date desc", array($pid_row['pid']));
     ...
     $dos        = sqlStatement("SELECT DISTINCT fe.encounter as encounter, DATE_FORMAT( fe.date,  '%Y-%m-%d' ) AS date, openemr_postcalendar_categories.pc_catname
            ...
            LEFT JOIN openemr_postcalendar_categories ON fe.pc_catid = openemr_postcalendar_categories.pc_catid
            WHERE fe.pid = ?
            order by fe.date desc", array($pid_row['pid']));
...
//          " left join openemr_postcalendar_categories on fe.pc_catid=openemr_postcalendar_categories.pc_catid  WHERE fe.pid = ? order by fe.date desc", array($pid_row['pid']));
    ...
    <br><br>
    <?php 
    $grpm   = sqlStatement("select * from tbl_visitcat_chartgrp_mapping where visit_category='".$res_dos['pc_catid']."'");
    ...
    if($grpres['chart_group'] != ''){ 
        ...
        $datastring     = $res.'patientid'.'='.$pid.'&'.'encounter_id'.'='.$encounter.'&'.'dos'.'='.$dateofservice.'&'.'chartgroupshidden'.'='.$grpres['chart_group']; ?>
       ...
       <?php 
           ...
           echo '<div style="text-align:center;margin-bottom: 11px;"><a class="btn btn-primary btn-md cssbtn" data-toggle="modal" data-target="#preview" data-urlhref = "chartoutput/preview_group.php?enc='.$encounter.'&pid='.$pid.'&date='.$dateofservice.'&grp=0" href="#"  ><span>Preview</span></a></div>';
         } ?>
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('Issues/stats_full_custom.php?active=all&category=$grouptitle&encounter=$encounter&pid=$pid&formid=$lbf_form_id&location=provider_portal&provider=$provider&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            ...
            if($group != 'auditform')
                ...
                $checkstring = '';
            //echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new_custom.php?encounter=$encounter&pid=$pid&id=$static_form_id".$checkstring."&provider=$provider&location=provider_portal&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new_custom.php?encounter=$encounter&pid=$pid&id=$static_form_id".$checkstring."&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new.php?encounter=$encounter&pid=$pid&id=$static_form_id&formid=$lbf_form_id&provider=$provider&location=provider_portal&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new.php?encounter=$encounter&pid=$pid&id=$static_form_id&formid=$lbf_form_id&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
            
        }
        else if($group == 'codes'){
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('codes/feesheet_custom.php?pid=$pid&encounter=$encounter&provider=$provider&location=provider_portal&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='codes/feesheet_custom.php?pid=$pid&encounter=$encounter&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
        else if($group == 'ros'){
            $group = "allcare_ros";
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/view_custom.php?formname=$group&encounter=$encounter&pid=$pid&id=$ros_form_id&provider=$provider&location=provider_portal&isSingleView=1') ><span>".$form_name."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/view_custom.php?formname=$group&encounter=$encounter&pid=$pid&id=$ros_form_id&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>".$form_name."</span></a></div>";
        }
            ...
            //echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/new_custom.php?formname=$group&edit=custom_pe&encounter=$encounter&pid=$pid&id=$allcare_physical_exam_form_id&menu_val=&provider=$provider&location=provider_portal&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/new_custom.php?formname=$group&edit=custom_pe&encounter=$encounter&pid=$pid&id=$allcare_physical_exam_form_id&menu_val=&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('$group/view_custom.php?formname=$formname&encounter=$encounter&pid=$pid&id=$vitals_form_id&&status=$status&provider=$provider&location=provider_portal&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            echo "<div class='newwindow' id='$group'><a href='$group/view_custom.php?formname=$formname&encounter=$encounter&pid=$pid&id=$vitals_form_id&&status=$status&provider=".$rendprovider."&location=provider_portal&isSingleView=1&isFromCharts=$isFromCharts' onclick=win1(this) ><span>". ucwords(str_replace("_"," ",$group))."</span></a></div>";
        }
//        else{
