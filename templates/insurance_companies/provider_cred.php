...
$id = empty($_REQUEST['providercredid']) ? 0 : $_REQUEST['providercredid'] + 0;
 ...
 /*$sqlBindArray=array();
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_patientprovidercredentials SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
     
    array_push($sqlBindArray,$insuranceid);
    $sets .= " insuranceid = ?";
    $id = sqlInsert("INSERT INTO tbl_patientprovidercredentials SET $sets", $sqlBindArray);
  
  }*/
$newdata = array();
$newdata['tbl_patientprovidercredentials']['insuranceid'] = $insuranceid;
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'PROVCRED' AND uor > 0 AND field_id != '' " .
  "ORDER BY group_name, seq");
while ($frow = sqlFetchArray($fres)) {
  ...
  // $value  = '';
  $colname = $field_id;
  $table = 'tbl_patientprovidercredentials';
...
/*$sql=sqlStatement("select * from tbl_patientprovidercredentials where id='".$id."'");
$rowpha=sqlFetchArray($sql);*/
if($id)
{
  updateProvcred1ton($id, $newdata['tbl_patientprovidercredentials'] ,$create=false);
}
else
{
    updateProvcred1ton($id, $newdata['tbl_patientprovidercredentials'],$create=true);

}
  if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='provider_credentials_1ton.php?insurance=$insuranceid';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='provider_credentials_1ton.php?insurance=$insuranceid';";
}
...
// If we are editing a Insurance Company, get its ID and data.
$trow = $id ? getProvDatabyId($id) : array();
...
<script type="text/javascript">
    var insurancetype = '<?php echo $mode; ?>' || "";
   var insurancenumber = '<?php echo $insuranceid; ?>' || "";
   if(insurancetype=="add")
      window.parent.hidepenal(["save",insurancenumber,"provider_cred_list"]);
...
// onclick='return deleteme()'
 dlgopen('../../interface/patient_file/deleter.php?providercredid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>', '_blank', 500, 450);
 return false;
}
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('PROVCRED'); ?>
...
function submitme() {
 var f = document.forms['new_insurance_company'];
 if (validate(f)) {
  //top.restoreSession();
  f.submit();
 }
...
function canceladd(event){
    ...
    window.parent.hidepenal("cancel");
}
</script>
...
<body class="body_top">
    <form name='new_insurance_company' method='post' action='provider_cred.php?providercredid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
        <input type='hidden' id="hdnmode" name='mode' value='add'>
        <input type="hidden" name="insuranceid" value="<?php echo $_REQUEST['insuranceid']; ?> ">
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="provider_credentials_1ton.php?insurance=<?php echo $_REQUEST['insuranceid']; ?>"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="canceladd(event)" >
...
<div id="insurancecompany_data">
        <ul class="tabNav">
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'PROVCRED' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
