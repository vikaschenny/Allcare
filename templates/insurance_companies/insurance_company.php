...
$id = empty($_REQUEST['insuid']) ? 0 : $_REQUEST['insuid'] + 0;
...
$newdata['tbl_patientinsurancecompany']['insuranceid'] = $insuranceid;
...
if($id)
{ 
    updateInsurance1ton($id, $newdata['tbl_patientinsurancecompany'] ,$create=false);
}
else
{  
    updateInsurance1ton($id, $newdata['tbl_patientinsurancecompany'],$create=true);
}


if ($GLOBALS['concurrent_layout'])
        $body_onload_code = "javascript:location.href='insuranceplans_1ton.php?insurance=$insuranceid';";
    else
        $body_onload_code = "javascript:parent.New_group__service.location.href='insuranceplans_1ton.php?insurance=$insuranceid';";
    //}
}
...
$trow = $id ? getInsDatabyId($id) : array();
...
<script type="text/javascript">
   ...
   if(insurancetype=="add")
      window.parent.hidepenal(["save",insurancenumber,"insurance_company_list"]);
...
// onclick='return deleteme()'
 dlgopen('../../interface/patient_file/deleter.php?insuid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>', '_blank', 500, 450);
 return false;
}
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('INSUCOMP'); ?>
...
<body class="body_top" >
    <form name='new_insurance_company' id='new_insurance_company' method='post' action='insurance_company.php?insuid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
        <input type='hidden' id="hdnmode" name='mode' value='add'>
        <input type="hidden" name="insuranceid" value="<?php echo $_REQUEST['insuranceid']; ?> ">
<!--        <input type='hidden' name='insuid' id='insuid' value='<?php echo $id; ?>' />-->
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="insuranceplans_1ton.php?insurance=<?php echo $_REQUEST['insuranceid']; ?>"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="canceladd(event)" >
...
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'INSUCOMP' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
