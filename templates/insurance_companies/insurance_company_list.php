...
if ($result = getInsDatabyPid($insuranceid)) {
    ...
    foreach ($result as $iter) {
        ...
        echo "<tr height='25'><td>";
                print "<a href='insurance_company.php?insuranceid=".$_REQUEST['insuranceid']."&insuid=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
                "&inmode=edit' onclick='insurancetable(event,this,\"Insurance Plans\")' class='css_button_small'><span>".
                htmlspecialchars( xl('Edit'), ENT_NOQUOTES)."</span></a>";
        echo "</td><td>";
        if (acl_check('admin', 'super')) {
            
                echo "<a href='../../interface/patient_file/practice_settings_deleter.php?plan_name=".$iter{"id"}."&insuranceid=".$_REQUEST['insuranceid']."&insuid=".
                        htmlspecialchars( $iter{"id"}, ENT_QUOTES).
                        "'onclick='deleteinsurance1tonrow(event,this,\"insuranceid\",\"WP\")' class='css_button_small'><span>".
        ...
        foreach($field_id as $attr){
            $fres2=sqlStatement("SELECT data_type FROM layout_options WHERE form_id = 'INSUCOMP' AND uor > 0 AND field_id = '$attr'");
