 ...
 /*$sqlBindArray=array();
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_patientinsurancecompany SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
    ...
    $sets .= " insuranceid = ?";
    $id = sqlInsert("INSERT INTO tbl_patientinsurancecompany SET $sets", $sqlBindArray);
...
<body class="body_top" onload="<?php echo $body_onload_code; ?>" >
    <form name='new_insurance_company' method='post' action='insurance_company.php?insuid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a href="insuranceplans_1ton.php?insurance=<?php echo $_REQUEST['insuranceid']; ?>"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
