...
require_once("$srcdir/pharmacy_data_lib.php");
...
$id = empty($_REQUEST['pharmacyid']) ? 0 : $_REQUEST['pharmacyid'] + 0;
$mode    = empty($_POST['mode' ]) ? '' : $_POST['mode' ];
$inmode    = $_GET['inmode'];
$body_onload_code="";

$pharmacyid = $_REQUEST['pharmaid'];
...
if ($mode) {   
 ...
 /*$sqlBindArray=array();
  $fres = sqlStatement("SELECT * FROM layout_options " .
    "WHERE form_id = 'PHA' AND uor > 0 AND field_id != '' " .
  ...
  while ($frow = sqlFetchArray($fres)) {
   ...
   if ($id) {       
    ...
    $sets1= rtrim($sets,',');
    sqlStatement("UPDATE tbl_patientpharmacy SET $sets1 WHERE id = ?", $sqlBindArray);
    }
    else {
    //use sql placemaker
     
    array_push($sqlBindArray,$pharmacyid);
    $sets .= " pharmacyid = ?";
    $id = sqlInsert("INSERT INTO tbl_patientpharmacy SET $sets", $sqlBindArray);
  
  } */
$newdata = array();
$newdata['tbl_patientpharmacy']['pharmacyid'] = $pharmacyid;
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'PHA' AND uor > 0 AND field_id != '' " .
  "ORDER BY group_name, seq");
while ($frow = sqlFetchArray($fres)) {
...
if($id)
{
  updatePharmacy1ton($id, $newdata['tbl_patientpharmacy'] ,$create=false);
}
else
{
    updatePharmacy1ton($id, $newdata['tbl_patientpharmacy'],$create=true);

} 
  if ($GLOBALS['concurrent_layout'])
    $body_onload_code = "javascript:location.href='pharmacy_1ton.php?pharmacy=$pharmacyid';";
  else
    $body_onload_code = "javascript:parent.New_group__service.location.href='pharmacy_1ton.php?pharmacy=$pharmacyid';";
}
...
$trow = $id ? getPhaDatabyId($id) : array();
...
<script type="text/javascript">
   var pharmacytype = '<?php echo $mode; ?>' || "";
   var pharmacynumber = '<?php echo $pharmacyid; ?>' || "";
   if(pharmacytype=="add")
      window.parent.hidepenal(["save",pharmacynumber]);
...
// onclick='return deleteme()'
 dlgopen('../interface/patient_file/deleter.php?pharmacyid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>', '_blank', 500, 450);
 return false;
}
...
function imdeleted() {
 top.restoreSession();
 location.href = 'add_pharmacy.php';
}
...
function validate(f) {
 ...
 var errMsgs = new Array();
    ...
    <?php generate_layout_validation('USERS'); ?>
...
function submitme() {
 var f = document.forms['new_pharmacy'];
 if (validate(f)) {
  f.submit();
 }
}
...
<body class="body_top" >
    <form name='new_pharmacy' method='post' action='pharmacy_data.php?pharmacyid=<?php echo htmlspecialchars( $id, ENT_QUOTES); ?>' onsubmit='return validate(this)'>
        <input type='hidden' id="hdnmode" name='mode' value='add'>
        <input type="hidden" name="pharmaid" value="<?php echo $_REQUEST['pharmaid']; ?> ">
	...
	<table>
	    <tr>
            <td>
             ...
             <td>
                 <a onclick="canceladd(event)" href="pharmacy_1ton.php?pharmacy=<?php echo $_REQUEST['pharmaid']; ?>"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" >
...
<div id="pharmacy_data">
        <ul class="tabNav">
<?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'PHA' AND uor > 0 " .
  "ORDER BY group_name, seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
