...
if ($result = getPhaDatabyPid($_REQUEST['pharmaid'])) {

    foreach ($result as $iter) {

        echo "<tr height='25'><td>";
        print "<a href='pharmacy_data.php?pharmaid=$pharmaid&pharmacyid=".htmlspecialchars( $iter{"id"}, ENT_NOQUOTES).
                "&inmode=edit' onclick='phrmacytable(event,this)' class='css_button_small'><span>".
                htmlspecialchars( xl('Edit'), ENT_NOQUOTES)."</span></a>";
        echo "</td><td>";
        if (acl_check('admin', 'super')) {
                echo "<a href='../../interface/patient_file/deleter.php?pharmacyid=".
                        htmlspecialchars( $iter{"id"}, ENT_QUOTES).
                        "' onclick='deleterow(event,this)' class='css_button_small '><span>".
        ...
        foreach($field_id as $attr){
            $fres2=sqlStatement("SELECT data_type FROM layout_options WHERE form_id = 'PHA' AND uor > 0 AND field_id = '$attr'");
...
//                echo "<td><b>&nbsp;" .
//			"</b></td><td>" ;
//                $pharname =sqlStatement( "SELECT name from pharmacies WHERE id=".$iter{"pharmacyname"});
//                while ($frow = sqlFetchArray($pharname)) {        
