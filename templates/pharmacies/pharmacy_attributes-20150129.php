...
{
    var prevValue = parseInt(document.getElementById('hiddenaddcount_pharmacy['+recid+']').value);   
    document.getElementById('hiddenaddcount_pharmacy['+recid+']').value=prevValue+1;
...
<table border="0" width="100%" bgcolor="#DDDDDD" id="table_pharmacy_attributes">
...
FROM tbl_allcare_pharmacy1to1_fieldmapping mg
INNER JOIN tbl_allcare_tablemeta_pharmacy addr 
...
if($sqlGroupRows>0)
{
         ...
         while($rowGroup=mysql_fetch_array($getGroups))
         {
             ...
             {
                    $columnTypeqry1 =  mysql_query("SHOW COLUMNS FROM tbl_allcare_pharmacy1to1 where Field='".$rowGroupFields['field_Name']."'");
                    while($columnTypeRe = mysql_fetch_array($columnTypeqry1)) 
                    {
                       ...
                       if( ($first3letters=='int'  || $first3letters=='flo' || $first3letters =='boo'  || $first3letters=='big' || $first3letters=='tin') )
                       {
                        ...
                        if ($_POST['pharmacy_id']) 
                        {
                             //echo "select ".$rowGroupFields['field_Name']." from tbl_allcare_pharmacy1to1 where pharmacy_id=".$userid;
                           $columnvalueqry1 =  mysql_query("select ".$rowGroupFields['field_Name']." from tbl_allcare_pharmacy1to1 where pharmacy_id=".$_POST['pharmacy_id']);
...
?>
        ...
        $getRecordSet = mysql_query("SELECT DISTINCT mg.Recordset_Name,mg.Recordset_ID
                                        FROM tbl_allcare_pharmacy1ton_fieldmapping mg
                                        INNER JOIN tbl_allcare_tablemeta_pharmacy addr 
        ...
        if($sqlRecordsetRows>0)
        {
                 ...
                 while($rowRecordSet=mysql_fetch_array($getRecordSet))
                 {
                    ...
                    //$cnt++;
                     ...
                     $getRecordsetSql="SELECT mg.Recordset_Name,mg.Recordset_ID,addr.table_Name,addr.field_Name
                                    FROM tbl_allcare_pharmacy1ton_fieldmapping mg
                                    INNER JOIN tbl_allcare_tablemeta_pharmacy addr 
                                    ON addr.Field_ID=mg.Field_ID 
                    ...
                    if($_POST['pharmacy_id'])
                    {
                     $updateRecorsetSql = "select $RecordsetFieldNames from tbl_allcare_pharmacy1ton where Recordset_ID=$rowRecordSet[Recordset_ID] and pharmacy_id=".$_POST['pharmacy_id'];
                    ...
                    {
                        echo "<input type='hidden' name='hiddenaddcount_pharmacy[$rowRecordSet[Recordset_ID]]' id='hiddenaddcount_pharmacy[$rowRecordSet[Recordset_ID]]' value='".$updateRecorsetRows."' />";
                        $cnt1=0;
                           ...
                           // echo "<pre>";print_r($updateRecorsetRes);echo "</pre>";
                            echo "<tr>";
                              for ($cnt2=0;$cnt2<$colcount;$cnt2++)
                              {
                                  ...
                                  $columnTypeqry2 =  mysql_query("SHOW COLUMNS FROM tbl_allcare_pharmacy1ton where Field='".$splitNames[$cnt2]."'");
                    ...
                    {
                        echo "<input type='hidden' name='hiddenaddcount_pharmacy[$rowRecordSet[Recordset_ID]]' id='hiddenaddcount_pharmacy[$rowRecordSet[Recordset_ID]]' value='1' />";

                        echo "<tr>";
                            while($rowRecordsetFields=mysql_fetch_array($getRecordsetFields))
                            {

                                $columnTypeqry2 =  mysql_query("SHOW COLUMNS FROM tbl_allcare_pharmacy1ton where Field='".$rowRecordsetFields['field_Name']."'");
                                   while($columnTypeRes = mysql_fetch_array($columnTypeqry2)) 
                                   {
                                               $validationpart1='';
...
<input type='hidden' name='hiddensqlGroupRows_pharmacy' value='$sqlGroupRows' >
<input type='hidden' name='hiddenarrayGroup_pharmacy' value='$GroupFieldNames' >
