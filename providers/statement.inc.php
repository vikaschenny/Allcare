...
function create_statementpdf($stmt,$agecolmns=5, $ageinc=30,$is_ageby_lad) {
	global $pdf;
        $console = "";
 if (! $stmt['pid']) return ""; // get out if no data
...
$out .= "\n";
 ...
 // be specified in the order used.
 //        
 //$print = print_r($stmt['lines']);
 
  // this function or create ageing columns
 $agetable = "";
 if ($agecolmns > 0 && $agecolmns < 50) {
    if ($agecolmns <= 0) $agecolmns = 30;
  } else {
    $agecolmns = 0;
    $ageinc  = 0;
  }
 ...
 foreach ($stmt['lines'] as $line) {
  ...
  $today = date("Y-m-d");
  ksort($line['detail']);
  $datetodays = round((strtotime($today) - strtotime($date)) /
        (24 * 60 * 60));
  $leedval= $is_ageby_lad === 0?$stmt['last']:$datetodays;
  $agecolno = min($agecolmns - 1, max(0, floor($leedval / $ageinc)));
  ...
  $pamout = number_format(($line['amount'] - $line['paid']),2);
  $paid = $line['paid'];
  $adj = $line['adjust'];
  $dxCode = $line['dxCode'];
  $destext = $line['code_text'];
  if (array_key_exists((string)$agecolno, $agearray))
      $agearray[(string)$agecolno] = ($agearray[(string)$agecolno] + $pamout);
  else
      $agearray[(string)$agecolno] = $pamout;
  
  
  $statements .="<tr><td align=\"center\" >$date</td><td>$tmp</td><td>$destext</td><td align=\"right\">$dxCode</td><td align=\"right\">$change</td><td align=\"right\">$paid</td><td align=\"right\">$adj</td><td align=\"right\">$pamout</td></tr>";
 }
 ksort($agearray);
 //print_r($agearray);
 // This generates blank lines until we are at line 42.
...
for ($c = 0; $c < $agecolmns;) {
    ...
    if($c%5==0)$agetable .='<table border="1" cellpadding="3"><tr>';
    $agetable .=  "<td class=\"daystable\"><b>";
    $headerval="";
    $startval = ($ageinc * $c);
    //table header
    if (++$c < $agecolmns)
       $headerval = $startval."-".($ageinc * $c - 1)." DAYS";
    else
       $headerval = "OVER ".$startval." DAYS";
    
    $agetable .= "$headerval</b></td>";//end table header
    //start table values
    if($c%5==0){
        $agetable .= "</tr><tr>";
        $a=($c-5);
        while($a<$c){
            //get age colum number and value
            $agecolno = -1;
            foreach ($agearray as $akey => $avalue) {
                if($a==(int)$akey){
                   $agecolno = $akey;
                   $agevalue = $avalue;
                   break;
            ...
            $agetable .= "<td class=\"daystable\"><b>";
            $agetable .= $a==$agecolno?"$$agevalue</b></td>":"$0.00</b></td>";
            $a++;
        }
        $agetable .= "</tr></table>";  
    }
    
    if($c == ($agecolmns) && $c%5!=0){
        $agetable .= "</tr><tr>";
        $a=($c - ($c%5));
        while($a<$c){
            //get age colum number and value
        ...
        $agetable .= "</tr></table>";//end table values
    }
				...
				<table border="0" cellpadding="0">
					...
					<tr>
						<td colspan="2">
							...
							<thead>
								<tr>
									<th width="11%"><b>DATE</b></th>
									<th width="11%"><b>PROCEDURE CODE</b></th>
									<th width="23%"><b>DESCRIPTION</b></th>
                                                                        <th width="11%"><b>DIAGNOSIS</b></th>
                                                                        <th width="11%"><b>CHARGE</b></th>
                                                                        <th width="11%"><b>PAID</b></th>
                                                                        <th width="11%"><b>ADJ</b></th>
                                                                        <th width="11%"><b>BALANCE</b></th>
								</tr>
							</thead>
							$statements
							</table>
				...
				<table border="0" cellpadding="0">
					...
					<tr>
						<td colspan="2">
							...
							$statements
							<tr class="borderbot"><td height="$setheight"></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
							</table>
						</td>						
					</tr>
					<tr>
...
//$pdf->Write(0, $print, '', 0, '', true, 0, false, false, 0);
 return $out;
}
?>
