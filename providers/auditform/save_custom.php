...
$encounter      = isset($_REQUEST['encounter']) ?$_REQUEST['encounter'] : $_SESSION['encounter'];
...
if($sqllbf)
    save_form_flag($lbf_formid, $status);
...
$sets = "pid = {$_REQUEST["pid"]},
    authProvider = '" . $_REQUEST["provider"] . "',
    user = '" . $_REQUEST["provider"] . "',
...
if(!empty($auditform_comments)){
    ...
    if(mysql_num_rows($get_layout_comments)){
        ...
        $form_id = sqlInsert("INSERT INTO forms  (pid, encounter,DATE, form_name,formdir,deleted,authorized,user) VALUES($pid, $encounter, NOW(),'Allcare Encounter Forms', 'LBF2',0,$userauthorized,'".$provider."')");
...
if($isSingleView == 1 && $isFromCharts == 0)
    ...
    echo "<script> window.close(); 
        window.opener.location.href = '../../../providers/provider_incomplete_charts.php?provider=".$_SESSION['provider']."&checkencounter=".$encounter."';</script>";

function save_form_flag($lbf_formid,$status){
    
    if($lbf_formid !== 0 ){
      $formid = $lbf_formid; 
    }
    if(empty($status)){
        $result = mysql_query("SELECT audited_status FROM form_encounter where id = $formid ");
        while ($row = mysql_fetch_array($result,MYSQL_ASSOC)){
            $status =  $row['audited_status'] ; 
        } 
    }
    $logdata= array();
...
//    $finalized = $_POST['finalized'];
    ...
    if(empty($_REQUEST['id'])):
            $array2[] = array( 'authuser' =>$_REQUEST["provider"],'Status' => $status,'date' => date("Y/m/d"), 'action'=>'updated','ip_address'=>$ip_addr.'(Provider Portal Audit Form)' ,'count'=> $count+1);
            $logdata = array_merge_recursive($array, $array2);
            $logdata= ($logdata? serialize($logdata): serialize($array2) );
            sqlInsert("INSERT into `tbl_allcare_formflag`(`form_id`, `encounter_id`, `form_name`, `logdate`) VALUES(".$formid.",".$_REQUEST['encounter'].",'Patient Encounter','".$logdata."')");
    else: 
            ...
            if(mysql_num_rows($result) > 0){
                    ...
                    $array2[] = array( 'authuser' =>$provider,'Status' => $status, 'date' => date("Y/m/d"), 'action'=>'updated','ip_address'=>$ip_addr.'(Provider Portal Audit Form)' ,'count'=> $count+1);
                    $logdata = array_merge_recursive($array, $array2);
                    $logdata= ($logdata? serialize($logdata): serialize($array2) );
                    sqlInsert("INSERT into `tbl_allcare_formflag`(`form_id`, `encounter_id`, `form_name`, `logdate`) VALUES(".$formid.",".$_REQUEST['encounter'].",'Patient Encounter','".$logdata."')");
            }
