...
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'LBF2' AND uor > 0 AND field_id != '' AND group_name LIKE '%$gname%' AND field_id LIKE '%_stat%'  " .
  "ORDER BY seq");
$field_id1=array();
...
function dopclick(id,category,pid,location) {
    ...
    window.open('add_edit_issue_custom.php?issue=' + encodeURIComponent(id) + '&thistype=' + encodeURIComponent(category) + '&thispid='+pid+'&thislocation='+location, '_blank', 550, 400);
    <?php //else: ?>
...
<form method='post' action='stats_full_custom.php' onsubmit=''>
...
foreach ($ISSUE_TYPES as $focustype => $focustitles) {
  ...
  $pres = sqlStatement("SELECT * FROM lists WHERE pid = ? AND type = ? $condition" .
   "ORDER BY begdate", array($patient_id,$focustype) );
...
<div class="tabContainer">							
 <?php
$fres = sqlStatement("SELECT * FROM layout_options " .
  "WHERE form_id = 'LBF2' AND uor > 0 AND group_name LIKE '%$gname%' AND field_id LIKE '%_stat%'" .
  "ORDER BY  seq");
$last_group = '';
$cell_count = 0;
$item_count = 0;
