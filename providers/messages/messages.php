...
$showall = isset($_REQUEST['show_all']) ? $_REQUEST['show_all'] : "" ;
if ($showall == "yes") {
    $show_all = $showall;
}
else
...
if ($show_all=='yes') {
    $showall = "yes";
    $lnkvar="'home.php?show_all=no&$activity_string_html' name='Just Mine' onclick=\"\"> (".htmlspecialchars( xl('Just Mine'), ENT_NOQUOTES).")";
}
else {
    $showall = "no";
    $lnkvar="'home.php?show_all=yes&$activity_string_html' name='See All' onclick=\"\"> (".htmlspecialchars( xl('See All'), ENT_NOQUOTES).")";
}
//}
?>
<br>
<table><tr><td><b><span class="title"><?php echo htmlspecialchars( xl('Messages'), ENT_NOQUOTES); ?></span></b> <a class='more' href=<?php echo $lnkvar; ?></a></td></tr></table>
<?php
...
if (empty($task) || $task=="add" || $task=="delete") { ?>
  ...
  <?php } else { ?>
    <a href="home.php" class="link" onclick=""><span><?php echo xlt('Show All'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '1') { ?>
    <span><?php echo xlt('Show Active'); ?></span>
  <?php } else { ?>
    <a href="home.php?form_active=1" class="link" onclick=""><span><?php echo xlt('Show Active'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '0') { ?>
    <span><?php echo xlt('Show Inactive'); ?></span>
  <?php } else { ?>
    <a href="home.php?form_inactive=1" class="link" onclick=""><span><?php echo xlt('Show Inactive'); ?></span></a>
  <?php } ?>
<?php } ?>
...
switch($task) {
    case "add" :
    {
        ...
        $assigned_to_list = explode(';', $_REQUEST['assigned_to']); 
    ...
    case "save" : {
        ...
        $task = "edit";
        $note = $_REQUEST['note'];
        $title = str_replace("_"," ",$_REQUEST['form_note_type']);
        $reply_to = $_REQUEST['reply_to'];
    }
    case "edit" : {
        if ($noteid == "") {
...
//       
//        echo $delete_id = $_REQUEST['delete_id'];
//       
...
echo "
<form name=new_note id=new_note action=\"home.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&provider=".attr($provider)."&$activity_string_html\" method=post>
...
if ($noteid) {
  ...
  if (sqlNumRows($tmp)) {
    ...
    while ($gprow = sqlFetchArray($tmp)) {
      ...
      echo "&as_file=true' target='_blank' onclick=''>";
...
$(document).ready(function(){
    ...
    var PrintNote = function () {
       // top.restoreSession();
        window.open('messages/pnotes_print.php?noteid=<?php echo htmlspecialchars( $noteid, ENT_QUOTES); ?>', '_blank', 'resizable=1,scrollbars=1,width=600,height=500');
    }
...
<?php } ?>
 ...
 function sel_patient() {
  ...
  window.open('calendar/find_patient_popup.php', '_blank', 500, 400);
 }
  ...
  function addtolist(sel){
    ...
    for($i = 0; $i < count($sort); $i++) {
        $sortlink[$i] = "<a href=\"home.php?show_all=".attr($showall)."&sortby=".attr($sort[$i])."&sortorder=asc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>";
    }
    for($i = 0; $i < count($sort); $i++) {
        if($sortby == $sort[$i]) {
            switch($sortorder) {
                case "asc"      : $sortlink[$i] = "<a href=\"home.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=desc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortup.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>"; break;
                case "desc"     : $sortlink[$i] = "<a href=\"home.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=asc&$activity_string_html\" onclick=\"\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Down'), ENT_QUOTES)."\"></a>"; break;
            } break;
        }
    }
    ...
    if($prev >= 0) {
        $prevlink = "<a href=\home.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($prev)."&$activity_string_html\" onclick=\"\"><<</a>";
    }
    else {
        $prevlink = "<<";
    }
    ...
    if($next < $total) {
        $nextlink = "<a href=\"home.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($next)."&$activity_string_html\" onclick=\"\">>></a>";
    }
    else {
        $nextlink = ">>";
    }
    ...
    <form name='MessageList' id='MessageList' action=\"home.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&$activity_string_html\" method=post>
        ...
        while ($myrow = sqlFetchArray($result)) {
            ...
            <tr id=\"row$count\" style=\"background:white\" height=\"24\">
	          ...
	          htmlspecialchars( $name, ENT_NOQUOTES) . "</td><td width=5></td></tr></table></td>
                <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\"><a href=\"home.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&task=edit&noteid=" .
	          htmlspecialchars( $myrow['id'], ENT_QUOTES) . "&$activity_string_html\" onclick=\"\">" .
    ...
    <table border=0 cellpadding=5 cellspacing=0 width=90%>
        <tr>
            <td class=\"text\"><a href=\"home.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&task=addnew&$activity_string_html\" onclick=\"\">" .
              htmlspecialchars( xl('Add New'), ENT_NOQUOTES) . "</a> &nbsp; <a href=\"javascript:confirmDeleteSelected()\" onclick=\"\">" .
...
function confirmDeleteSelected() {
    if(confirm("<?php echo htmlspecialchars( xl('Do you really want to delete the selection?'), ENT_QUOTES); ?>")) {
      var delete_id1='';
         ...
         for($i = 1; $i <= count; $i++) {
             var val=$("#check"+$i).attr("checked") ? "Checked" : "Unchecked";
             if(val=='Checked'){
                 var chk_val=jQuery("#check"+$i).val();
                 
                 delete_id1+=chk_val+",";
                
               // alert(facilityDataArray);
             }
        }
        if(delete_id1!=''){
            $.ajax({
		type: 'POST',
		url: "messages/delete_msg.php",	
                //data:{delete_id1:delete_id1,des:des},
                data:{delete_id1:delete_id1},
