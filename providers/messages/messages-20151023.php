...
echo "
<form name=new_note id=new_note action=\"home.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&$activity_string_html\" method=post>
...
$ures = sqlStatement("SELECT username, fname, lname FROM users " .
 ...
 "( info IS NULL OR info NOT LIKE '%Inactive%' ) AND id=$provider_id  " .
 "ORDER BY lname, fname");
