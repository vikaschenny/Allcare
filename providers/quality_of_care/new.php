...
{ 
    ...
    updateENC_forms($form_id, $newdata['lbf_data'] ,$create=false,$ecounter,$pid1,$provider);
    if(count($newValue) > 0)
    {   
        updateENC_forms($form_id, $newValue ,$create=true,$ecounter,$pid1,$provider);
    }
...
{  
   if(!empty($fid)){
        ...
        updateENC_forms($form_id, $newdata['lbf_data'] ,$create=true,$ecounter,$pid1,$provider);
    }
...
<form name='new_f2f' method='post' action='new.php?form_id=<?php echo htmlspecialchars( $form_id, ENT_QUOTES); ?>&enc3=<?php echo htmlspecialchars( $ecounter, ENT_QUOTES); ?>&file_name1=<?php echo htmlspecialchars( $file_name, ENT_QUOTES); ?>&fname1=<?php echo htmlspecialchars( $form_name, ENT_QUOTES); ?>&pid1=<?php echo htmlspecialchars( $pid1, ENT_QUOTES); ?>&groupname=<?php echo htmlspecialchars( $groupname, ENT_QUOTES); ?>&provider=<?php echo $provider; ?>&location=<?php echo $location ; ?>'onsubmit='return validate(this)'>
	    ...
	    <tr>
            <td>
             ...
             <?php  require_once '../Mobile_Detect.php';
                    ...
                    if($detect->isMobile()) {  ?>   
                       <a href="javascript:;"  <?php if (!$GLOBALS['concurrent_layout']) echo "target='Main'"; ?> class="css_button" onclick="window.opener.location.href = '../provider_incomplete_charts.php';">
...
<br>
    <?php  
                ...
                $dos_sql=sqlStatement("select * from form_encounter where encounter='".$res_row1['encounter']."'");
                ...
                echo "<table>";
                echo "<tr><td><b>Patient Name: </b>".$res_row1['pname']."</td><td>&nbsp;</td><td><b>Encounter: </b>".$res_row1['encounter']."</td></tr>";
