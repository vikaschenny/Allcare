...
//            echo "<div class='newwindow'><a href='javascript:;' onclick=win1('Issues/stats_full_custom.php?active=all&category=$grouptitle&encounter=$encounter&pid=$pid&formid=$lbf_form_id&location=provider_portal&provider=$provider&isSingleView=1') ><span>".str_replace("_"," ", ucwords($group))."</span></a></div>";
            ...
            <div id='f2fdiv'>
                ...
                $fres = sqlStatement("SELECT * FROM layout_options WHERE form_id IN('HIS','LBF2') AND field_id IN($history_required_fields,$required_fields) AND ((group_name LIKE '%$groupname') OR (group_name LIKE '%$group')) ORDER BY group_name, seq");
                $last_group = '';
                $cell_count = 0;
                $item_count = 0;
                $display_style = 'block';
