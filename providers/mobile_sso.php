...
$sql1= sqlStatement("SELECT title FROM `list_options` where list_id='AllCareSSO' and option_id='provider_google_sso'");
$sql1_exc = sqlFetchArray($sql1);
$res = $sql1_exc[title];
if($res == 'enabled'){
	$sql_scrt = sqlStatement("SELECT title FROM `list_options` where list_id='AllCareSSO' and option_id='provider_google_secret'");
	$scrt_query = sqlFetchArray($sql_scrt);
	$client_secret = $scrt_query[title];

	$sql2 = sqlStatement("SELECT title FROM `list_options` where list_id='AllCareSSO' and option_id = 'provider_google_redirect'");
	$sql2_exc = sqlFetchArray($sql2);
	$redirect = $sql2_exc[title];

	$sql3 = sqlStatement("SELECT title FROM `list_options` where list_id='AllCareSSO' and option_id = 'provider_google_client_id'");
...
//	}
	if(!empty($redirect) && !empty($client_id) && !empty($client_secret)){
		...
		$client->setApplicationName("Provider Portal"); // Set your applicatio name
		...
		if(isset($_SESSION['gplusuer'])){ 
                    ...
                    if(!empty($id_user) && !empty($name_user)){
                        ...
                        $usercount=mysql_num_rows($query_two); echo "test";
...
function callProvider($id_user,$username){
    ...
    $sql5 = sqlStatement("SELECT title FROM `list_options` where list_id='AllCareSSO' and option_id = 'provider_google_landing'");
	$sql5_exc = sqlFetchArray($sql5);
...
<!--             <input type='hidden' name='ggllogin' value='1'>-->
             <input type='hidden' name='mbl_ggllogin' value='1'>
	</form>
