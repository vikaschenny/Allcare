...
/* Things that might be passed by our opener. */
 ...
 $portalprovider=$_REQUEST['provider'];
 //
 if ($date)
   ...
   global $event_date,$info_msg;
	 ...
	 if ($GLOBALS['auto_create_new_encounters'] && $_POST['form_apptstatus'] == '@' && $event_date <= date('Y-m-d'))
	 {
		 ...
		 // Check if encounter already exists
                 $dateObj = date_create($event_date);
                 $encCheck  = sqlStatement("SELECT encounter FROM form_encounter WHERE pid = ". $_POST['form_pid'] . "
                                            AND date = '". date_format($dateObj,'Y-m-d H:i:s')."'");
                 $encCount = sqlNumRows($encCheck);
                 if($encCount):
                     $encQuery = sqlFetchArray($encCheck); // Get existing encounter ID for updating Rendering Provider
                     $encounter = $encQuery['encounter'];
                 else:    
                     $encounter = todaysEncounterCheckProviderPortal($_POST['form_pid'], $event_date, $_POST['form_comments'], $_POST['facility'], $_POST['billing_facility'], $_POST['form_provider'], $_POST['form_category'], false);
                     $info_msg .= xl("New encounter created with id"); 
                     $info_msg .= " $encounter";
                 endif;
                 
                    if($encounter){
                       $queryStr1 =  sqlStatement("SHOW COLUMNS FROM `form_encounter` LIKE 'rendering_provider'");
                       $rowCount = sqlNumRows($queryStr1);
                       if($rowCount):
                           sqlStatement("UPDATE form_encounter SET rendering_provider = '".$_POST['form_provider']."' WHERE encounter = ".$encounter);
                       endif;
                    }
	 }
 }
...
// }
if ($_POST['form_action'] != "") {
    $check_msg  = addslashes($info_msg); 
    $encounter  = trim(str_replace("New encounter created with id ","",$check_msg));
    $pid        = $_REQUEST['form_pid'];
    // Close this window and refresh the calendar display.
    echo "<html>\n<body>\n<script language='JavaScript'>\n";
    if ($check_msg) echo " alert('" . addslashes($check_msg) . "');\n";
    echo " if (opener && !opener.closed && opener.refreshme) opener.refreshme();\n";
    // check whether the redirect to singleview form is tuned on or off
    if(!empty($encounter)){
        $get_is_Single_view = sqlStatement("SELECT title FROM list_options WHERE list_id='allcareConfig' AND option_id='AppointmentSingleViewRedirect' ");
        while($set_is_Single_view = sqlFetchArray($get_is_Single_view)){
            if(trim($set_is_Single_view['title']) == 'yes' || trim($set_is_Single_view['title']) == 'Yes' || trim($set_is_Single_view['title']) == 'YES')
                echo " window.open('single_view_form.php?encounter=$encounter&pid=$pid', '_blank', 500, 400);";
        }
    }
    // end of single view redirect screen code
    echo " window.close();\n";
    echo "</script>\n</body>\n</html>\n";
    exit();
 }
...
*********************************************************************/
 ...
 $ures = sqlStatement("SELECT id, username, fname, lname FROM users WHERE " .
  "authorized != 0 AND active = 1 AND id=$portalprovider ORDER BY lname, fname");
 ...
 function setappt(year,mon,mday,hours,minutes) {
  ...
  f.form_minute.value = ('' + (minutes + 100)).substring(1);
	...
	var formDate = document.forms[0].form_date;
        window.open('<?php echo $GLOBALS['web_root']; ?>/providers/appointments/find_appt_popup.php' +
                '?providerid=' + s +
    ...
    function getptwindow(){
        var transprentwindow = window.parent.document.getElementsByClassName('fancybox-overlay');
        $(transprentwindow).hide();
        $(window.parent.document).find('html').removeClass('fancybox-margin fancybox-lock');
    }
...
<form method='post' name='theform' id='theform' action='edit_appointment.php?eid=<?php echo attr($eid) ?>' />
...
?>
		 <li <?php echo $normal;?>>
		 <a href='edit_appointment.php?eid=<?php echo attr($eid);?>&startampm=<?php echo attr($startm);?>&starttimeh=<?php echo attr($starth);?>&userid=<?php echo attr($uid);?>&starttimem=<?php echo attr($starttm);?>&date=<?php echo attr($dt);?>&catid=<?php echo attr($cid);?>&provider=<?php echo $portalprovider; ?>'>
		 <?php echo xlt('Patient');?></a>
		 </li>
		 <li <?php echo $provider_class;?>>
		 <a href='edit_appointment.php?prov=true&eid=<?php echo attr($eid);?>&startampm=<?php echo attr($startm);?>&starttimeh=<?php echo attr($starth);?>&userid=<?php echo attr($uid);?>&starttimem=<?php echo attr($starttm);?>&date=<?php echo attr($dt);?>&catid=<?php echo attr($cid);?>&provider=<?php echo $portalprovider; ?>'>
...
function SubmitForm() {
 ...
 <?php if (!($GLOBALS['select_multi_providers'])) { // multi providers appt is not supported by check slot avail window, so skip ?>
  if (f.form_action.value != 'delete') {
    ...
    f.submit();
    window.parent.location.href = 'home.php?provider=<?php echo $_SESSION['portal_username']; ?>'; parent.$.fancybox.close();  
  }
 <?php } else { ?>
...
<?php
function todaysEncounterCheckProviderPortal($patient_id, $enc_date = '', $reason = '', $fac_id = '', $billing_fac = '', $provider = '', $cat = '', $return_existing = true){
  global $today;
  ...
  $tmprow = sqlQuery("SELECT name FROM facility WHERE id = ?", array($fac_id) );
  $facility = $tmprow['name'];
