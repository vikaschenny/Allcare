...
function eligibility_table($form_id,$field_id,$field_val,$pid,$month,$username,$patient_bal,$insurance_bal,$total_bal){
    if($form_id == 0){
        $insert_elig_table = sqlStatement("INSERT INTO tbl_eligibility_response_data (pid,month, created_date, updated_date, user, domain, patient_bal, insurance_bal, total_bal,`$field_id`) VALUES($pid,'$month', NOW(), NOW(), '$username', 'Provider_Portal','$patient_bal','$insurance_bal','$total_bal','".addslashes($field_val)."')");
