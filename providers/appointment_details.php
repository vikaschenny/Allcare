...
?>
    ...
    <?php $sql_pdata=sqlStatement("SELECT ope.*,pc_startTime,p.fname,p.lname ,lo.title as stat,p.*
              ...
              WHERE ope.pc_aid='$id'  AND ope.pc_eventdate='$date' and p.practice_status = 'YES' AND p.deceased_stat != 'YES' AND (p.deceased_date = '' OR p.deceased_date = '0000-00-00 00:00:00' ) and ope.pc_catid IN ($enc_value) GROUP BY  ope.pc_eid");    
     $d1=explode("-",$date);
     $d2=$d1[0].$d1[1].$d1[2];
     $count = 0;
...
<script>
    $('.mapV').click(function(){
        ...
        window.open('map.php?hdnAddressBack1='+addr+'&pid='+this.id,'mapV','width=800,height=700');
    });
</script>