...
$sql=sqlStatement("insert into incomplete_charts_filter (provider,patient,audit_status,encounter_status,`from`,`to`,facility,visit_category) values('$provider','$patient_val1','$audit_val1','$enc_val1','$from','$to','$fac_val1','$vc_val1')");
 
} else{
    $sql1=sqlStatement("select * from incomplete_charts_filter where provider='$provider' order by id desc");
...
function VisitsWithNoForms($resid1,$respid1,$resenc1,$resfname1,$ros,$pe,$allcare,$audit_stat,$provider,$menu,$id1)
       {
                        ...
                        $fuv_sql=sqlStatement("SELECT DISTINCT (
                                        ...
                                        WHERE  form_encounter.encounter=$resenc1 AND form_encounter.provider_id=$id1 GROUP BY f.screen_group ORDER BY f.id DESC"); 
                                $i=0;
                                                   ...
                                                   if($field_id=='chief_complaint' || $field_id=='hpi' || $field_id=='assessment_note' || $field_id=='progress_note' || $field_id=='plan_note' ){
                                                        ...
                                                        if($priority=='Required'){
                                                          // echo "select count(*) as cnt from lbf_data  where field_id LIKE  '%$field_id1%' AND field_value='finalized' AND form_id='".$resid1."'";
                                                            $ra = sqlStatement("select field_value from lbf_data  where field_id LIKE  '%$field_id1%' AND field_value='finalized' AND form_id='".$resid1."'");
                                                             while($frowa = sqlFetchArray($ra)){
                                                               $a=explode("|",$res_row1['field_value']);
                                                             }
                              ...
                              <select name='pc_catid[]' multiple id='pc_catid'>
                                <option value='' selected >-- <?php echo xlt('ALL'); ?> --</option>
                                 ...
                                 <td>
                                  ...
                                  <input type="submit" id='def' name='def' value="Default" /></div>
                             </form>
                           </div>
                                                                                 ...
                                                                                 if($field=='facility' && in_array($field, $avail3)){
                                                                                  echo "<td>";if($result1[$field]!='')  $pro_id2=sqlStatement("select name from facility where id='".$result1[$field]."' ");
                                                                                     ...
                                                                                     echo "<td>"; if($result1[$field]!='') $pro_id1=sqlStatement("select pc_catname from openemr_postcalendar_categories where pc_catid='".$result1[$field]."' ");
                                                                                      ...
                                                                                      echo "<td>"; echo "<form id='audit_note' name='audit_note' action='' method='post'> <textarea name='form_audit_note' id='form_audit_note' title='' cols='60' rows='10'>$result1[$field];</textarea> </form>"; echo "</td>";
                                                                                 }elseif(in_array($field, $avail3)){
                                                                                       echo "<td>".$result1[$field]."</td>";
                                                                                 }
                                                                              }
