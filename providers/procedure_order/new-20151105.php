...
$id1=$id['id'];
 ...
 $form_id=$_REQUEST['formid'];// for staus finalized/pending
...
while ($frow = sqlFetchArray($fres)) {
 ...
 $res1=sqlstatement("select * from lbf_data lb INNER JOIN layout_options l ON l.field_id=lb.field_id where lb.form_id='$form_id' AND l.form_id='LBF2' AND l.group_name LIKE '%PProcedure%' AND lb.field_id LIKE 'procedure_stat' order by seq");
 $res_row1=sqlFetchArray($res1);
 if(!empty($res_row1)){
     updateENC_forms($form_id, $newdata['lbf_data'] ,$create=false,$encounter,$pid,$provider);
 }else{
      updateENC_forms($form_id, $newdata['lbf_data'] ,$create=true,$encounter,$pid,$provider);
 }
 }
...
if ($_POST['bn_save'] || $_POST['bn_xmit']) {
  ...
  function formJump_custom($provider)
{
	echo "\n<script language='Javascript'> window.close(); window.opener.location.href = '../provider_incomplete_charts.php?provider=$provider';</script>\n";
	
}
  formJump_custom($provider);
  formFooter();
  exit;
}
 ...
 ********************************************************************/
 ...
 window.open('../../interface/orders/find_order_popup.php' +
  '?labid=' + f.form_lab_id.value +
  '&order=' + f[ptvarname].value +
  '&formid=<?php echo $formid; ?>' +
  '&formseq=' + formseq,
...
<form method="post" action="new.php?id=<?php echo $formid ?>&encounter=<?php echo $encounter; ?>"
