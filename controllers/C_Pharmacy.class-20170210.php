...
require_once($GLOBALS['fileroot'] ."/library/classes/PharmacyFolderCreation.class.php");
//require_once($GLOBALS['fileroot'] ."/library/classes/PharmacyCentralDB.class.php");
class C_Pharmacy extends Controller {
	...
	function edit_action_process() {
		...
		//echo $this->pharmacies[0]->toString(true);
                ...
                $phid= $this->pharmacies[0]->get_id();
                if($status=='new_one') {
                    $ph_iobj=$this->pharmacies[0];
                    $ph_iobj  = new PharmacyFolderCreation;
                    $ph_iobj->curlQuery1($phid);
                    
                }
                
                // Add/update pharmacy in Central DB
//                $pharmacyObj  = new PharmacyCentralDB;
//                $pharmacyObj->processPharmacy($phid);
