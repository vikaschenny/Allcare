...
require_once($GLOBALS['fileroot'] . "/library/classes/InsuranceCompanyFolderCreate.class.php");
require_once($GLOBALS['fileroot'] . "/library/classes/InsuranceCompanyCentralDB.class.php");
class C_InsuranceCompany extends Controller {
	...
	function edit_action_process() {
		...
		$this->icompanies[0]->persist();
                //custom code start
                $ins_id=$this->icompanies[0]->get_id();
                if($status=='new_one') {
                     $custom_ins=$this->icompanies[0];
                     $custom_ins = new InsuranceFolderCreation;
                     $custom_ins->curlQuery($ins_id);
                     
                }
               
                //ends
                // Add/update Insurance Company in Central DB
                $insuranceObj  = new InsuranceCompanyCentralDB;
                $insuranceObj->processInsurance($ins_id);
